...
|---|---|
| 要件ID | `NFR-MAINT-006` |
| 要件名 | Tickの使用を必要なクラスに限定する |
| 要件種別 | 非機能要件 |
| 要件カテゴリ | 保守性 |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「Tickの使用を必要なクラスに限定する」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
| 関連Issue | 未割当 |
| 関連PR | 未割当 |
...
- 元要件「Tickの使用を必要なクラスに限定する」の判定基準、検証条件、証跡を定義する。
- Event・Timer・Delegate・AI Serviceで代替し、必要なTickも非戦闘・非表示時に停止する。
...
| `Performance / Architecture Review` | 計測・レビュー・テスト結果を収集して合否を判定する |
...
- Tick有効クラスごとに目的・頻度・停止条件が文書化されているか確認する。
...
4. Event・Timer・Delegate・AI Serviceで代替し、必要なTickも非戦闘・非表示時に停止する。
5. 承認済みTickと負荷記録を証跡とともに記録する。
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 検証対象 | Build / Source / Asset / Save Fixture | Yes | Tick利用一覧とProfiler結果 |
| 実行条件 | Hardware Profile / Test Matrix / Checklist | Yes | Tick有効クラスごとに目的・頻度・停止条件が文書化されているか確認する |
| 比較基準 | 数値閾値 / 0件条件 / Architecture Rule | Yes | Tick Interval / Enable Policy |
...
| 証跡 | Report / Log / Trace / Screenshot / Checklist | 承認済みTickと負荷記録 |
...
| クラス・システム | 責務 | 実装区分 |
|---|---|---|
| `Performance / Architecture Review` | 条件固定、収集、比較、判定 | `Tool / C++ / Test` |
...
FVerificationResult VerifyAuditTickUsage(const FVerificationContext& Context);
bool MeetsAuditTickUsageRequirement(const FVerificationResult& Result);
```
...
|---|---|---:|---|---|
| Tick Interval / Enable Policy | Test Config / Number / Rule | 原則Disabled | 設計規約 / Data Asset | 合否基準または検証条件 |
...
| 種別 | 内容 |
|---|---|
| 開発UI | Unreal InsightsでTick Function別負荷を確認する |
...
- [ ] 新規Tick利用に必要性と計測根拠があり、毎Frameの全Actor検索を含まない。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-NFR-MAINT-006-001` | 基準確認 | すべての前提条件が固定済み | Tick利用一覧とProfiler結果で検証する | 新規Tick利用に必要性と計測根拠があり、毎Frameの全Actor検索を含まない |
| `TC-NFR-MAINT-006-002` | 再現性 | 同じBuild・環境・手順 | 別担当者が再実施する | 許容差内で同じ判定となる |
| `TC-NFR-MAINT-006-003` | 条件不備 | 必須Metadataまたは証跡なし | 判定を要求する | PassにせずBlockedまたは測定不能となる |
...
|---|---|---|---|---|
| `ADR-NFR-MAINT-006-001` | 検証方式 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | 担当者の主観だけで合否を決める | 再現性、比較可能性、Release判断の追跡性を保つため |
...
|---:|---|---|---|
| 1 | `NFR-MAINT-006: 検証条件・Fixture整備` | 環境、手順、期待値、証跡形式が確定する | なし |
| 2 | `NFR-MAINT-006: 計測・レビュー・試験` | 基準Buildの結果と証跡が揃う | #1 |
| 3 | `NFR-MAINT-006: 未達修正と再確認` | Must/Should基準がPassまたは承認済み判断となる | #2 |
