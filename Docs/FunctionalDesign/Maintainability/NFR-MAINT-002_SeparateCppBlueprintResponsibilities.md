...
|---|---|
| 要件ID | `NFR-MAINT-002` |
| 要件名 | C++とBlueprintの責務を分離する |
| 要件種別 | 非機能要件 |
| 要件カテゴリ | 保守性 |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「C++とBlueprintの責務を分離する」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
| 関連Issue | 未割当 |
| 関連PR | 未割当 |
...
- 元要件「C++とBlueprintの責務を分離する」の判定基準、検証条件、証跡を定義する。
- Blueprint公開APIを必要最小限にし、C++で不変条件と参照検証を保証する。
...
| `Architecture Review` | 計測・レビュー・テスト結果を収集して合否を判定する |
...
- 状態・判定・永続化がC++、Asset参照・演出・WidgetがBlueprint中心か確認する。
...
4. Blueprint公開APIを必要最小限にし、C++で不変条件と参照検証を保証する。
5. 責務レビュー結果を証跡とともに記録する。
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 検証対象 | Build / Source / Asset / Save Fixture | Yes | クラス・Blueprint設計 |
| 実行条件 | Hardware Profile / Test Matrix / Checklist | Yes | 状態・判定・永続化がC++、Asset参照・演出・WidgetがBlueprint中心か確認する |
| 比較基準 | 数値閾値 / 0件条件 / Architecture Rule | Yes | 公開API / Asset Reference |
...
| 証跡 | Report / Log / Trace / Screenshot / Checklist | 責務レビュー結果 |
...
| クラス・システム | 責務 | 実装区分 |
|---|---|---|
| `Architecture Review` | 条件固定、収集、比較、判定 | `Tool / C++ / Test` |
...
FVerificationResult VerifyReviewCppBlueprintBoundary(const FVerificationContext& Context);
bool MeetsReviewCppBlueprintBoundaryRequirement(const FVerificationResult& Result);
```
...
|---|---|---:|---|---|
| 公開API / Asset Reference | Test Config / Number / Rule | Data Asset定義値 | 設計規約 / Data Asset | 合否基準または検証条件 |
...
| 種別 | 内容 |
|---|---|
| 開発UI | 未設定AssetをEditor ValidationとWarningで通知する |
...
- [ ] 主要ルールがBlueprint Graphだけに隠れず、Blueprint未設定時もC++が安全に失敗する。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-NFR-MAINT-002-001` | 基準確認 | すべての前提条件が固定済み | クラス・Blueprint設計で検証する | 主要ルールがBlueprint Graphだけに隠れず、Blueprint未設定時もC++が安全に失敗する |
| `TC-NFR-MAINT-002-002` | 再現性 | 同じBuild・環境・手順 | 別担当者が再実施する | 許容差内で同じ判定となる |
| `TC-NFR-MAINT-002-003` | 条件不備 | 必須Metadataまたは証跡なし | 判定を要求する | PassにせずBlockedまたは測定不能となる |
...
|---|---|---|---|---|
| `ADR-NFR-MAINT-002-001` | 検証方式 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | 担当者の主観だけで合否を決める | 再現性、比較可能性、Release判断の追跡性を保つため |
...
|---:|---|---|---|
| 1 | `NFR-MAINT-002: 検証条件・Fixture整備` | 環境、手順、期待値、証跡形式が確定する | なし |
| 2 | `NFR-MAINT-002: 計測・レビュー・試験` | 基準Buildの結果と証跡が揃う | #1 |
| 3 | `NFR-MAINT-002: 未達修正と再確認` | Must/Should基準がPassまたは承認済み判断となる | #2 |
