...
|---|---|
| 要件ID | `NFR-MAINT-005` |
| 要件名 | Gameplay Tagで状態を一元管理する |
| 要件種別 | 非機能要件 |
| 要件カテゴリ | 保守性 |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「Gameplay Tagで状態を一元管理する」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
| 関連Issue | 未割当 |
| 関連PR | 未割当 |
...
Ability・AI・UI間で同じ状態定義を共有し、boolの不整合を防ぐ。
...
- 元要件「Gameplay Tagで状態を一元管理する」の判定基準、検証条件、証跡を定義する。
- ASCまたは明確な状態OwnerがTagを付与・解除し、各利用側はTag Queryで参照する。
...
| `UActionAbilitySystemComponent / Gameplay Tags` | 計測・レビュー・テスト結果を収集して合否を判定する |
...
- Native/Config Tag登録、命名、排他状態、付与元と解除経路を検証する。
...
4. ASCまたは明確な状態OwnerがTagを付与・解除し、各利用側はTag Queryで参照する。
5. Gameplay Tag Containerを証跡とともに記録する。
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 検証対象 | Build / Source / Asset / Save Fixture | Yes | 状態変更Event |
| 実行条件 | Hardware Profile / Test Matrix / Checklist | Yes | Native/Config Tag登録、命名、排他状態、付与元と解除経路を検証する |
| 比較基準 | 数値閾値 / 0件条件 / Architecture Rule | Yes | Gameplay Tag Dictionary |
...
| 証跡 | Report / Log / Trace / Screenshot / Checklist | Gameplay Tag Container |
...
| クラス・システム | 責務 | 実装区分 |
|---|---|---|
| `UActionAbilitySystemComponent / Gameplay Tags` | 条件固定、収集、比較、判定 | `Tool / C++ / Test` |
...
FVerificationResult VerifyManageGameplayStateTags(const FVerificationContext& Context);
bool MeetsManageGameplayStateTagsRequirement(const FVerificationResult& Result);
```
...
[Test Plan / Reviewer]
          |
          v
[UActionAbilitySystemComponent / Gameplay Tags]
   |             |
   v             v
...
|---|---|---:|---|---|
| Gameplay Tag Dictionary | Test Config / Number / Rule | Data Asset定義値 | 設計規約 / Data Asset | 合否基準または検証条件 |
...
| 種別 | 内容 |
|---|---|
| 開発UI | Gameplay Debuggerで有効Tagを確認可能にする |
...
- [ ] 戦闘の主要状態が定義済みTagで参照でき、中断・死亡・UnPossessed時に一時Tagが残らない。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-NFR-MAINT-005-001` | 基準確認 | すべての前提条件が固定済み | 状態変更Eventで検証する | 戦闘の主要状態が定義済みTagで参照でき、中断・死亡・UnPossessed時に一時Tagが残らない |
| `TC-NFR-MAINT-005-002` | 再現性 | 同じBuild・環境・手順 | 別担当者が再実施する | 許容差内で同じ判定となる |
| `TC-NFR-MAINT-005-003` | 条件不備 | 必須Metadataまたは証跡なし | 判定を要求する | PassにせずBlockedまたは測定不能となる |
...
|---|---|---|---|---|
| `ADR-NFR-MAINT-005-001` | 検証方式 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | 担当者の主観だけで合否を決める | 再現性、比較可能性、Release判断の追跡性を保つため |
...
|---:|---|---|---|
| 1 | `NFR-MAINT-005: 検証条件・Fixture整備` | 環境、手順、期待値、証跡形式が確定する | なし |
| 2 | `NFR-MAINT-005: 計測・レビュー・試験` | 基準Buildの結果と証跡が揃う | #1 |
| 3 | `NFR-MAINT-005: 未達修正と再確認` | Must/Should基準がPassまたは承認済み判断となる | #2 |
