# NFR-MAINT-003 キャラクター移動実装をインターフェースで抽象化する
...
|---|---|
| 要件ID | `NFR-MAINT-003` |
| 要件名 | キャラクター移動実装をインターフェースで抽象化する |
| 要件種別 | 非機能要件 |
| 要件カテゴリ | 保守性 |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「キャラクター移動実装をインターフェースで抽象化する」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
| 関連Issue | 未割当 |
| 関連PR | 未割当 |
...
- 元要件「キャラクター移動実装をインターフェースで抽象化する」の判定基準、検証条件、証跡を定義する。
- Move・Jump・Dodge・Facing等をIMovementDriver経由で要求する。
...
| `IMovementDriver / UMovementAdapterComponent` | 計測・レビュー・テスト結果を収集して合否を判定する |
...
- 利用側が具体的Movement Componentへ直接依存していないか検証する。
...
4. Move・Jump・Dodge・Facing等をIMovementDriver経由で要求する。
5. Driver実装による移動結果を証跡とともに記録する。
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 検証対象 | Build / Source / Asset / Save Fixture | Yes | Movement Request |
| 実行条件 | Hardware Profile / Test Matrix / Checklist | Yes | 利用側が具体的Movement Componentへ直接依存していないか検証する |
| 比較基準 | 数値閾値 / 0件条件 / Architecture Rule | Yes | Movement Driver設定 |
...
| 証跡 | Report / Log / Trace / Screenshot / Checklist | Driver実装による移動結果 |
...
| クラス・システム | 責務 | 実装区分 |
|---|---|---|
| `IMovementDriver / UMovementAdapterComponent` | 条件固定、収集、比較、判定 | `Tool / C++ / Test` |
...
FVerificationResult VerifyUseMovementDriver(const FVerificationContext& Context);
bool MeetsUseMovementDriverRequirement(const FVerificationResult& Result);
```
...
[Test Plan / Reviewer]
          |
          v
[IMovementDriver / UMovementAdapterComponent]
   |             |
   v             v
...
|---|---|---:|---|---|
| Movement Driver設定 | Test Config / Number / Rule | MoverDriver候補 | 設計規約 / Data Asset | 合否基準または検証条件 |
...
- [ ] 戦闘・AbilityがIMovementDriverだけを通じて移動要求を発行する。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-NFR-MAINT-003-001` | 基準確認 | すべての前提条件が固定済み | Movement Requestで検証する | 戦闘・AbilityがIMovementDriverだけを通じて移動要求を発行する |
| `TC-NFR-MAINT-003-002` | 再現性 | 同じBuild・環境・手順 | 別担当者が再実施する | 許容差内で同じ判定となる |
| `TC-NFR-MAINT-003-003` | 条件不備 | 必須Metadataまたは証跡なし | 判定を要求する | PassにせずBlockedまたは測定不能となる |
...
| ID | 内容 | 影響範囲 | 決定期限 | 状態 |
|---|---|---|---|---|
| `Q-NFR-MAINT-003` | Moverの最終採否は2～3週間のプロトタイプ後に決定する | 設計・計測・テスト | Release判定前 | Open |
...
|---|---|---|---|---|
| `ADR-NFR-MAINT-003-001` | 検証方式 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | 担当者の主観だけで合否を決める | 再現性、比較可能性、Release判断の追跡性を保つため |
...
|---:|---|---|---|
| 1 | `NFR-MAINT-003: 検証条件・Fixture整備` | 環境、手順、期待値、証跡形式が確定する | なし |
| 2 | `NFR-MAINT-003: 計測・レビュー・試験` | 基準Buildの結果と証跡が揃う | #1 |
| 3 | `NFR-MAINT-003: 未達修正と再確認` | Must/Should基準がPassまたは承認済み判断となる | #2 |
