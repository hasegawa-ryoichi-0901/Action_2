# NFR-MAINT-004 Moverを不採用にしても戦闘システムへ影響させない
...
|---|---|
| 要件ID | `NFR-MAINT-004` |
| 要件名 | Moverを不採用にしても戦闘システムへ影響させない |
| 要件種別 | 非機能要件 |
| 要件カテゴリ | 保守性 |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「Moverを不採用にしても戦闘システムへ影響させない」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
| 関連Issue | 未割当 |
| 関連PR | 未割当 |
...
- 元要件「Moverを不採用にしても戦闘システムへ影響させない」の判定基準、検証条件、証跡を定義する。
- MoverDriverとCharacterMovementDriverをAdapter配下で差し替える。
...
| `UMovementAdapterComponent` | 計測・レビュー・テスト結果を収集して合否を判定する |
...
- 戦闘モジュールにMover固有型・Include・Tag依存がないか検証する。
...
4. MoverDriverとCharacterMovementDriverをAdapter配下で差し替える。
5. 同一Movement Interface結果を証跡とともに記録する。
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 検証対象 | Build / Source / Asset / Save Fixture | Yes | Driver選択 |
| 実行条件 | Hardware Profile / Test Matrix / Checklist | Yes | 戦闘モジュールにMover固有型・Include・Tag依存がないか検証する |
| 比較基準 | 数値閾値 / 0件条件 / Architecture Rule | Yes | Movement Driver Class |
...
| 証跡 | Report / Log / Trace / Screenshot / Checklist | 同一Movement Interface結果 |
...
| クラス・システム | 責務 | 実装区分 |
|---|---|---|
| `UMovementAdapterComponent` | 条件固定、収集、比較、判定 | `Tool / C++ / Test` |
...
FVerificationResult VerifySwapMovementDriver(const FVerificationContext& Context);
bool MeetsSwapMovementDriverRequirement(const FVerificationResult& Result);
```
...
|---|---|---:|---|---|
| Movement Driver Class | Test Config / Number / Rule | Data Asset定義値 | 設計規約 / Data Asset | 合否基準または検証条件 |
...
- [ ] DriverをCharacterMovementへ変更しても攻撃・回避・ロックオンの呼出側コードを変更しない。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-NFR-MAINT-004-001` | 基準確認 | すべての前提条件が固定済み | Driver選択で検証する | DriverをCharacterMovementへ変更しても攻撃・回避・ロックオンの呼出側コードを変更しない |
| `TC-NFR-MAINT-004-002` | 再現性 | 同じBuild・環境・手順 | 別担当者が再実施する | 許容差内で同じ判定となる |
| `TC-NFR-MAINT-004-003` | 条件不備 | 必須Metadataまたは証跡なし | 判定を要求する | PassにせずBlockedまたは測定不能となる |
...
|---|---|---|---|---|
| `ADR-NFR-MAINT-004-001` | 検証方式 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | 担当者の主観だけで合否を決める | 再現性、比較可能性、Release判断の追跡性を保つため |
...
|---:|---|---|---|
| 1 | `NFR-MAINT-004: 検証条件・Fixture整備` | 環境、手順、期待値、証跡形式が確定する | なし |
| 2 | `NFR-MAINT-004: 計測・レビュー・試験` | 基準Buildの結果と証跡が揃う | #1 |
| 3 | `NFR-MAINT-004: 未達修正と再確認` | Must/Should基準がPassまたは承認済み判断となる | #2 |
