...
|---|---|
| 要件ID | `NFR-MAINT-001` |
| 要件名 | 武器、攻撃、敵、Abilityをデータ駆動で追加できる |
| 要件種別 | 非機能要件 |
| 要件カテゴリ | 保守性 |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「武器、攻撃、敵、Abilityをデータ駆動で追加できる」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
| 関連Issue | 未割当 |
| 関連PR | 未割当 |
| 関連設計書 | [Docs/05_GASDesign.md](../../05_GASDesign.md)<br>[Docs/07_ClassDesign.md](../../07_ClassDesign.md)<br>[Docs/08_DataDesign.md](../../08_DataDesign.md)<br>[Docs/11_TestPlan.md](../../11_TestPlan.md) |

---

## 2. 目的

コンテンツ追加時の既存C++修正と分岐増加を抑える。
...
- 元要件「武器、攻撃、敵、Abilityをデータ駆動で追加できる」の判定基準、検証条件、証跡を定義する。
- Primary Data Asset/Data Tableを登録し、共通実行ロジックがDefinitionを解釈する。
...
| アクター | 役割 |
|---|---|
| `開発者 / Reviewer` | 条件を固定し検証を実施する |
| `Data Asset Registry / Data Validation` | 計測・レビュー・テスト結果を収集して合否を判定する |
| `C++ / Blueprint / Data Asset` | 要件を満たす対象と証跡の生成元 |
...
- Definitionの必須ID・Class・Montage・Tag・参照整合性を検証する。
...
[基準Build・環境を固定]
      |
      v
[Weapon/Attack/Enemy/Ability Definitionを準備]
      |
      v
[Definitionの必須ID・Class・Montage・Tag・参照整合性を検証する]
      |
...
4. Primary Data Asset/Data Tableを登録し、共通実行ロジックがDefinitionを解釈する。
5. 登録済みゲームプレイ要素を証跡とともに記録する。
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 検証対象 | Build / Source / Asset / Save Fixture | Yes | Weapon/Attack/Enemy/Ability Definition |
| 実行条件 | Hardware Profile / Test Matrix / Checklist | Yes | Definitionの必須ID・Class・Montage・Tag・参照整合性を検証する |
| 比較基準 | 数値閾値 / 0件条件 / Architecture Rule | Yes | Primary Data Asset / Data Table |
...
| 証跡 | Report / Log / Trace / Screenshot / Checklist | 登録済みゲームプレイ要素 |
...
| クラス・システム | 責務 | 実装区分 |
|---|---|---|
| `Data Asset Registry / Data Validation` | 条件固定、収集、比較、判定 | `Tool / C++ / Test` |
| `設計規約 / Data Asset` | 閾値、環境、履歴、証跡 | `Config / Document` |
...
FVerificationResult VerifyValidateDataDrivenContent(const FVerificationContext& Context);
bool MeetsValidateDataDrivenContentRequirement(const FVerificationResult& Result);
```
...
[Test Plan / Reviewer]
          |
          v
[Data Asset Registry / Data Validation]
   |             |
   v             v
...
|---|---|---:|---|---|
| Primary Data Asset / Data Table | Test Config / Number / Rule | 定義別 | 設計規約 / Data Asset | 合否基準または検証条件 |
...
| 種別 | 内容 |
|---|---|
| 開発UI | Editor Validation結果をMessage Logへ出力する |
...
- [ ] 同種要素をDefinition追加とAsset設定で登録でき、既存実行クラスの分岐追加を要求しない。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-NFR-MAINT-001-001` | 基準確認 | すべての前提条件が固定済み | Weapon/Attack/Enemy/Ability Definitionで検証する | 同種要素をDefinition追加とAsset設定で登録でき、既存実行クラスの分岐追加を要求しない |
| `TC-NFR-MAINT-001-002` | 再現性 | 同じBuild・環境・手順 | 別担当者が再実施する | 許容差内で同じ判定となる |
| `TC-NFR-MAINT-001-003` | 条件不備 | 必須Metadataまたは証跡なし | 判定を要求する | PassにせずBlockedまたは測定不能となる |
...
|---|---|---|---|---|
| `ADR-NFR-MAINT-001-001` | 検証方式 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | 担当者の主観だけで合否を決める | 再現性、比較可能性、Release判断の追跡性を保つため |
...
|---:|---|---|---|
| 1 | `NFR-MAINT-001: 検証条件・Fixture整備` | 環境、手順、期待値、証跡形式が確定する | なし |
| 2 | `NFR-MAINT-001: 計測・レビュー・試験` | 基準Buildの結果と証跡が揃う | #1 |
| 3 | `NFR-MAINT-001: 未達修正と再確認` | Must/Should基準がPassまたは承認済み判断となる | #2 |
