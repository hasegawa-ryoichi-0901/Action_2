...
|---|---|
| 要件ID | `FR-SAVE-006` |
| 要件名 | 設定変更時に設定データを保存する |
| 要件種別 | 機能要件 |
| 要件カテゴリ | セーブ |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「設定変更時に設定データを保存する」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
...
- 元要件「設定変更時に設定データを保存する」を実現する。
- 進行データと分離可能な設定構造へ確定値を保存する。
...
|---|---|---|---|
| `Event.Settings.Changed` | 要件の状態・受付・通知 | 処理開始または条件成立時 | 完了・中断・条件解除時 |
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 実行入力 | InputAction / Gameplay Event / System Event | Yes | 設定確定Event、設定値 |
| 状態・対象参照 | Gameplay Tag / Attribute / Weak Object Reference | Yes | 変更値の範囲・設定Key・保存先を検証する |
| 設定データ | Data Asset / Config | Yes | Settings Data |

### 10.2 出力

| 出力項目 | 型・形式 | 説明 |
|---|---|---|
| 実行結果 | State / Attribute / Result Event | 設定保存結果 |
| 通知 | Delegate / Gameplay Event | 完了・失敗・中断を必要な連携先へ通知する |
| フィードバック | UI / Animation / VFX / SE | 保存失敗時だけ再試行可能な通知を表示する |
...
| `UActionSaveGame` | Settings DataとAsset参照 | `Data Asset / Config` |
...
bool IsSaveSettingsActive() const;
```

### 12.2 通知イベント

```cpp
DECLARE_MULTICAST_DELEGATE(FOnSaveSettingsStarted);
DECLARE_MULTICAST_DELEGATE(FOnSaveSettingsCompleted);
DECLARE_MULTICAST_DELEGATE(FOnSaveSettingsCancelled);
DECLARE_MULTICAST_DELEGATE_OneParam(FOnSaveSettingsFailed, EFailureReason);
```
...
|---|---|---:|---|---|
| Settings Data | Data Asset / Struct / Config | TBD | UActionSaveGame | 要件固有の判定・調整データ |
...
- [ ] 設定確定後に保存され、再起動後に同じ有効値が適用される。
- [ ] 成功通知が1回発行され、一時状態が完了時に解除される。
- [ ] 保存失敗時だけ再試行可能な通知を表示する。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-FR-SAVE-006-001` | 正常系 | すべての事前条件を満たす | 設定確定Event、設定値を発生させる | 設定確定後に保存され、再起動後に同じ有効値が適用される |
| `TC-FR-SAVE-006-002` | 異常系 | 必須参照または実行条件を1つ無効にする | 同じ要求を発生させる | 状態とリソースを変更せず安全に拒否する |
| `TC-FR-SAVE-006-003` | 多重・中断 | 処理中または連続要求 | 再要求、死亡、Owner破棄のいずれかを発生させる | 重複せず一時状態を解除しCrashしない |
...
| ID | 内容 | 影響範囲 | 決定期限 | 状態 |
|---|---|---|---|---|
| `Q-FR-SAVE-006` | 保存対象設定項目と進行データからの分離方式はTBD | 設計・データ・テスト | 実装Issue開始前 | Open |
...
|---|---|---|---|---|
| `ADR-FR-SAVE-006-001` | 責務配置 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | Blueprintだけにルールと状態を保持する | 状態整合性、検証容易性、データ調整性を保つため |
...
|---:|---|---|---|
| 1 | `FR-SAVE-006: データ・状態・公開Interface` | 必須データ、状態、Validation、APIが定義される | なし |
| 2 | `FR-SAVE-006: 主要ロジック` | 正常・拒否・中断・多重実行テストが通る | #1 |
| 3 | `FR-SAVE-006: Asset・UI・結合確認` | 必須Assetを設定し受入条件を確認できる | #2 |
