...
|---|---|
| 要件ID | `FR-SAVE-005` |
| 要件名 | ステージクリア時にオートセーブする |
| 要件種別 | 機能要件 |
| 要件カテゴリ | セーブ |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「ステージクリア時にオートセーブする」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
...
- 元要件「ステージクリア時にオートセーブする」を実現する。
- Stage Clear Flagと最終進行データを保存して完了通知後にResultへ進む。
...
- Stage Clear判定と報酬・進行更新が完了していることを検証する。
...
4. Stage Clear Flagと最終進行データを保存して完了通知後にResultへ進む。
5. 保存結果とClear状態を反映する。
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 実行入力 | InputAction / Gameplay Event / System Event | Yes | Stage Clear Event |
| 状態・対象参照 | Gameplay Tag / Attribute / Weak Object Reference | Yes | Stage Clear判定と報酬・進行更新が完了していることを検証する |
| 設定データ | Data Asset / Config | Yes | StageProgress |

### 10.2 出力

| 出力項目 | 型・形式 | 説明 |
|---|---|---|
| 実行結果 | State / Attribute / Result Event | 保存結果とClear状態 |
| 通知 | Delegate / Gameplay Event | 完了・失敗・中断を必要な連携先へ通知する |
| フィードバック | UI / Animation / VFX / SE | Clear表示とAutosave Indicatorを順序制御する |
...
| `UActionSaveGame` | StageProgressとAsset参照 | `Data Asset / Config` |
...
bool CanSaveStageClear() const;
void RequestSaveStageClear();
void CancelSaveStageClear();
bool IsSaveStageClearActive() const;
```

### 12.2 通知イベント

```cpp
DECLARE_MULTICAST_DELEGATE(FOnSaveStageClearStarted);
DECLARE_MULTICAST_DELEGATE(FOnSaveStageClearCompleted);
DECLARE_MULTICAST_DELEGATE(FOnSaveStageClearCancelled);
DECLARE_MULTICAST_DELEGATE_OneParam(FOnSaveStageClearFailed, EFailureReason);
```
...
|---|---|---:|---|---|
| StageProgress | Data Asset / Struct / Config | Data Asset定義値 | UActionSaveGame | 要件固有の判定・調整データ |
...
| 種別 | 内容 |
|---|---|
| UI | Clear表示とAutosave Indicatorを順序制御する |
...
- [ ] Stage Clear成立後に1回保存され、再起動後もClear済みと判定される。
- [ ] 成功通知が1回発行され、一時状態が完了時に解除される。
- [ ] Clear表示とAutosave Indicatorを順序制御する。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-FR-SAVE-005-001` | 正常系 | すべての事前条件を満たす | Stage Clear Eventを発生させる | Stage Clear成立後に1回保存され、再起動後もClear済みと判定される |
| `TC-FR-SAVE-005-002` | 異常系 | 必須参照または実行条件を1つ無効にする | 同じ要求を発生させる | 状態とリソースを変更せず安全に拒否する |
| `TC-FR-SAVE-005-003` | 多重・中断 | 処理中または連続要求 | 再要求、死亡、Owner破棄のいずれかを発生させる | 重複せず一時状態を解除しCrashしない |
...
|---|---|---|---|---|
| `ADR-FR-SAVE-005-001` | 責務配置 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | Blueprintだけにルールと状態を保持する | 状態整合性、検証容易性、データ調整性を保つため |
...
|---:|---|---|---|
| 1 | `FR-SAVE-005: データ・状態・公開Interface` | 必須データ、状態、Validation、APIが定義される | なし |
| 2 | `FR-SAVE-005: 主要ロジック` | 正常・拒否・中断・多重実行テストが通る | #1 |
| 3 | `FR-SAVE-005: Asset・UI・結合確認` | 必須Assetを設定し受入条件を確認できる | #2 |
