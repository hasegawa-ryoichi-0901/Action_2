...
| 要件種別 | 機能要件 |
| 要件カテゴリ | セーブ |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「セーブスロットは1つ」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
...
Vertical Sliceの保存導線を単純化し、常に同じ進行データを更新する。
...
- 元要件「セーブスロットは1つ」を実現する。
- すべての進行保存・読込要求を唯一の論理スロットへ集約する。
...
4. すべての進行保存・読込要求を唯一の論理スロットへ集約する。
5. 単一Slot識別子を反映する。
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 実行入力 | InputAction / Gameplay Event / System Event | Yes | Save / Load要求 |
| 状態・対象参照 | Gameplay Tag / Attribute / Weak Object Reference | Yes | 固定SlotNameとUserIndexが有効か検証する |
| 設定データ | Data Asset / Config | Yes | SlotName / UserIndex |

### 10.2 出力

| 出力項目 | 型・形式 | 説明 |
|---|---|---|
| 実行結果 | State / Attribute / Result Event | 単一Slot識別子 |
| 通知 | Delegate / Gameplay Event | 完了・失敗・中断を必要な連携先へ通知する |
| フィードバック | UI / Animation / VFX / SE | 保存中・完了・失敗だけを表示する |
...
| `UActionSaveGame` | SlotName / UserIndexとAsset参照 | `Data Asset / Config` |
...
bool CanResolveSaveSlot() const;
void RequestResolveSaveSlot();
void CancelResolveSaveSlot();
bool IsResolveSaveSlotActive() const;
```

### 12.2 通知イベント

```cpp
DECLARE_MULTICAST_DELEGATE(FOnResolveSaveSlotStarted);
DECLARE_MULTICAST_DELEGATE(FOnResolveSaveSlotCompleted);
DECLARE_MULTICAST_DELEGATE(FOnResolveSaveSlotCancelled);
DECLARE_MULTICAST_DELEGATE_OneParam(FOnResolveSaveSlotFailed, EFailureReason);
```
...
|---|---|---:|---|---|
| SlotName / UserIndex | Data Asset / Struct / Config | ActionSave / 0 | UActionSaveGame | 要件固有の判定・調整データ |
...
- [ ] 進行セーブが常に同じ1スロットを対象とし、複数選択UIを表示しない。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-FR-SAVE-001-001` | 正常系 | すべての事前条件を満たす | Save / Load要求を発生させる | 進行セーブが常に同じ1スロットを対象とし、複数選択UIを表示しない |
| `TC-FR-SAVE-001-002` | 異常系 | 必須参照または実行条件を1つ無効にする | 同じ要求を発生させる | 状態とリソースを変更せず安全に拒否する |
| `TC-FR-SAVE-001-003` | 多重・中断 | 処理中または連続要求 | 再要求、死亡、Owner破棄のいずれかを発生させる | 重複せず一時状態を解除しCrashしない |
...
|---|---|---|---|---|
| `ADR-FR-SAVE-001-001` | 責務配置 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | Blueprintだけにルールと状態を保持する | 状態整合性、検証容易性、データ調整性を保つため |
...
|---:|---|---|---|
| 1 | `FR-SAVE-001: データ・状態・公開Interface` | 必須データ、状態、Validation、APIが定義される | なし |
| 2 | `FR-SAVE-001: 主要ロジック` | 正常・拒否・中断・多重実行テストが通る | #1 |
| 3 | `FR-SAVE-001: Asset・UI・結合確認` | 必須Assetを設定し受入条件を確認できる | #2 |
