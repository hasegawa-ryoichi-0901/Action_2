...
|---|---|
| 要件ID | `FR-SAVE-002` |
| 要件名 | チェックポイント使用時にオートセーブする |
| 要件種別 | 機能要件 |
| 要件カテゴリ | セーブ |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「チェックポイント使用時にオートセーブする」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
...
- 元要件「チェックポイント使用時にオートセーブする」を実現する。
- ActiveCheckpoint・進行・装備・素材を構築して一時書込後に本保存を置換する。
...
- Checkpoint処理完了、SaveGameデータ整合性、保存中でないことを検証する。
...
4. ActiveCheckpoint・進行・装備・素材を構築して一時書込後に本保存を置換する。
5. 保存結果を反映する。
...
|---|---|---|---|
| `Event.Save.Requested` | 要件の状態・受付・通知 | 処理開始または条件成立時 | 完了・中断・条件解除時 |
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 実行入力 | InputAction / Gameplay Event / System Event | Yes | Checkpoint Rest完了Event |
| 状態・対象参照 | Gameplay Tag / Attribute / Weak Object Reference | Yes | Checkpoint処理完了、SaveGameデータ整合性、保存中でないことを検証する |
| 設定データ | Data Asset / Config | Yes | ActiveCheckpointId / PlayerProgress |

### 10.2 出力

| 出力項目 | 型・形式 | 説明 |
|---|---|---|
| 実行結果 | State / Attribute / Result Event | 保存結果 |
| 通知 | Delegate / Gameplay Event | 完了・失敗・中断を必要な連携先へ通知する |
| フィードバック | UI / Animation / VFX / SE | 非阻害のAutosave Indicatorを表示する |
...
| `UActionSaveGame` | ActiveCheckpointId / PlayerProgressとAsset参照 | `Data Asset / Config` |
...
- ActiveCheckpointId / PlayerProgressを保持する。
...
bool CanSaveAtCheckpoint() const;
void RequestSaveAtCheckpoint();
void CancelSaveAtCheckpoint();
bool IsSaveAtCheckpointActive() const;
```

### 12.2 通知イベント

```cpp
DECLARE_MULTICAST_DELEGATE(FOnSaveAtCheckpointStarted);
DECLARE_MULTICAST_DELEGATE(FOnSaveAtCheckpointCompleted);
DECLARE_MULTICAST_DELEGATE(FOnSaveAtCheckpointCancelled);
DECLARE_MULTICAST_DELEGATE_OneParam(FOnSaveAtCheckpointFailed, EFailureReason);
```
...
|---|---|---:|---|---|
| ActiveCheckpointId / PlayerProgress | Data Asset / Struct / Config | Data Asset定義値 | UActionSaveGame | 要件固有の判定・調整データ |
...
| 種別 | 内容 |
|---|---|
| UI | 非阻害のAutosave Indicatorを表示する |
...
- [ ] Checkpoint使用完了後に1回保存され、再起動後に同じActiveCheckpointから再開できる。
- [ ] 成功通知が1回発行され、一時状態が完了時に解除される。
- [ ] 非阻害のAutosave Indicatorを表示する。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-FR-SAVE-002-001` | 正常系 | すべての事前条件を満たす | Checkpoint Rest完了Eventを発生させる | Checkpoint使用完了後に1回保存され、再起動後に同じActiveCheckpointから再開できる |
| `TC-FR-SAVE-002-002` | 異常系 | 必須参照または実行条件を1つ無効にする | 同じ要求を発生させる | 状態とリソースを変更せず安全に拒否する |
| `TC-FR-SAVE-002-003` | 多重・中断 | 処理中または連続要求 | 再要求、死亡、Owner破棄のいずれかを発生させる | 重複せず一時状態を解除しCrashしない |
...
|---|---|---|---|---|
| `ADR-FR-SAVE-002-001` | 責務配置 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | Blueprintだけにルールと状態を保持する | 状態整合性、検証容易性、データ調整性を保つため |
...
|---:|---|---|---|
| 1 | `FR-SAVE-002: データ・状態・公開Interface` | 必須データ、状態、Validation、APIが定義される | なし |
| 2 | `FR-SAVE-002: 主要ロジック` | 正常・拒否・中断・多重実行テストが通る | #1 |
| 3 | `FR-SAVE-002: Asset・UI・結合確認` | 必須Assetを設定し受入条件を確認できる | #2 |
