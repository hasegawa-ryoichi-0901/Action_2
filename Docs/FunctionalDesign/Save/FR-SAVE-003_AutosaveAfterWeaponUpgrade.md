...
| 要件種別 | 機能要件 |
| 要件カテゴリ | セーブ |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「武器強化時にオートセーブする」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
...
- 元要件「武器強化時にオートセーブする」を実現する。
- WeaponUpgradeStates・UpgradeMaterial・GrantedAbilities再構築情報を保存する。
...
- 強化Transaction成功後でデータ整合性が取れているか検証する。
...
4. WeaponUpgradeStates・UpgradeMaterial・GrantedAbilities再構築情報を保存する。
5. 保存結果を反映する。
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 実行入力 | InputAction / Gameplay Event / System Event | Yes | Weapon Upgrade成功Event |
| 状態・対象参照 | Gameplay Tag / Attribute / Weak Object Reference | Yes | 強化Transaction成功後でデータ整合性が取れているか検証する |
| 設定データ | Data Asset / Config | Yes | WeaponUpgradeStates / UpgradeMaterial |
...
| フィードバック | UI / Animation / VFX / SE | 強化結果とAutosave Indicatorを表示する |
...
| `UActionSaveGame` | WeaponUpgradeStates / UpgradeMaterialとAsset参照 | `Data Asset / Config` |
...
- WeaponUpgradeStates / UpgradeMaterialを保持する。
...
bool CanSaveWeaponUpgrade() const;
void RequestSaveWeaponUpgrade();
void CancelSaveWeaponUpgrade();
bool IsSaveWeaponUpgradeActive() const;
```

### 12.2 通知イベント

```cpp
DECLARE_MULTICAST_DELEGATE(FOnSaveWeaponUpgradeStarted);
DECLARE_MULTICAST_DELEGATE(FOnSaveWeaponUpgradeCompleted);
DECLARE_MULTICAST_DELEGATE(FOnSaveWeaponUpgradeCancelled);
DECLARE_MULTICAST_DELEGATE_OneParam(FOnSaveWeaponUpgradeFailed, EFailureReason);
```
...
|---|---|---:|---|---|
| WeaponUpgradeStates / UpgradeMaterial | Data Asset / Struct / Config | Data Asset定義値 | UActionSaveGame | 要件固有の判定・調整データ |
...
| 種別 | 内容 |
|---|---|
| UI | 強化結果とAutosave Indicatorを表示する |
...
- [ ] 強化成功ごとに1回保存され、再起動後もLevelと素材残量が一致する。
- [ ] 成功通知が1回発行され、一時状態が完了時に解除される。
- [ ] 強化結果とAutosave Indicatorを表示する。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-FR-SAVE-003-001` | 正常系 | すべての事前条件を満たす | Weapon Upgrade成功Eventを発生させる | 強化成功ごとに1回保存され、再起動後もLevelと素材残量が一致する |
| `TC-FR-SAVE-003-002` | 異常系 | 必須参照または実行条件を1つ無効にする | 同じ要求を発生させる | 状態とリソースを変更せず安全に拒否する |
| `TC-FR-SAVE-003-003` | 多重・中断 | 処理中または連続要求 | 再要求、死亡、Owner破棄のいずれかを発生させる | 重複せず一時状態を解除しCrashしない |
...
|---|---|---|---|---|
| `ADR-FR-SAVE-003-001` | 責務配置 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | Blueprintだけにルールと状態を保持する | 状態整合性、検証容易性、データ調整性を保つため |
...
|---:|---|---|---|
| 1 | `FR-SAVE-003: データ・状態・公開Interface` | 必須データ、状態、Validation、APIが定義される | なし |
| 2 | `FR-SAVE-003: 主要ロジック` | 正常・拒否・中断・多重実行テストが通る | #1 |
| 3 | `FR-SAVE-003: Asset・UI・結合確認` | 必須Assetを設定し受入条件を確認できる | #2 |
