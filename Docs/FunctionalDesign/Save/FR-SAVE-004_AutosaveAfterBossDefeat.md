...
| 要件種別 | 機能要件 |
| 要件カテゴリ | セーブ |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「ボス撃破時にオートセーブする」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
...
Boss撃破済み進行を失わず、再ロード時の重複撃破状態を防ぐ。
...
- 元要件「ボス撃破時にオートセーブする」を実現する。
- Boss Defeated Flagと関連進行・報酬を一つの保存要求へまとめる。
...
4. Boss Defeated Flagと関連進行・報酬を一つの保存要求へまとめる。
5. 保存結果を反映する。
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 実行入力 | InputAction / Gameplay Event / System Event | Yes | Boss Defeated Event |
| 状態・対象参照 | Gameplay Tag / Attribute / Weak Object Reference | Yes | Boss死亡・報酬・進行更新が完了していることを検証する |
| 設定データ | Data Asset / Config | Yes | BossProgress |
...
| フィードバック | UI / Animation / VFX / SE | 撃破演出を妨げないAutosave Indicatorを表示する |
...
| `UActionSaveGame` | BossProgressとAsset参照 | `Data Asset / Config` |
...
bool CanSaveBossDefeat() const;
void RequestSaveBossDefeat();
void CancelSaveBossDefeat();
bool IsSaveBossDefeatActive() const;
```

### 12.2 通知イベント

```cpp
DECLARE_MULTICAST_DELEGATE(FOnSaveBossDefeatStarted);
DECLARE_MULTICAST_DELEGATE(FOnSaveBossDefeatCompleted);
DECLARE_MULTICAST_DELEGATE(FOnSaveBossDefeatCancelled);
DECLARE_MULTICAST_DELEGATE_OneParam(FOnSaveBossDefeatFailed, EFailureReason);
```
...
|---|---|---:|---|---|
| BossProgress | Data Asset / Struct / Config | Data Asset定義値 | UActionSaveGame | 要件固有の判定・調整データ |
...
| 種別 | 内容 |
|---|---|
| UI | 撃破演出を妨げないAutosave Indicatorを表示する |
...
- [ ] Boss撃破後に1回保存され、再起動後に撃破済み状態が復元される。
- [ ] 成功通知が1回発行され、一時状態が完了時に解除される。
- [ ] 撃破演出を妨げないAutosave Indicatorを表示する。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-FR-SAVE-004-001` | 正常系 | すべての事前条件を満たす | Boss Defeated Eventを発生させる | Boss撃破後に1回保存され、再起動後に撃破済み状態が復元される |
| `TC-FR-SAVE-004-002` | 異常系 | 必須参照または実行条件を1つ無効にする | 同じ要求を発生させる | 状態とリソースを変更せず安全に拒否する |
| `TC-FR-SAVE-004-003` | 多重・中断 | 処理中または連続要求 | 再要求、死亡、Owner破棄のいずれかを発生させる | 重複せず一時状態を解除しCrashしない |
...
|---|---|---|---|---|
| `ADR-FR-SAVE-004-001` | 責務配置 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | Blueprintだけにルールと状態を保持する | 状態整合性、検証容易性、データ調整性を保つため |
...
|---:|---|---|---|
| 1 | `FR-SAVE-004: データ・状態・公開Interface` | 必須データ、状態、Validation、APIが定義される | なし |
| 2 | `FR-SAVE-004: 主要ロジック` | 正常・拒否・中断・多重実行テストが通る | #1 |
| 3 | `FR-SAVE-004: Asset・UI・結合確認` | 必須Assetを設定し受入条件を確認できる | #2 |
