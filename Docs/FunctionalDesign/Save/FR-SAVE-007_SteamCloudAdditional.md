# FR-SAVE-007 Steam CloudはSteam公開版の追加要件とする
...
|---|---|
| 要件ID | `FR-SAVE-007` |
| 要件名 | Steam CloudはSteam公開版の追加要件とする |
| 要件種別 | 機能要件 |
| 要件カテゴリ | セーブ |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「Steam CloudはSteam公開版の追加要件とする」 |
| 優先度 | `Could` |
| 設計状態 | `Draft` |
| 対応範囲 | 追加要件（現時点のVertical Slice対象外） |
| 関連Issue | 未割当 |
| 関連PR | 未割当 |
| 関連設計書 | [Docs/07_ClassDesign.md](../../07_ClassDesign.md)<br>[Docs/08_DataDesign.md](../../08_DataDesign.md)<br>[Docs/09_SaveCheckpointDeath.md](../../09_SaveCheckpointDeath.md)<br>[Docs/11_TestPlan.md](../../11_TestPlan.md) |

---

## 2. 目的

Steam公開時にローカルセーブを複数端末間で同期可能にする。
...
- 元要件「Steam CloudはSteam公開版の追加要件とする」を実現する。
- 採用方式決定後にローカル保存を正本としてUpload・Download・競合解決を追加する。
- 成功・拒否・中断時の状態整合性と通知までを扱う。

### 3.2 対象外

- Vertical Sliceと非Steam版。
...
| アクター | 役割 |
|---|---|
| `Gameplay System / Player` | 入力または処理開始条件を発行する |
| `USaveGameSubsystem` | 実行可否判定、状態変更、主要処理を担当する |
| `UActionSaveGame / ローカル保存領域` | 処理結果の反映または影響を受ける |
...
- ローカルセーブ安定、Steam初期化、競合方針決定済みであることを検証する。
...
4. 採用方式決定後にローカル保存を正本としてUpload・Download・競合解決を追加する。
5. Cloud同期結果を反映する。
...
|---|---|---|---|
| 該当なし | この要件固有のGameplay Tagは追加しない | - | - |
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 実行入力 | InputAction / Gameplay Event / System Event | Yes | Steam同期Event、ローカル保存 |
| 状態・対象参照 | Gameplay Tag / Attribute / Weak Object Reference | Yes | ローカルセーブ安定、Steam初期化、競合方針決定済みであることを検証する |
| 設定データ | Data Asset / Config | Yes | Steam Cloud設定 / SaveVersion |

### 10.2 出力

| 出力項目 | 型・形式 | 説明 |
|---|---|---|
| 実行結果 | State / Attribute / Result Event | Cloud同期結果 |
| 通知 | Delegate / Gameplay Event | 完了・失敗・中断を必要な連携先へ通知する |
| フィードバック | UI / Animation / VFX / SE | 同期中・競合・失敗を明示する |
...
| クラス・システム | 責務 | 実装区分 |
|---|---|---|
| `USaveGameSubsystem` | 実行可否、状態遷移、主要ロジック、重複防止 | `C++` |
| `UActionSaveGame` | Steam Cloud設定 / SaveVersionとAsset参照 | `Data Asset / Config` |
...
- Steam Cloud設定 / SaveVersionを保持する。
...
bool CanSyncSteamCloud() const;
void RequestSyncSteamCloud();
void CancelSyncSteamCloud();
bool IsSyncSteamCloudActive() const;
```

### 12.2 通知イベント

```cpp
DECLARE_MULTICAST_DELEGATE(FOnSyncSteamCloudStarted);
DECLARE_MULTICAST_DELEGATE(FOnSyncSteamCloudCompleted);
DECLARE_MULTICAST_DELEGATE(FOnSyncSteamCloudCancelled);
DECLARE_MULTICAST_DELEGATE_OneParam(FOnSyncSteamCloudFailed, EFailureReason);
```

### 12.3 呼び出し方向

```text
[Gameplay System / Player]
        |
        v
[USaveGameSubsystem]
   |          |
   v          v
[UActionSaveGame]  [UActionSaveGame / ローカル保存領域]
        |
        v
...
|---|---|---:|---|---|
| Steam Cloud設定 / SaveVersion | Data Asset / Struct / Config | TBD | UActionSaveGame | 要件固有の判定・調整データ |
...
- [ ] Steam公開版で再起動・別端末取得・競合時に決定済み方針どおりデータを保全する。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-FR-SAVE-007-001` | 正常系 | すべての事前条件を満たす | Steam同期Event、ローカル保存を発生させる | Steam公開版で再起動・別端末取得・競合時に決定済み方針どおりデータを保全する |
| `TC-FR-SAVE-007-002` | 異常系 | 必須参照または実行条件を1つ無効にする | 同じ要求を発生させる | 状態とリソースを変更せず安全に拒否する |
| `TC-FR-SAVE-007-003` | 多重・中断 | 処理中または連続要求 | 再要求、死亡、Owner破棄のいずれかを発生させる | 重複せず一時状態を解除しCrashしない |
...
| ID | 内容 | 影響範囲 | 決定期限 | 状態 |
|---|---|---|---|---|
| `Q-FR-SAVE-007` | Auto-CloudかRemote Storageか、競合解決方式はSteam公開前に決定する | 設計・データ・テスト | 実装Issue開始前 | Open |
...
|---|---|---|---|---|
| `ADR-FR-SAVE-007-001` | 責務配置 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | Blueprintだけにルールと状態を保持する | 状態整合性、検証容易性、データ調整性を保つため |
...
|---:|---|---|---|
| 1 | `FR-SAVE-007: データ・状態・公開Interface` | 必須データ、状態、Validation、APIが定義される | なし |
| 2 | `FR-SAVE-007: 主要ロジック` | 正常・拒否・中断・多重実行テストが通る | #1 |
| 3 | `FR-SAVE-007: Asset・UI・結合確認` | 必須Assetを設定し受入条件を確認できる | #2 |
