...
|---|---|
| 要件ID | `NFR-PERF-004` |
| 要件名 | RTX 3060搭載開発環境で継続的に計測する |
| 要件種別 | 非機能要件 |
| 要件カテゴリ | パフォーマンス |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「RTX 3060搭載開発環境で継続的に計測する」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
| 関連Issue | 未割当 |
| 関連PR | 未割当 |
...
- 元要件「RTX 3060搭載開発環境で継続的に計測する」の判定基準、検証条件、証跡を定義する。
- 主要マイルストーンごとに基準端末でテスト地点を再計測し履歴化する。
...
- GPU・CPU・Memory・Driver・Build・品質設定を記録する。
...
4. 主要マイルストーンごとに基準端末でテスト地点を再計測し履歴化する。
5. 時系列性能記録を証跡とともに記録する。
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 検証対象 | Build / Source / Asset / Save Fixture | Yes | 基準端末とビルド |
| 実行条件 | Hardware Profile / Test Matrix / Checklist | Yes | GPU・CPU・Memory・Driver・Build・品質設定を記録する |
| 比較基準 | 数値閾値 / 0件条件 / Architecture Rule | Yes | Hardware Profile |
...
| 証跡 | Report / Log / Trace / Screenshot / Checklist | 時系列性能記録 |
...
FVerificationResult VerifyMeasureReferenceMachine(const FVerificationContext& Context);
bool MeetsMeasureReferenceMachineRequirement(const FVerificationResult& Result);
```
...
|---|---|---:|---|---|
| Hardware Profile | Test Config / Number / Rule | RTX 3060 / CPU等TBD | 計測プロファイル / テスト記録 | 合否基準または検証条件 |
...
- [ ] 各性能レポートにRTX 3060を含む環境情報とビルド識別子が残る。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-NFR-PERF-004-001` | 基準確認 | すべての前提条件が固定済み | 基準端末とビルドで検証する | 各性能レポートにRTX 3060を含む環境情報とビルド識別子が残る |
| `TC-NFR-PERF-004-002` | 再現性 | 同じBuild・環境・手順 | 別担当者が再実施する | 許容差内で同じ判定となる |
| `TC-NFR-PERF-004-003` | 条件不備 | 必須Metadataまたは証跡なし | 判定を要求する | PassにせずBlockedまたは測定不能となる |
...
| ID | 内容 | 影響範囲 | 決定期限 | 状態 |
|---|---|---|---|---|
| `Q-NFR-PERF-004` | 基準CPU・Memory・Driver・品質PresetはTBD | 設計・計測・テスト | Release判定前 | Open |
...
|---|---|---|---|---|
| `ADR-NFR-PERF-004-001` | 検証方式 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | 担当者の主観だけで合否を決める | 再現性、比較可能性、Release判断の追跡性を保つため |
...
|---:|---|---|---|
| 1 | `NFR-PERF-004: 検証条件・Fixture整備` | 環境、手順、期待値、証跡形式が確定する | なし |
| 2 | `NFR-PERF-004: 計測・レビュー・試験` | 基準Buildの結果と証跡が揃う | #1 |
| 3 | `NFR-PERF-004: 未達修正と再確認` | Must/Should基準がPassまたは承認済み判断となる | #2 |
