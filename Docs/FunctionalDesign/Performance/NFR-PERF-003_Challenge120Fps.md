...
| 要件種別 | 非機能要件 |
| 要件カテゴリ | パフォーマンス |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「120fpsを挑戦目標とする」 |
| 優先度 | `Could` |
| 設計状態 | `Draft` |
| 関連Issue | 未割当 |
| 関連PR | 未割当 |
...
- 元要件「120fpsを挑戦目標とする」の判定基準、検証条件、証跡を定義する。
- 8.33ms Frame Budgetとの差分を各地点で記録し、未達を必須不具合と分離する。
...
4. 8.33ms Frame Budgetとの差分を各地点で記録し、未達を必須不具合と分離する。
5. 120fps達成状況を証跡とともに記録する。
...
| 実行条件 | Hardware Profile / Test Matrix / Checklist | Yes | 60fps必須目標を損なわず同一計測条件を使用する |
| 比較基準 | 数値閾値 / 0件条件 / Architecture Rule | Yes | Challenge Frame Budget |
...
| 証跡 | Report / Log / Trace / Screenshot / Checklist | 120fps達成状況 |
...
FVerificationResult VerifyVerify120Fps(const FVerificationContext& Context);
bool MeetsVerify120FpsRequirement(const FVerificationResult& Result);
```
...
|---|---|---:|---|---|
| Challenge Frame Budget | Test Config / Number / Rule | 8.33ms | 計測プロファイル / テスト記録 | 合否基準または検証条件 |
...
- [ ] 120fpsの達成地点・未達地点・主ボトルネックが記録される。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-NFR-PERF-003-001` | 基準確認 | すべての前提条件が固定済み | 各性能テスト地点で検証する | 120fpsの達成地点・未達地点・主ボトルネックが記録される |
| `TC-NFR-PERF-003-002` | 再現性 | 同じBuild・環境・手順 | 別担当者が再実施する | 許容差内で同じ判定となる |
| `TC-NFR-PERF-003-003` | 条件不備 | 必須Metadataまたは証跡なし | 判定を要求する | PassにせずBlockedまたは測定不能となる |
...
|---|---|---|---|---|
| `ADR-NFR-PERF-003-001` | 検証方式 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | 担当者の主観だけで合否を決める | 再現性、比較可能性、Release判断の追跡性を保つため |
...
|---:|---|---|---|
| 1 | `NFR-PERF-003: 検証条件・Fixture整備` | 環境、手順、期待値、証跡形式が確定する | なし |
| 2 | `NFR-PERF-003: 計測・レビュー・試験` | 基準Buildの結果と証跡が揃う | #1 |
| 3 | `NFR-PERF-003: 未達修正と再確認` | Must/Should基準がPassまたは承認済み判断となる | #2 |
