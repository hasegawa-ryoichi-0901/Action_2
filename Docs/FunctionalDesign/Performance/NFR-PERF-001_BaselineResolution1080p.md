...
|---|---|
| 要件ID | `NFR-PERF-001` |
| 要件名 | 1920x1080を基準解像度候補とする |
| 要件種別 | 非機能要件 |
| 要件カテゴリ | パフォーマンス |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「1920x1080を基準解像度候補とする」 |
| 優先度 | `Should` |
| 設計状態 | `Draft` |
| 関連Issue | 未割当 |
| 関連PR | 未割当 |
...
- 元要件「1920x1080を基準解像度候補とする」の判定基準、検証条件、証跡を定義する。
- 1920x1080を基準設定として各計測地点を実行し結果へ環境情報を添付する。
...
- 実行解像度・Screen Percentage・品質Presetを記録する。
...
4. 1920x1080を基準設定として各計測地点を実行し結果へ環境情報を添付する。
5. 計測記録を証跡とともに記録する。
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 検証対象 | Build / Source / Asset / Save Fixture | Yes | 解像度・品質設定・テスト地点 |
| 実行条件 | Hardware Profile / Test Matrix / Checklist | Yes | 実行解像度・Screen Percentage・品質Presetを記録する |
| 比較基準 | 数値閾値 / 0件条件 / Architecture Rule | Yes | Resolution / Screen Percentage |
...
| 証跡 | Report / Log / Trace / Screenshot / Checklist | 計測記録 |
...
FVerificationResult VerifyVerify1080p(const FVerificationContext& Context);
bool MeetsVerify1080pRequirement(const FVerificationResult& Result);
```
...
|---|---|---:|---|---|
| Resolution / Screen Percentage | Test Config / Number / Rule | 1920x1080 / TBD | 計測プロファイル / テスト記録 | 合否基準または検証条件 |
...
- [ ] 性能記録に1920x1080と品質条件が明記され、同条件で再計測できる。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-NFR-PERF-001-001` | 基準確認 | すべての前提条件が固定済み | 解像度・品質設定・テスト地点で検証する | 性能記録に1920x1080と品質条件が明記され、同条件で再計測できる |
| `TC-NFR-PERF-001-002` | 再現性 | 同じBuild・環境・手順 | 別担当者が再実施する | 許容差内で同じ判定となる |
| `TC-NFR-PERF-001-003` | 条件不備 | 必須Metadataまたは証跡なし | 判定を要求する | PassにせずBlockedまたは測定不能となる |
...
| ID | 内容 | 影響範囲 | 決定期限 | 状態 |
|---|---|---|---|---|
| `Q-NFR-PERF-001` | 最低動作環境と最終品質PresetはVertical Slice計測後に決定する | 設計・計測・テスト | Release判定前 | Open |
...
|---|---|---|---|---|
| `ADR-NFR-PERF-001-001` | 検証方式 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | 担当者の主観だけで合否を決める | 再現性、比較可能性、Release判断の追跡性を保つため |
...
|---:|---|---|---|
| 1 | `NFR-PERF-001: 検証条件・Fixture整備` | 環境、手順、期待値、証跡形式が確定する | なし |
| 2 | `NFR-PERF-001: 計測・レビュー・試験` | 基準Buildの結果と証跡が揃う | #1 |
| 3 | `NFR-PERF-001: 未達修正と再確認` | Must/Should基準がPassまたは承認済み判断となる | #2 |
