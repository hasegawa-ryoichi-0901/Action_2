...
|---|---|
| 要件ID | `NFR-PERF-005` |
| 要件名 | 平均fpsだけでなくフレーム時間を確認する |
| 要件種別 | 非機能要件 |
| 要件カテゴリ | パフォーマンス |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「平均fpsだけでなくフレーム時間を確認する」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
| 関連Issue | 未割当 |
| 関連PR | 未割当 |
...
- 元要件「平均fpsだけでなくフレーム時間を確認する」の判定基準、検証条件、証跡を定義する。
- Frame Time時系列、1% Low相当、最大Spikeと発生箇所を記録する。
...
4. Frame Time時系列、1% Low相当、最大Spikeと発生箇所を記録する。
5. Frame Time統計とSpike原因を証跡とともに記録する。
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 検証対象 | Build / Source / Asset / Save Fixture | Yes | Profiler Capture |
| 実行条件 | Hardware Profile / Test Matrix / Checklist | Yes | 計測区間・Warmup・Capture長・統計方法を揃える |
| 比較基準 | 数値閾値 / 0件条件 / Architecture Rule | Yes | Capture Duration / Percentile |
...
| 証跡 | Report / Log / Trace / Screenshot / Checklist | Frame Time統計とSpike原因 |
...
FVerificationResult VerifyInspectFrameTime(const FVerificationContext& Context);
bool MeetsInspectFrameTimeRequirement(const FVerificationResult& Result);
```
...
|---|---|---:|---|---|
| Capture Duration / Percentile | Test Config / Number / Rule | TBD | 計測プロファイル / テスト記録 | 合否基準または検証条件 |
...
| 種別 | 内容 |
|---|---|
| 開発UI | グラフとSpike Bookmarkを性能レポートへ添付する |
...
- [ ] 各必須計測で平均fpsに加えてFrame Timeと落ち込み指標が記録される。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-NFR-PERF-005-001` | 基準確認 | すべての前提条件が固定済み | Profiler Captureで検証する | 各必須計測で平均fpsに加えてFrame Timeと落ち込み指標が記録される |
| `TC-NFR-PERF-005-002` | 再現性 | 同じBuild・環境・手順 | 別担当者が再実施する | 許容差内で同じ判定となる |
| `TC-NFR-PERF-005-003` | 条件不備 | 必須Metadataまたは証跡なし | 判定を要求する | PassにせずBlockedまたは測定不能となる |
...
| ID | 内容 | 影響範囲 | 決定期限 | 状態 |
|---|---|---|---|---|
| `Q-NFR-PERF-005` | Capture時間と1% Low相当の算出方法はTBD | 設計・計測・テスト | Release判定前 | Open |
...
|---|---|---|---|---|
| `ADR-NFR-PERF-005-001` | 検証方式 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | 担当者の主観だけで合否を決める | 再現性、比較可能性、Release判断の追跡性を保つため |
...
|---:|---|---|---|
| 1 | `NFR-PERF-005: 検証条件・Fixture整備` | 環境、手順、期待値、証跡形式が確定する | なし |
| 2 | `NFR-PERF-005: 計測・レビュー・試験` | 基準Buildの結果と証跡が揃う | #1 |
| 3 | `NFR-PERF-005: 未達修正と再確認` | Must/Should基準がPassまたは承認済み判断となる | #2 |
