...
| 要件種別 | 非機能要件 |
| 要件カテゴリ | パフォーマンス |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「60fpsを最低保証目標とする」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
| 関連Issue | 未割当 |
| 関連PR | 未割当 |
...
戦闘入力とAnimationの応答性を保つ最低性能基準を定める。
...
- 元要件「60fpsを最低保証目標とする」の判定基準、検証条件、証跡を定義する。
- 各地点のFrame Timeと落ち込みを計測し16.67ms基準に対する結果を記録する。
...
- 基準環境・解像度・品質・計測地点・Shipping条件を固定する。
...
4. 各地点のFrame Timeと落ち込みを計測し16.67ms基準に対する結果を記録する。
5. fps・Frame Time・合否を証跡とともに記録する。
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 検証対象 | Build / Source / Asset / Save Fixture | Yes | 各性能テスト地点 |
| 実行条件 | Hardware Profile / Test Matrix / Checklist | Yes | 基準環境・解像度・品質・計測地点・Shipping条件を固定する |
| 比較基準 | 数値閾値 / 0件条件 / Architecture Rule | Yes | Frame Budget |
...
| 証跡 | Report / Log / Trace / Screenshot / Checklist | fps・Frame Time・合否 |
...
FVerificationResult VerifyVerify60Fps(const FVerificationContext& Context);
bool MeetsVerify60FpsRequirement(const FVerificationResult& Result);
```
...
|---|---|---:|---|---|
| Frame Budget | Test Config / Number / Rule | 16.67ms | 計測プロファイル / テスト記録 | 合否基準または検証条件 |
...
| 種別 | 内容 |
|---|---|
| 開発UI | Unreal Insightsと性能レポートへ結果を記録する |
...
- [ ] 基準条件の各必須地点で60fps目標を満たし、重大な継続カクつきがない。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-NFR-PERF-002-001` | 基準確認 | すべての前提条件が固定済み | 各性能テスト地点で検証する | 基準条件の各必須地点で60fps目標を満たし、重大な継続カクつきがない |
| `TC-NFR-PERF-002-002` | 再現性 | 同じBuild・環境・手順 | 別担当者が再実施する | 許容差内で同じ判定となる |
| `TC-NFR-PERF-002-003` | 条件不備 | 必須Metadataまたは証跡なし | 判定を要求する | PassにせずBlockedまたは測定不能となる |
...
|---|---|---|---|---|
| `ADR-NFR-PERF-002-001` | 検証方式 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | 担当者の主観だけで合否を決める | 再現性、比較可能性、Release判断の追跡性を保つため |
...
|---:|---|---|---|
| 1 | `NFR-PERF-002: 検証条件・Fixture整備` | 環境、手順、期待値、証跡形式が確定する | なし |
| 2 | `NFR-PERF-002: 計測・レビュー・試験` | 基準Buildの結果と証跡が揃う | #1 |
| 3 | `NFR-PERF-002: 未達修正と再確認` | Must/Should基準がPassまたは承認済み判断となる | #2 |
