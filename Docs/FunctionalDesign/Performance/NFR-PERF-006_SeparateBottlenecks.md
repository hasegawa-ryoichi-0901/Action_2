...
|---|---|
| 要件ID | `NFR-PERF-006` |
| 要件名 | Game Thread、Render Thread、GPUのボトルネックを分離する |
| 要件種別 | 非機能要件 |
| 要件カテゴリ | パフォーマンス |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「Game Thread、Render Thread、GPUのボトルネックを分離する」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
| 関連Issue | 未割当 |
| 関連PR | 未割当 |
| 関連設計書 | [Docs/10_PerformanceRequirements.md](../../10_PerformanceRequirements.md)<br>[Docs/11_TestPlan.md](../../11_TestPlan.md) |

---

## 2. 目的

最適化対象を誤らず、原因Thread・GPU Passへ対策を集中する。
...
- 元要件「Game Thread、Render Thread、GPUのボトルネックを分離する」の判定基準、検証条件、証跡を定義する。
- Game・Draw/Render・GPU時間を比較し、支配要因と詳細Captureを関連付ける。
...
| アクター | 役割 |
|---|---|
| `開発者 / QA` | 条件を固定し検証を実施する |
| `Performance Verification` | 計測・レビュー・テスト結果を収集して合否を判定する |
| `ゲーム実行環境 / ビルド` | 要件を満たす対象と証跡の生成元 |
...
- stat unitとUnreal Insights/GPU Captureを同じ再現条件で取得する。
...
4. Game・Draw/Render・GPU時間を比較し、支配要因と詳細Captureを関連付ける。
5. ボトルネック分類と根拠を証跡とともに記録する。
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 検証対象 | Build / Source / Asset / Save Fixture | Yes | Thread/GPU Timing |
| 実行条件 | Hardware Profile / Test Matrix / Checklist | Yes | stat unitとUnreal Insights/GPU Captureを同じ再現条件で取得する |
| 比較基準 | 数値閾値 / 0件条件 / Architecture Rule | Yes | Thread/GPU Timing |
...
| 証跡 | Report / Log / Trace / Screenshot / Checklist | ボトルネック分類と根拠 |
...
| クラス・システム | 責務 | 実装区分 |
|---|---|---|
| `Performance Verification` | 条件固定、収集、比較、判定 | `Tool / C++ / Test` |
| `計測プロファイル / テスト記録` | 閾値、環境、履歴、証跡 | `Config / Document` |
...
FVerificationResult VerifyClassifyBottleneck(const FVerificationContext& Context);
bool MeetsClassifyBottleneckRequirement(const FVerificationResult& Result);
```
...
|---|---|---:|---|---|
| Thread/GPU Timing | Test Config / Number / Rule | 計測値 | 計測プロファイル / テスト記録 | 合否基準または検証条件 |
...
| 種別 | 内容 |
|---|---|
| 開発UI | 性能レポートにTimingとCaptureリンクを記載する |
...
- [ ] 性能未達ごとにGame Thread・Render Thread・GPUまたは複合の分類と証跡が残る。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-NFR-PERF-006-001` | 基準確認 | すべての前提条件が固定済み | Thread/GPU Timingで検証する | 性能未達ごとにGame Thread・Render Thread・GPUまたは複合の分類と証跡が残る |
| `TC-NFR-PERF-006-002` | 再現性 | 同じBuild・環境・手順 | 別担当者が再実施する | 許容差内で同じ判定となる |
| `TC-NFR-PERF-006-003` | 条件不備 | 必須Metadataまたは証跡なし | 判定を要求する | PassにせずBlockedまたは測定不能となる |
...
|---|---|---|---|---|
| `ADR-NFR-PERF-006-001` | 検証方式 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | 担当者の主観だけで合否を決める | 再現性、比較可能性、Release判断の追跡性を保つため |
...
|---:|---|---|---|
| 1 | `NFR-PERF-006: 検証条件・Fixture整備` | 環境、手順、期待値、証跡形式が確定する | なし |
| 2 | `NFR-PERF-006: 計測・レビュー・試験` | 基準Buildの結果と証跡が揃う | #1 |
| 3 | `NFR-PERF-006: 未達修正と再確認` | Must/Should基準がPassまたは承認済み判断となる | #2 |
