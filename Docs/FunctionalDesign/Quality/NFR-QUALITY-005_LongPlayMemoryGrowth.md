...
| 要件名 | 長時間プレイでメモリ増加を確認する |
| 要件種別 | 非機能要件 |
| 要件カテゴリ | 品質 |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「長時間プレイでメモリ増加を確認する」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
| 関連Issue | 未割当 |
| 関連PR | 未割当 |
...
反復戦闘・死亡・Level遷移による継続的なLeakや参照残留を検出する。
...
- 元要件「長時間プレイでメモリ増加を確認する」の判定基準、検証条件、証跡を定義する。
- Memory Insightsで定期Snapshotを取得し、安定化後の増加傾向とObject種別を比較する。
...
- 試験時間、反復シナリオ、GC地点、開始時Memoryを記録する。
...
4. Memory Insightsで定期Snapshotを取得し、安定化後の増加傾向とObject種別を比較する。
5. Memory推移と残留Object分析を証跡とともに記録する。
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 検証対象 | Build / Source / Asset / Save Fixture | Yes | 長時間反復シナリオ |
| 実行条件 | Hardware Profile / Test Matrix / Checklist | Yes | 試験時間、反復シナリオ、GC地点、開始時Memoryを記録する |
| 比較基準 | 数値閾値 / 0件条件 / Architecture Rule | Yes | Test Duration / Memory Threshold |
...
| 証跡 | Report / Log / Trace / Screenshot / Checklist | Memory推移と残留Object分析 |
...
FVerificationResult VerifyVerifyLongPlayMemory(const FVerificationContext& Context);
bool MeetsVerifyLongPlayMemoryRequirement(const FVerificationResult& Result);
```
...
|---|---|---:|---|---|
| Test Duration / Memory Threshold | Test Config / Number / Rule | TBD | テスト計画 / 不具合管理 | 合否基準または検証条件 |
...
| 種別 | 内容 |
|---|---|
| 開発UI | Memory Snapshotと推移Graphをテスト記録へ添付する |
...
- [ ] 定義済み長時間試験で継続的な単調増加がなく、増加時はOwner/参照経路が特定される。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-NFR-QUALITY-005-001` | 基準確認 | すべての前提条件が固定済み | 長時間反復シナリオで検証する | 定義済み長時間試験で継続的な単調増加がなく、増加時はOwner/参照経路が特定される |
| `TC-NFR-QUALITY-005-002` | 再現性 | 同じBuild・環境・手順 | 別担当者が再実施する | 許容差内で同じ判定となる |
| `TC-NFR-QUALITY-005-003` | 条件不備 | 必須Metadataまたは証跡なし | 判定を要求する | PassにせずBlockedまたは測定不能となる |
...
| ID | 内容 | 影響範囲 | 決定期限 | 状態 |
|---|---|---|---|---|
| `Q-NFR-QUALITY-005` | 試験時間と許容増加量はTBD | 設計・計測・テスト | Release判定前 | Open |
...
|---|---|---|---|---|
| `ADR-NFR-QUALITY-005-001` | 検証方式 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | 担当者の主観だけで合否を決める | 再現性、比較可能性、Release判断の追跡性を保つため |
...
|---:|---|---|---|
| 1 | `NFR-QUALITY-005: 検証条件・Fixture整備` | 環境、手順、期待値、証跡形式が確定する | なし |
| 2 | `NFR-QUALITY-005: 計測・レビュー・試験` | 基準Buildの結果と証跡が揃う | #1 |
| 3 | `NFR-QUALITY-005: 未達修正と再確認` | Must/Should基準がPassまたは承認済み判断となる | #2 |
