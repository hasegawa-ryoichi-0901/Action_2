...
| 要件種別 | 非機能要件 |
| 要件カテゴリ | 品質 |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「既知のクラッシュを0件にする」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
| 関連Issue | 未割当 |
| 関連PR | 未割当 |
...
- 元要件「既知のクラッシュを0件にする」の判定基準、検証条件、証跡を定義する。
- Crashを収集・分類・修正後に元手順と関連ライフサイクル経路を再試験する。
...
- Crash再現手順、Callstack、Build ID、修正確認条件を管理する。
...
4. Crashを収集・分類・修正後に元手順と関連ライフサイクル経路を再試験する。
5. 未解決既知Crash件数を証跡とともに記録する。
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 検証対象 | Build / Source / Asset / Save Fixture | Yes | Crash Report / Callstack |
| 実行条件 | Hardware Profile / Test Matrix / Checklist | Yes | Crash再現手順、Callstack、Build ID、修正確認条件を管理する |
| 比較基準 | 数値閾値 / 0件条件 / Architecture Rule | Yes | Known Crash Count |
...
| 証跡 | Report / Log / Trace / Screenshot / Checklist | 未解決既知Crash件数 |
...
FVerificationResult VerifyVerifyNoKnownCrashes(const FVerificationContext& Context);
bool MeetsVerifyNoKnownCrashesRequirement(const FVerificationResult& Result);
```
...
|---|---|---:|---|---|
| Known Crash Count | Test Config / Number / Rule | 0 | テスト計画 / 不具合管理 | 合否基準または検証条件 |
...
| 種別 | 内容 |
|---|---|
| 開発UI | Crash ReporterとログへBuild識別情報を残す |
...
- [ ] Release候補Buildで再現可能な未解決既知Crashが0件である。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-NFR-QUALITY-002-001` | 基準確認 | すべての前提条件が固定済み | Crash Report / Callstackで検証する | Release候補Buildで再現可能な未解決既知Crashが0件である |
| `TC-NFR-QUALITY-002-002` | 再現性 | 同じBuild・環境・手順 | 別担当者が再実施する | 許容差内で同じ判定となる |
| `TC-NFR-QUALITY-002-003` | 条件不備 | 必須Metadataまたは証跡なし | 判定を要求する | PassにせずBlockedまたは測定不能となる |
...
|---|---|---|---|---|
| `ADR-NFR-QUALITY-002-001` | 検証方式 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | 担当者の主観だけで合否を決める | 再現性、比較可能性、Release判断の追跡性を保つため |
...
|---:|---|---|---|
| 1 | `NFR-QUALITY-002: 検証条件・Fixture整備` | 環境、手順、期待値、証跡形式が確定する | なし |
| 2 | `NFR-QUALITY-002: 計測・レビュー・試験` | 基準Buildの結果と証跡が揃う | #1 |
| 3 | `NFR-QUALITY-002: 未達修正と再確認` | Must/Should基準がPassまたは承認済み判断となる | #2 |
