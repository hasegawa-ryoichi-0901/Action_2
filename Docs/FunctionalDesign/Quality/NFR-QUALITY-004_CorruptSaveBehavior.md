...
| 要件種別 | 非機能要件 |
| 要件カテゴリ | 品質 |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「セーブ破損時の挙動を確認する」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
| 関連Issue | 未割当 |
| 関連PR | 未割当 |
...
不正・中断された保存でCrashや進行不能を起こさず回復手段を提供する。
...
- 元要件「セーブ破損時の挙動を確認する」の判定基準、検証条件、証跡を定義する。
- ValidateData失敗時に本データを使用せずBackup・復旧・新規作成・Errorの決定方針へ分岐する。
...
- 破損、Version不正、値不正、一時File残存のFixtureを用意する。
...
4. ValidateData失敗時に本データを使用せずBackup・復旧・新規作成・Errorの決定方針へ分岐する。
5. 安全なLoad結果と診断Logを証跡とともに記録する。
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 検証対象 | Build / Source / Asset / Save Fixture | Yes | 破損Save Fixture |
| 実行条件 | Hardware Profile / Test Matrix / Checklist | Yes | 破損、Version不正、値不正、一時File残存のFixtureを用意する |
| 比較基準 | 数値閾値 / 0件条件 / Architecture Rule | Yes | SaveVersion / Backup Policy |
...
| 証跡 | Report / Log / Trace / Screenshot / Checklist | 安全なLoad結果と診断Log |
...
FVerificationResult VerifyVerifyCorruptSaveRecovery(const FVerificationContext& Context);
bool MeetsVerifyCorruptSaveRecoveryRequirement(const FVerificationResult& Result);
```
...
|---|---|---:|---|---|
| SaveVersion / Backup Policy | Test Config / Number / Rule | TBD | テスト計画 / 不具合管理 | 合否基準または検証条件 |
...
| 種別 | 内容 |
|---|---|
| 開発UI | ユーザー向けErrorと開発用診断Logを分離する |
...
- [ ] 各破損FixtureでCrashせず、破損元を保全して明示された復旧経路へ進む。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-NFR-QUALITY-004-001` | 基準確認 | すべての前提条件が固定済み | 破損Save Fixtureで検証する | 各破損FixtureでCrashせず、破損元を保全して明示された復旧経路へ進む |
| `TC-NFR-QUALITY-004-002` | 再現性 | 同じBuild・環境・手順 | 別担当者が再実施する | 許容差内で同じ判定となる |
| `TC-NFR-QUALITY-004-003` | 条件不備 | 必須Metadataまたは証跡なし | 判定を要求する | PassにせずBlockedまたは測定不能となる |
...
| ID | 内容 | 影響範囲 | 決定期限 | 状態 |
|---|---|---|---|---|
| `Q-NFR-QUALITY-004` | 自動Backup世代数と新規Save移行のユーザー確認方針はTBD | 設計・計測・テスト | Release判定前 | Open |
...
|---|---|---|---|---|
| `ADR-NFR-QUALITY-004-001` | 検証方式 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | 担当者の主観だけで合否を決める | 再現性、比較可能性、Release判断の追跡性を保つため |
...
|---:|---|---|---|
| 1 | `NFR-QUALITY-004: 検証条件・Fixture整備` | 環境、手順、期待値、証跡形式が確定する | なし |
| 2 | `NFR-QUALITY-004: 計測・レビュー・試験` | 基準Buildの結果と証跡が揃う | #1 |
| 3 | `NFR-QUALITY-004: 未達修正と再確認` | Must/Should基準がPassまたは承認済み判断となる | #2 |
