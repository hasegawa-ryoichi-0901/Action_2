...
| 要件種別 | 非機能要件 |
| 要件カテゴリ | 品質 |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「進行不能バグを0件にする」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
| 関連Issue | 未割当 |
| 関連PR | 未割当 |
...
新規開始からStage Clearまで必須経路を完了可能にする。
...
- 元要件「進行不能バグを0件にする」の判定基準、検証条件、証跡を定義する。
- 通常経路と死亡・再開を含む経路を反復し、進行不能をRelease blockerとして管理する。
...
- 通しテスト手順・必要Asset・Checkpoint・Boss状態を固定する。
...
4. 通常経路と死亡・再開を含む経路を反復し、進行不能をRelease blockerとして管理する。
5. 進行不能件数と再現証跡を証跡とともに記録する。
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 検証対象 | Build / Source / Asset / Save Fixture | Yes | 通しテストBuild |
| 実行条件 | Hardware Profile / Test Matrix / Checklist | Yes | 通しテスト手順・必要Asset・Checkpoint・Boss状態を固定する |
| 比較基準 | 数値閾値 / 0件条件 / Architecture Rule | Yes | Blocker Defect Count |
...
| 証跡 | Report / Log / Trace / Screenshot / Checklist | 進行不能件数と再現証跡 |
...
FVerificationResult VerifyVerifyNoProgressionBlockers(const FVerificationContext& Context);
bool MeetsVerifyNoProgressionBlockersRequirement(const FVerificationResult& Result);
```
...
|---|---|---:|---|---|
| Blocker Defect Count | Test Config / Number / Rule | 0 | テスト計画 / 不具合管理 | 合否基準または検証条件 |
...
| 種別 | 内容 |
|---|---|
| 開発UI | 失敗地点・Save・Log・動画を不具合へ添付する |
...
- [ ] Release候補Buildで既知の未解決進行不能が0件であり、必須通しテストが完了する。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-NFR-QUALITY-001-001` | 基準確認 | すべての前提条件が固定済み | 通しテストBuildで検証する | Release候補Buildで既知の未解決進行不能が0件であり、必須通しテストが完了する |
| `TC-NFR-QUALITY-001-002` | 再現性 | 同じBuild・環境・手順 | 別担当者が再実施する | 許容差内で同じ判定となる |
| `TC-NFR-QUALITY-001-003` | 条件不備 | 必須Metadataまたは証跡なし | 判定を要求する | PassにせずBlockedまたは測定不能となる |
...
|---|---|---|---|---|
| `ADR-NFR-QUALITY-001-001` | 検証方式 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | 担当者の主観だけで合否を決める | 再現性、比較可能性、Release判断の追跡性を保つため |
...
|---:|---|---|---|
| 1 | `NFR-QUALITY-001: 検証条件・Fixture整備` | 環境、手順、期待値、証跡形式が確定する | なし |
| 2 | `NFR-QUALITY-001: 計測・レビュー・試験` | 基準Buildの結果と証跡が揃う | #1 |
| 3 | `NFR-QUALITY-001: 未達修正と再確認` | Must/Should基準がPassまたは承認済み判断となる | #2 |
