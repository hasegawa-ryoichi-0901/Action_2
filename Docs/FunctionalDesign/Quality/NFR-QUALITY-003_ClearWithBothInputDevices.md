...
| 要件名 | ゲームパッドとキーボード・マウスの両方でクリア確認を行う |
| 要件種別 | 非機能要件 |
| 要件カテゴリ | 品質 |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「ゲームパッドとキーボード・マウスの両方でクリア確認を行う」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
| 関連Issue | 未割当 |
| 関連PR | 未割当 |
...
両入力方式で必須操作と全攻略経路が利用可能であることを保証する。
...
- 元要件「ゲームパッドとキーボード・マウスの両方でクリア確認を行う」の判定基準、検証条件、証跡を定義する。
- 各デバイスを主入力として新規開始からBoss撃破・Stage Clearまで実行する。
...
- デバイス別Mapping、UI表記、同一Release候補Buildを確認する。
...
4. 各デバイスを主入力として新規開始からBoss撃破・Stage Clearまで実行する。
5. デバイス別通しテスト結果を証跡とともに記録する。
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 検証対象 | Build / Source / Asset / Save Fixture | Yes | ゲームパッド / キーボード・マウス |
| 実行条件 | Hardware Profile / Test Matrix / Checklist | Yes | デバイス別Mapping、UI表記、同一Release候補Buildを確認する |
| 比較基準 | 数値閾値 / 0件条件 / Architecture Rule | Yes | Device Test Matrix |
...
| 証跡 | Report / Log / Trace / Screenshot / Checklist | デバイス別通しテスト結果 |
...
FVerificationResult VerifyVerifyInputDevicePlaythroughs(const FVerificationContext& Context);
bool MeetsVerifyInputDevicePlaythroughsRequirement(const FVerificationResult& Result);
```
...
|---|---|---:|---|---|
| Device Test Matrix | Test Config / Number / Rule | Data Asset定義値 | テスト計画 / 不具合管理 | 合否基準または検証条件 |
...
- [ ] 両デバイスで補助デバイスへ切り替えずStage Clearできる。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-NFR-QUALITY-003-001` | 基準確認 | すべての前提条件が固定済み | ゲームパッド / キーボード・マウスで検証する | 両デバイスで補助デバイスへ切り替えずStage Clearできる |
| `TC-NFR-QUALITY-003-002` | 再現性 | 同じBuild・環境・手順 | 別担当者が再実施する | 許容差内で同じ判定となる |
| `TC-NFR-QUALITY-003-003` | 条件不備 | 必須Metadataまたは証跡なし | 判定を要求する | PassにせずBlockedまたは測定不能となる |
...
|---|---|---|---|---|
| `ADR-NFR-QUALITY-003-001` | 検証方式 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | 担当者の主観だけで合否を決める | 再現性、比較可能性、Release判断の追跡性を保つため |
...
|---:|---|---|---|
| 1 | `NFR-QUALITY-003: 検証条件・Fixture整備` | 環境、手順、期待値、証跡形式が確定する | なし |
| 2 | `NFR-QUALITY-003: 計測・レビュー・試験` | 基準Buildの結果と証跡が揃う | #1 |
| 3 | `NFR-QUALITY-003: 未達修正と再確認` | Must/Should基準がPassまたは承認済み判断となる | #2 |
