...
|---|---|
| 要件ID | `FR-PLAYER-017` |
| 要件名 | 斧でガードまたは受け止めを実行できる |
| 要件種別 | 機能要件 |
| 要件カテゴリ | プレイヤー |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「斧でガードまたは受け止めを実行できる」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
...
- 元要件「斧でガードまたは受け止めを実行できる」を実現する。
- 防御中の攻撃をGuardPolicyに従って軽減・受け止めし、必要なStaminaとリアクションを適用する。
...
| `UGA_Guard` | 実行可否判定、状態変更、主要処理を担当する |
...
- 斧装備、スタミナ、攻撃のGuardPolicy、禁止Tagを検証する。
...
- 被ダメージ・Stamina・リアクションが要件どおり反映される。
...
[開始条件成立]
      |
      v
[Input.Guard、敵Attack Eventを受け付ける]
      |
      v
[斧装備、スタミナ、攻撃のGuardPolicy、禁止Tagを検証する]
      |
...
4. 防御中の攻撃をGuardPolicyに従って軽減・受け止めし、必要なStaminaとリアクションを適用する。
5. 被ダメージ・Stamina・リアクションを反映する。
...
|---|---|---|---|
| `State.Action.Guarding` | 要件の状態・受付・通知 | 処理開始または条件成立時 | 完了・中断・条件解除時 |
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 実行入力 | InputAction / Gameplay Event / System Event | Yes | Input.Guard、敵Attack Event |
| 状態・対象参照 | Gameplay Tag / Attribute / Weak Object Reference | Yes | 斧装備、スタミナ、攻撃のGuardPolicy、禁止Tagを検証する |
| 設定データ | Data Asset / Config | Yes | GuardPolicy / StaggerResistance |

### 10.2 出力

| 出力項目 | 型・形式 | 説明 |
|---|---|---|
| 実行結果 | State / Attribute / Result Event | 被ダメージ・Stamina・リアクション |
| 通知 | Delegate / Gameplay Event | 完了・失敗・中断を必要な連携先へ通知する |
| フィードバック | UI / Animation / VFX / SE | 防御成功・崩し・失敗のAnimation、VFX、SEを区別する |
...
| クラス・システム | 責務 | 実装区分 |
|---|---|---|
| `UGA_Guard` | 実行可否、状態遷移、主要ロジック、重複防止 | `C++` |
| `Primary Data Asset` | GuardPolicy / StaggerResistanceとAsset参照 | `Data Asset / Config` |
...
- GuardPolicy / StaggerResistanceを保持する。
...
```cpp
DECLARE_MULTICAST_DELEGATE(FOnGuardStarted);
DECLARE_MULTICAST_DELEGATE(FOnGuardCompleted);
DECLARE_MULTICAST_DELEGATE(FOnGuardCancelled);
DECLARE_MULTICAST_DELEGATE_OneParam(FOnGuardFailed, EFailureReason);
```
...
|---|---|---:|---|---|
| GuardPolicy / StaggerResistance | Data Asset / Struct / Config | TBD | Primary Data Asset | 要件固有の判定・調整データ |
...
| 種別 | 内容 |
|---|---|
| UI | 防御成功・崩し・失敗のAnimation、VFX、SEを区別する |
...
- [ ] Guardable攻撃へ定義済み効果を適用し、GuardBreakは防御成功にしない。
- [ ] 成功通知が1回発行され、一時状態が完了時に解除される。
- [ ] 防御成功・崩し・失敗のAnimation、VFX、SEを区別する。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-FR-PLAYER-017-001` | 正常系 | すべての事前条件を満たす | Input.Guard、敵Attack Eventを発生させる | Guardable攻撃へ定義済み効果を適用し、GuardBreakは防御成功にしない |
| `TC-FR-PLAYER-017-002` | 異常系 | 必須参照または実行条件を1つ無効にする | 同じ要求を発生させる | 状態とリソースを変更せず安全に拒否する |
| `TC-FR-PLAYER-017-003` | 多重・中断 | 処理中または連続要求 | 再要求、死亡、Owner破棄のいずれかを発生させる | 重複せず一時状態を解除しCrashしない |
...
| ID | 内容 | 影響範囲 | 決定期限 | 状態 |
|---|---|---|---|---|
| `Q-FR-PLAYER-017` | ガードか受け止めか、消費量、GuardBreak、Super Armor対象はTBD | 設計・データ・テスト | 実装Issue開始前 | Open |
...
|---|---|---|---|---|
| `ADR-FR-PLAYER-017-001` | 責務配置 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | Blueprintだけにルールと状態を保持する | 状態整合性、検証容易性、データ調整性を保つため |
...
|---:|---|---|---|
| 1 | `FR-PLAYER-017: データ・状態・公開Interface` | 必須データ、状態、Validation、APIが定義される | なし |
| 2 | `FR-PLAYER-017: 主要ロジック` | 正常・拒否・中断・多重実行テストが通る | #1 |
| 3 | `FR-PLAYER-017: Asset・UI・結合確認` | 必須Assetを設定し受入条件を確認できる | #2 |
