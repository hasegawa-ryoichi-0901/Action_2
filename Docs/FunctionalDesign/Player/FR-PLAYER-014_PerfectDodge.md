...
| 要件種別 | 機能要件 |
| 要件カテゴリ | プレイヤー |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「ジャスト回避を実行できる」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
...
- 元要件「ジャスト回避を実行できる」を実現する。
- 成功時に対象敵を記録しCounter受付Tagを付与する。スローモーションは使用しない。
...
| `UGA_Dodge / UCombatComponent` | 実行可否判定、状態変更、主要処理を担当する |
...
- Dodge Ability中、Window.PerfectDodgeと敵攻撃判定の重なりを検証する。
...
- 成功通知、対象参照、Counter Windowが要件どおり反映される。
...
4. 成功時に対象敵を記録しCounter受付Tagを付与する。スローモーションは使用しない。
5. 成功通知、対象参照、Counter Windowを反映する。
...
|---|---|---|---|
| `Window.PerfectDodge` | 要件の状態・受付・通知 | 処理開始または条件成立時 | 完了・中断・条件解除時 |
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 実行入力 | InputAction / Gameplay Event / System Event | Yes | 回避入力、敵Attack Event |
| 状態・対象参照 | Gameplay Tag / Attribute / Weak Object Reference | Yes | Dodge Ability中、Window.PerfectDodgeと敵攻撃判定の重なりを検証する |
| 設定データ | Data Asset / Config | Yes | PerfectDodgeWindow |

### 10.2 出力

| 出力項目 | 型・形式 | 説明 |
|---|---|---|
| 実行結果 | State / Attribute / Result Event | 成功通知、対象参照、Counter Window |
| 通知 | Delegate / Gameplay Event | 完了・失敗・中断を必要な連携先へ通知する |
| フィードバック | UI / Animation / VFX / SE | 短いHit StopとHUDカウンター表示を行う |
...
| クラス・システム | 責務 | 実装区分 |
|---|---|---|
| `UGA_Dodge / UCombatComponent` | 実行可否、状態遷移、主要ロジック、重複防止 | `C++` |
| `Primary Data Asset` | PerfectDodgeWindowとAsset参照 | `Data Asset / Config` |
...
void RequestPerfectDodge();
void CancelPerfectDodge();
bool IsPerfectDodgeActive() const;
```

### 12.2 通知イベント

```cpp
DECLARE_MULTICAST_DELEGATE(FOnPerfectDodgeStarted);
DECLARE_MULTICAST_DELEGATE(FOnPerfectDodgeCompleted);
DECLARE_MULTICAST_DELEGATE(FOnPerfectDodgeCancelled);
DECLARE_MULTICAST_DELEGATE_OneParam(FOnPerfectDodgeFailed, EFailureReason);
```
...
|---|---|---:|---|---|
| PerfectDodgeWindow | Data Asset / Struct / Config | TBD | Primary Data Asset | 要件固有の判定・調整データ |
...
- [ ] 受付時間内に攻撃を回避すると1回だけ成功し、通常回避と被弾を同時成立させない。
- [ ] 成功通知が1回発行され、一時状態が完了時に解除される。
- [ ] 短いHit StopとHUDカウンター表示を行う。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-FR-PLAYER-014-001` | 正常系 | すべての事前条件を満たす | 回避入力、敵Attack Eventを発生させる | 受付時間内に攻撃を回避すると1回だけ成功し、通常回避と被弾を同時成立させない |
| `TC-FR-PLAYER-014-002` | 異常系 | 必須参照または実行条件を1つ無効にする | 同じ要求を発生させる | 状態とリソースを変更せず安全に拒否する |
| `TC-FR-PLAYER-014-003` | 多重・中断 | 処理中または連続要求 | 再要求、死亡、Owner破棄のいずれかを発生させる | 重複せず一時状態を解除しCrashしない |
...
| ID | 内容 | 影響範囲 | 決定期限 | 状態 |
|---|---|---|---|---|
| `Q-FR-PLAYER-014` | 回避モーションとジャスト回避受付時間はTBD | 設計・データ・テスト | 実装Issue開始前 | Open |
...
|---|---|---|---|---|
| `ADR-FR-PLAYER-014-001` | 責務配置 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | Blueprintだけにルールと状態を保持する | 状態整合性、検証容易性、データ調整性を保つため |
...
|---:|---|---|---|
| 1 | `FR-PLAYER-014: データ・状態・公開Interface` | 必須データ、状態、Validation、APIが定義される | なし |
| 2 | `FR-PLAYER-014: 主要ロジック` | 正常・拒否・中断・多重実行テストが通る | #1 |
| 3 | `FR-PLAYER-014: Asset・UI・結合確認` | 必須Assetを設定し受入条件を確認できる | #2 |
