...
| 要件種別 | 機能要件 |
| 要件カテゴリ | プレイヤー |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「攻撃と回避でスタミナを消費する」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
...
- 元要件「攻撃と回避でスタミナを消費する」を実現する。
- GASのCost Gameplay Effectで行動ごとのStaminaCostを一度だけ減算する。
...
| `UActionAbilitySystemComponent / Gameplay Effect` | 実行可否判定、状態変更、主要処理を担当する |
...
- Ability起動時に現在Staminaが定義Cost以上であることを検証する。
...
4. GASのCost Gameplay Effectで行動ごとのStaminaCostを一度だけ減算する。
5. Stamina属性変更を反映する。
...
|---|---|---|---|
| `Attribute.Stamina` | 要件の状態・受付・通知 | 処理開始または条件成立時 | 完了・中断・条件解除時 |
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 実行入力 | InputAction / Gameplay Event / System Event | Yes | Ability起動要求、StaminaCost |
| 状態・対象参照 | Gameplay Tag / Attribute / Weak Object Reference | Yes | Ability起動時に現在Staminaが定義Cost以上であることを検証する |
| 設定データ | Data Asset / Config | Yes | AttackDefinition.StaminaCost |

### 10.2 出力

| 出力項目 | 型・形式 | 説明 |
|---|---|---|
| 実行結果 | State / Attribute / Result Event | Stamina属性変更 |
| 通知 | Delegate / Gameplay Event | 完了・失敗・中断を必要な連携先へ通知する |
| フィードバック | UI / Animation / VFX / SE | HUDのスタミナゲージを更新する |
...
| クラス・システム | 責務 | 実装区分 |
|---|---|---|
| `UActionAbilitySystemComponent / Gameplay Effect` | 実行可否、状態遷移、主要ロジック、重複防止 | `C++` |
| `Primary Data Asset` | AttackDefinition.StaminaCostとAsset参照 | `Data Asset / Config` |
...
- AttackDefinition.StaminaCostを保持する。
...
bool CanApplyStaminaCost() const;
void RequestApplyStaminaCost();
void CancelApplyStaminaCost();
bool IsApplyStaminaCostActive() const;
```

### 12.2 通知イベント

```cpp
DECLARE_MULTICAST_DELEGATE(FOnApplyStaminaCostStarted);
DECLARE_MULTICAST_DELEGATE(FOnApplyStaminaCostCompleted);
DECLARE_MULTICAST_DELEGATE(FOnApplyStaminaCostCancelled);
DECLARE_MULTICAST_DELEGATE_OneParam(FOnApplyStaminaCostFailed, EFailureReason);
```
...
[Player]
        |
        v
[UActionAbilitySystemComponent / Gameplay Effect]
   |          |
   v          v
...
|---|---|---:|---|---|
| AttackDefinition.StaminaCost | Data Asset / Struct / Config | TBD | Primary Data Asset | 要件固有の判定・調整データ |
...
- [ ] 成功した攻撃・回避の開始時に定義量を1回消費し、キャンセル経路で二重消費しない。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-FR-PLAYER-011-001` | 正常系 | すべての事前条件を満たす | Ability起動要求、StaminaCostを発生させる | 成功した攻撃・回避の開始時に定義量を1回消費し、キャンセル経路で二重消費しない |
| `TC-FR-PLAYER-011-002` | 異常系 | 必須参照または実行条件を1つ無効にする | 同じ要求を発生させる | 状態とリソースを変更せず安全に拒否する |
| `TC-FR-PLAYER-011-003` | 多重・中断 | 処理中または連続要求 | 再要求、死亡、Owner破棄のいずれかを発生させる | 重複せず一時状態を解除しCrashしない |
...
| ID | 内容 | 影響範囲 | 決定期限 | 状態 |
|---|---|---|---|---|
| `Q-FR-PLAYER-011` | 最大値、各行動Cost、回復開始遅延はTBD | 設計・データ・テスト | 実装Issue開始前 | Open |
...
|---|---|---|---|---|
| `ADR-FR-PLAYER-011-001` | 責務配置 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | Blueprintだけにルールと状態を保持する | 状態整合性、検証容易性、データ調整性を保つため |
...
|---:|---|---|---|
| 1 | `FR-PLAYER-011: データ・状態・公開Interface` | 必須データ、状態、Validation、APIが定義される | なし |
| 2 | `FR-PLAYER-011: 主要ロジック` | 正常・拒否・中断・多重実行テストが通る | #1 |
| 3 | `FR-PLAYER-011: Asset・UI・結合確認` | 必須Assetを設定し受入条件を確認できる | #2 |
