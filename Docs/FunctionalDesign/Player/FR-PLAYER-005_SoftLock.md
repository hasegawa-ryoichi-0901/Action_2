...
| 要件種別 | 機能要件 |
| 要件カテゴリ | プレイヤー |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「ソフトロックを使用できる」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
...
通常移動中の攻撃を画面内の妥当な敵へ補正し、操作負荷を下げる。
...
- 元要件「ソフトロックを使用できる」を実現する。
- UTargetingComponentが画面位置・距離・角度から一時攻撃対象を選ぶ。
...
| `UTargetingComponent` | 実行可否判定、状態変更、主要処理を担当する |
...
4. UTargetingComponentが画面位置・距離・角度から一時攻撃対象を選ぶ。
5. 一時ターゲット参照とFacing要求を反映する。
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 実行入力 | InputAction / Gameplay Event / System Event | Yes | 攻撃要求、カメラ、敵候補 |
| 状態・対象参照 | Gameplay Tag / Attribute / Weak Object Reference | Yes | 候補が生存中・範囲内・遮蔽されていないことを検証する |
| 設定データ | Data Asset / Config | Yes | ソフトロック距離・角度・スコア重み |

### 10.2 出力

| 出力項目 | 型・形式 | 説明 |
|---|---|---|
| 実行結果 | State / Attribute / Result Event | 一時ターゲット参照とFacing要求 |
| 通知 | Delegate / Gameplay Event | 完了・失敗・中断を必要な連携先へ通知する |
| フィードバック | UI / Animation / VFX / SE | 強制的なロックオンUIは表示しない |
...
| クラス・システム | 責務 | 実装区分 |
|---|---|---|
| `UTargetingComponent` | 実行可否、状態遷移、主要ロジック、重複防止 | `C++` |
| `Primary Data Asset` | ソフトロック距離・角度・スコア重みとAsset参照 | `Data Asset / Config` |
...
bool CanSoftLock() const;
void RequestSoftLock();
void CancelSoftLock();
bool IsSoftLockActive() const;
```

### 12.2 通知イベント

```cpp
DECLARE_MULTICAST_DELEGATE(FOnSoftLockStarted);
DECLARE_MULTICAST_DELEGATE(FOnSoftLockCompleted);
DECLARE_MULTICAST_DELEGATE(FOnSoftLockCancelled);
DECLARE_MULTICAST_DELEGATE_OneParam(FOnSoftLockFailed, EFailureReason);
```
...
|---|---|---:|---|---|
| ソフトロック距離・角度・スコア重み | Data Asset / Struct / Config | TBD | Primary Data Asset | 要件固有の判定・調整データ |
...
- [ ] 有効候補がある攻撃時だけ最適候補へ向きを補正し、候補なしでも攻撃できる。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-FR-PLAYER-005-001` | 正常系 | すべての事前条件を満たす | 攻撃要求、カメラ、敵候補を発生させる | 有効候補がある攻撃時だけ最適候補へ向きを補正し、候補なしでも攻撃できる |
| `TC-FR-PLAYER-005-002` | 異常系 | 必須参照または実行条件を1つ無効にする | 同じ要求を発生させる | 状態とリソースを変更せず安全に拒否する |
| `TC-FR-PLAYER-005-003` | 多重・中断 | 処理中または連続要求 | 再要求、死亡、Owner破棄のいずれかを発生させる | 重複せず一時状態を解除しCrashしない |
...
| ID | 内容 | 影響範囲 | 決定期限 | 状態 |
|---|---|---|---|---|
| `Q-FR-PLAYER-005` | 最大距離、角度、スコア重みはTBD | 設計・データ・テスト | 実装Issue開始前 | Open |
...
|---|---|---|---|---|
| `ADR-FR-PLAYER-005-001` | 責務配置 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | Blueprintだけにルールと状態を保持する | 状態整合性、検証容易性、データ調整性を保つため |
...
|---:|---|---|---|
| 1 | `FR-PLAYER-005: データ・状態・公開Interface` | 必須データ、状態、Validation、APIが定義される | なし |
| 2 | `FR-PLAYER-005: 主要ロジック` | 正常・拒否・中断・多重実行テストが通る | #1 |
| 3 | `FR-PLAYER-005: Asset・UI・結合確認` | 必須Assetを設定し受入条件を確認できる | #2 |
