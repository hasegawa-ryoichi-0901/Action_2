...
| 要件種別 | 機能要件 |
| 要件カテゴリ | プレイヤー |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「三人称視点で移動できる」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
...
- 元要件「三人称視点で移動できる」を実現する。
- 移動入力をIMovementDriverへ渡し、カメラ基準の移動要求へ変換する。
...
| `UMovementAdapterComponent / IMovementDriver` | 実行可否判定、状態変更、主要処理を担当する |
...
4. 移動入力をIMovementDriverへ渡し、カメラ基準の移動要求へ変換する。
5. 移動速度と向きを反映する。
...
|---|---|---|---|
| `State.Block.Movement` | 要件の状態・受付・通知 | 処理開始または条件成立時 | 完了・中断・条件解除時 |
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 実行入力 | InputAction / Gameplay Event / System Event | Yes | 2D移動軸、カメラ向き |
| 状態・対象参照 | Gameplay Tag / Attribute / Weak Object Reference | Yes | 移動入力、操作禁止Tag、接地状態を検証する |
| 設定データ | Data Asset / Config | Yes | 移動速度・加減速 |

### 10.2 出力

| 出力項目 | 型・形式 | 説明 |
|---|---|---|
| 実行結果 | State / Attribute / Result Event | 移動速度と向き |
| 通知 | Delegate / Gameplay Event | 完了・失敗・中断を必要な連携先へ通知する |
| フィードバック | UI / Animation / VFX / SE | 移動Animationとカメラ追従を更新する |
...
| クラス・システム | 責務 | 実装区分 |
|---|---|---|
| `UMovementAdapterComponent / IMovementDriver` | 実行可否、状態遷移、主要ロジック、重複防止 | `C++` |
| `Primary Data Asset` | 移動速度・加減速とAsset参照 | `Data Asset / Config` |
...
```cpp
DECLARE_MULTICAST_DELEGATE(FOnMoveStarted);
DECLARE_MULTICAST_DELEGATE(FOnMoveCompleted);
DECLARE_MULTICAST_DELEGATE(FOnMoveCancelled);
DECLARE_MULTICAST_DELEGATE_OneParam(FOnMoveFailed, EFailureReason);
```
...
[Player]
        |
        v
[UMovementAdapterComponent / IMovementDriver]
   |          |
   v          v
...
|---|---|---:|---|---|
| 移動速度・加減速 | Data Asset / Struct / Config | TBD | Primary Data Asset | 要件固有の判定・調整データ |
...
- [ ] 入力方向へ三人称カメラ基準で移動し、入力0で加速を停止する。
- [ ] 成功通知が1回発行され、一時状態が完了時に解除される。
- [ ] 移動Animationとカメラ追従を更新する。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-FR-PLAYER-001-001` | 正常系 | すべての事前条件を満たす | 2D移動軸、カメラ向きを発生させる | 入力方向へ三人称カメラ基準で移動し、入力0で加速を停止する |
| `TC-FR-PLAYER-001-002` | 異常系 | 必須参照または実行条件を1つ無効にする | 同じ要求を発生させる | 状態とリソースを変更せず安全に拒否する |
| `TC-FR-PLAYER-001-003` | 多重・中断 | 処理中または連続要求 | 再要求、死亡、Owner破棄のいずれかを発生させる | 重複せず一時状態を解除しCrashしない |
...
| ID | 内容 | 影響範囲 | 決定期限 | 状態 |
|---|---|---|---|---|
| `Q-FR-PLAYER-001` | Mover継続採用かCharacterMovementComponent移行かはプロトタイプ後に決定する | 設計・データ・テスト | 実装Issue開始前 | Open |
...
|---|---|---|---|---|
| `ADR-FR-PLAYER-001-001` | 責務配置 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | Blueprintだけにルールと状態を保持する | 状態整合性、検証容易性、データ調整性を保つため |
...
|---:|---|---|---|
| 1 | `FR-PLAYER-001: データ・状態・公開Interface` | 必須データ、状態、Validation、APIが定義される | なし |
| 2 | `FR-PLAYER-001: 主要ロジック` | 正常・拒否・中断・多重実行テストが通る | #1 |
| 3 | `FR-PLAYER-001: Asset・UI・結合確認` | 必須Assetを設定し受入条件を確認できる | #2 |
