...
| 要件種別 | 機能要件 |
| 要件カテゴリ | プレイヤー |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「戦闘中は武器を変更できない」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
...
戦闘中のリアルタイム武器切替を排除し、武器選択の一貫性を保つ。
...
- 元要件「戦闘中は武器を変更できない」を実現する。
- 武器変更要求をチェックポイント操作中かつ戦闘外の場合だけ許可する。
- 成功・拒否・中断時の状態整合性と通知までを扱う。

### 3.2 対象外

- リアルタイム武器切り替え（Vertical Slice対象外）。
...
| `UCombatComponent` | 実行可否判定、状態変更、主要処理を担当する |
...
4. 武器変更要求をチェックポイント操作中かつ戦闘外の場合だけ許可する。
5. 許可または拒否結果を反映する。
...
|---|---|---|---|
| `State.Combat.Active` | 要件の状態・受付・通知 | 処理開始または条件成立時 | 完了・中断・条件解除時 |
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 実行入力 | InputAction / Gameplay Event / System Event | Yes | 武器変更要求、戦闘状態Tag |
| 状態・対象参照 | Gameplay Tag / Attribute / Weak Object Reference | Yes | 戦闘状態とチェックポイント利用状態を検証する |
| 設定データ | Data Asset / Config | Yes | 戦闘状態Tag定義 |

### 10.2 出力

| 出力項目 | 型・形式 | 説明 |
|---|---|---|
| 実行結果 | State / Attribute / Result Event | 許可または拒否結果 |
| 通知 | Delegate / Gameplay Event | 完了・失敗・中断を必要な連携先へ通知する |
| フィードバック | UI / Animation / VFX / SE | 拒否理由をチェックポイントUIへ表示する |
...
| クラス・システム | 責務 | 実装区分 |
|---|---|---|
| `UCombatComponent` | 実行可否、状態遷移、主要ロジック、重複防止 | `C++` |
| `Primary Data Asset` | 戦闘状態Tag定義とAsset参照 | `Data Asset / Config` |
...
bool CanValidateWeaponChange() const;
void RequestValidateWeaponChange();
void CancelValidateWeaponChange();
bool IsValidateWeaponChangeActive() const;
```

### 12.2 通知イベント

```cpp
DECLARE_MULTICAST_DELEGATE(FOnValidateWeaponChangeStarted);
DECLARE_MULTICAST_DELEGATE(FOnValidateWeaponChangeCompleted);
DECLARE_MULTICAST_DELEGATE(FOnValidateWeaponChangeCancelled);
DECLARE_MULTICAST_DELEGATE_OneParam(FOnValidateWeaponChangeFailed, EFailureReason);
```
...
|---|---|---:|---|---|
| 戦闘状態Tag定義 | Data Asset / Struct / Config | Data Asset定義値 | Primary Data Asset | 要件固有の判定・調整データ |
...
- [ ] 戦闘状態中の変更要求では装備・Ability・見た目が変化しない。
- [ ] 成功通知が1回発行され、一時状態が完了時に解除される。
- [ ] 拒否理由をチェックポイントUIへ表示する。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-FR-PLAYER-009-001` | 正常系 | すべての事前条件を満たす | 武器変更要求、戦闘状態Tagを発生させる | 戦闘状態中の変更要求では装備・Ability・見た目が変化しない |
| `TC-FR-PLAYER-009-002` | 異常系 | 必須参照または実行条件を1つ無効にする | 同じ要求を発生させる | 状態とリソースを変更せず安全に拒否する |
| `TC-FR-PLAYER-009-003` | 多重・中断 | 処理中または連続要求 | 再要求、死亡、Owner破棄のいずれかを発生させる | 重複せず一時状態を解除しCrashしない |
...
|---|---|---|---|---|
| `ADR-FR-PLAYER-009-001` | 責務配置 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | Blueprintだけにルールと状態を保持する | 状態整合性、検証容易性、データ調整性を保つため |
...
|---:|---|---|---|
| 1 | `FR-PLAYER-009: データ・状態・公開Interface` | 必須データ、状態、Validation、APIが定義される | なし |
| 2 | `FR-PLAYER-009: 主要ロジック` | 正常・拒否・中断・多重実行テストが通る | #1 |
| 3 | `FR-PLAYER-009: Asset・UI・結合確認` | 必須Assetを設定し受入条件を確認できる | #2 |
