...
|---|---|
| 要件ID | `FR-PLAYER-012` |
| 要件名 | スタミナ不足時は対象行動を実行できない |
| 要件種別 | 機能要件 |
| 要件カテゴリ | プレイヤー |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「スタミナ不足時は対象行動を実行できない」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
...
- 元要件「スタミナ不足時は対象行動を実行できない」を実現する。
- Cost未満ならAbilityをActivateせず入力バッファ方針に従って拒否または期限内保留する。
...
| `UActionGameplayAbility` | 実行可否判定、状態変更、主要処理を担当する |
...
- 必要Costと現在StaminaをAbility Commit前に比較する。
...
4. Cost未満ならAbilityをActivateせず入力バッファ方針に従って拒否または期限内保留する。
5. 拒否結果を反映する。
...
|---|---|---|---|
| `Block.Action.InsufficientStamina` | 要件の状態・受付・通知 | 処理開始または条件成立時 | 完了・中断・条件解除時 |
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 実行入力 | InputAction / Gameplay Event / System Event | Yes | 行動要求、現在Stamina、必要Cost |
| 状態・対象参照 | Gameplay Tag / Attribute / Weak Object Reference | Yes | 必要Costと現在StaminaをAbility Commit前に比較する |
| 設定データ | Data Asset / Config | Yes | Ability Cost |

### 10.2 出力

| 出力項目 | 型・形式 | 説明 |
|---|---|---|
| 実行結果 | State / Attribute / Result Event | 拒否結果 |
| 通知 | Delegate / Gameplay Event | 完了・失敗・中断を必要な連携先へ通知する |
| フィードバック | UI / Animation / VFX / SE | 必要に応じてHUDで不足を通知する |
...
| クラス・システム | 責務 | 実装区分 |
|---|---|---|
| `UActionGameplayAbility` | 実行可否、状態遷移、主要ロジック、重複防止 | `C++` |
| `Primary Data Asset` | Ability CostとAsset参照 | `Data Asset / Config` |
...
bool CanValidateStamina() const;
void RequestValidateStamina();
void CancelValidateStamina();
bool IsValidateStaminaActive() const;
```

### 12.2 通知イベント

```cpp
DECLARE_MULTICAST_DELEGATE(FOnValidateStaminaStarted);
DECLARE_MULTICAST_DELEGATE(FOnValidateStaminaCompleted);
DECLARE_MULTICAST_DELEGATE(FOnValidateStaminaCancelled);
DECLARE_MULTICAST_DELEGATE_OneParam(FOnValidateStaminaFailed, EFailureReason);
```
...
|---|---|---:|---|---|
| Ability Cost | Data Asset / Struct / Config | Data Asset定義値 | Primary Data Asset | 要件固有の判定・調整データ |
...
- [ ] StaminaがCost未満のとき対象Ability・Montage・攻撃判定が開始されずCostも減らない。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-FR-PLAYER-012-001` | 正常系 | すべての事前条件を満たす | 行動要求、現在Stamina、必要Costを発生させる | StaminaがCost未満のとき対象Ability・Montage・攻撃判定が開始されずCostも減らない |
| `TC-FR-PLAYER-012-002` | 異常系 | 必須参照または実行条件を1つ無効にする | 同じ要求を発生させる | 状態とリソースを変更せず安全に拒否する |
| `TC-FR-PLAYER-012-003` | 多重・中断 | 処理中または連続要求 | 再要求、死亡、Owner破棄のいずれかを発生させる | 重複せず一時状態を解除しCrashしない |
...
|---|---|---|---|---|
| `ADR-FR-PLAYER-012-001` | 責務配置 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | Blueprintだけにルールと状態を保持する | 状態整合性、検証容易性、データ調整性を保つため |
...
|---:|---|---|---|
| 1 | `FR-PLAYER-012: データ・状態・公開Interface` | 必須データ、状態、Validation、APIが定義される | なし |
| 2 | `FR-PLAYER-012: 主要ロジック` | 正常・拒否・中断・多重実行テストが通る | #1 |
| 3 | `FR-PLAYER-012: Asset・UI・結合確認` | 必須Assetを設定し受入条件を確認できる | #2 |
