...
|---|---|
| 要件ID | `FR-PLAYER-013` |
| 要件名 | スタミナ枯渇時に息切れ状態となる |
| 要件種別 | 機能要件 |
| 要件カテゴリ | プレイヤー |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「スタミナ枯渇時に息切れ状態となる」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
...
- 元要件「スタミナ枯渇時に息切れ状態となる」を実現する。
- State.Action.Exhaustedを付与し、閾値まで攻撃・回避等を禁止し通常移動は許可する。
...
| `UActionAbilitySystemComponent / AttributeSet` | 実行可否判定、状態変更、主要処理を担当する |
...
- Staminaが0以下に到達したことをAttribute変更通知で検証する。
...
4. State.Action.Exhaustedを付与し、閾値まで攻撃・回避等を禁止し通常移動は許可する。
5. 息切れTagと行動制限を反映する。
...
|---|---|---|---|
| `State.Action.Exhausted` | 要件の状態・受付・通知 | 処理開始または条件成立時 | 完了・中断・条件解除時 |
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 実行入力 | InputAction / Gameplay Event / System Event | Yes | Stamina変更 |
| 状態・対象参照 | Gameplay Tag / Attribute / Weak Object Reference | Yes | Staminaが0以下に到達したことをAttribute変更通知で検証する |
| 設定データ | Data Asset / Config | Yes | ExhaustedRecoveryThreshold |

### 10.2 出力

| 出力項目 | 型・形式 | 説明 |
|---|---|---|
| 実行結果 | State / Attribute / Result Event | 息切れTagと行動制限 |
| 通知 | Delegate / Gameplay Event | 完了・失敗・中断を必要な連携先へ通知する |
| フィードバック | UI / Animation / VFX / SE | 息切れAnimation・SE・HUD表示を出す |
...
| クラス・システム | 責務 | 実装区分 |
|---|---|---|
| `UActionAbilitySystemComponent / AttributeSet` | 実行可否、状態遷移、主要ロジック、重複防止 | `C++` |
| `Primary Data Asset` | ExhaustedRecoveryThresholdとAsset参照 | `Data Asset / Config` |
...
- ExhaustedRecoveryThresholdを保持する。
...
bool CanEnterExhausted() const;
void RequestEnterExhausted();
void CancelEnterExhausted();
bool IsEnterExhaustedActive() const;
```

### 12.2 通知イベント

```cpp
DECLARE_MULTICAST_DELEGATE(FOnEnterExhaustedStarted);
DECLARE_MULTICAST_DELEGATE(FOnEnterExhaustedCompleted);
DECLARE_MULTICAST_DELEGATE(FOnEnterExhaustedCancelled);
DECLARE_MULTICAST_DELEGATE_OneParam(FOnEnterExhaustedFailed, EFailureReason);
```
...
[Player]
        |
        v
[UActionAbilitySystemComponent / AttributeSet]
   |          |
   v          v
...
|---|---|---:|---|---|
| ExhaustedRecoveryThreshold | Data Asset / Struct / Config | TBD | Primary Data Asset | 要件固有の判定・調整データ |
...
- [ ] Stamina 0以下で息切れし、ExhaustedRecoveryThreshold以上で一度だけ解除される。
- [ ] 成功通知が1回発行され、一時状態が完了時に解除される。
- [ ] 息切れAnimation・SE・HUD表示を出す。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-FR-PLAYER-013-001` | 正常系 | すべての事前条件を満たす | Stamina変更を発生させる | Stamina 0以下で息切れし、ExhaustedRecoveryThreshold以上で一度だけ解除される |
| `TC-FR-PLAYER-013-002` | 異常系 | 必須参照または実行条件を1つ無効にする | 同じ要求を発生させる | 状態とリソースを変更せず安全に拒否する |
| `TC-FR-PLAYER-013-003` | 多重・中断 | 処理中または連続要求 | 再要求、死亡、Owner破棄のいずれかを発生させる | 重複せず一時状態を解除しCrashしない |
...
| ID | 内容 | 影響範囲 | 決定期限 | 状態 |
|---|---|---|---|---|
| `Q-FR-PLAYER-013` | 息切れ解除閾値はTBD | 設計・データ・テスト | 実装Issue開始前 | Open |
...
|---|---|---|---|---|
| `ADR-FR-PLAYER-013-001` | 責務配置 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | Blueprintだけにルールと状態を保持する | 状態整合性、検証容易性、データ調整性を保つため |
...
|---:|---|---|---|
| 1 | `FR-PLAYER-013: データ・状態・公開Interface` | 必須データ、状態、Validation、APIが定義される | なし |
| 2 | `FR-PLAYER-013: 主要ロジック` | 正常・拒否・中断・多重実行テストが通る | #1 |
| 3 | `FR-PLAYER-013: Asset・UI・結合確認` | 必須Assetを設定し受入条件を確認できる | #2 |
