# FR-PLAYER-015 Perfect Dodge後に武器固有Counterを実行できる
...
| 設計状態 | `Draft` |
| 関連Issue | `#114` |
| 関連要件・設計 | `FR-PLAYER-014`, `Docs/04_WeaponSystem.md` |

## 2. 目的

Perfect Dodge成功を攻撃機会へ変換し、Sword固有の反撃Actionを成立させる。

## 3. 確定仕様・スコープ

- Perfect Dodge成功時のみCounter受付状態を開始する。
- Normal Dodgeでは受付を開始しない。
- Perfect結果で記録したAttack SourceをCounter Targetとして利用可能とする。
- 受付時間は調整可能とする。
- Initial VSではSword Counterを実装し、Axe / Bow固有CounterはPost-VSで追加する。
...
|---|---|
| `Window.Counter.Available` | Counter入力受付期間 |
...
| Attack Source / Target | 反撃対象 | Runtime |
| Montage / Damage | 反撃内容 | Asset / 調整値 |
...
### 依存
- `#57` Perfect Dodge Result
- `#58` Combat共有基盤

### Issue反映
- `#114`へ受付Window、Target保持、Counter Cleanupを反映する。
