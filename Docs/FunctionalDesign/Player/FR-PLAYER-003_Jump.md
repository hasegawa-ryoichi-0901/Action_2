...
| 要件種別 | 機能要件 |
| 要件カテゴリ | プレイヤー |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「ジャンプできる」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
...
- 元要件「ジャンプできる」を実現する。
- UGA_JumpからIMovementDriverへジャンプ要求を送る。
...
| `UGA_Jump / UMovementAdapterComponent` | 実行可否判定、状態変更、主要処理を担当する |
...
- 鉛直移動とState.Action.Jumpingが要件どおり反映される。
...
4. UGA_JumpからIMovementDriverへジャンプ要求を送る。
5. 鉛直移動とState.Action.Jumpingを反映する。
...
|---|---|---|---|
| `State.Action.Jumping` | 要件の状態・受付・通知 | 処理開始または条件成立時 | 完了・中断・条件解除時 |
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 実行入力 | InputAction / Gameplay Event / System Event | Yes | Input.Jump |
| 状態・対象参照 | Gameplay Tag / Attribute / Weak Object Reference | Yes | 接地、死亡・息切れ・強制リアクション中でないことを検証する |
| 設定データ | Data Asset / Config | Yes | ジャンプ初速 |

### 10.2 出力

| 出力項目 | 型・形式 | 説明 |
|---|---|---|
| 実行結果 | State / Attribute / Result Event | 鉛直移動とState.Action.Jumping |
| 通知 | Delegate / Gameplay Event | 完了・失敗・中断を必要な連携先へ通知する |
| フィードバック | UI / Animation / VFX / SE | ジャンプ・落下・着地Animationを再生する |
...
| クラス・システム | 責務 | 実装区分 |
|---|---|---|
| `UGA_Jump / UMovementAdapterComponent` | 実行可否、状態遷移、主要ロジック、重複防止 | `C++` |
| `Primary Data Asset` | ジャンプ初速とAsset参照 | `Data Asset / Config` |
...
```cpp
DECLARE_MULTICAST_DELEGATE(FOnJumpStarted);
DECLARE_MULTICAST_DELEGATE(FOnJumpCompleted);
DECLARE_MULTICAST_DELEGATE(FOnJumpCancelled);
DECLARE_MULTICAST_DELEGATE_OneParam(FOnJumpFailed, EFailureReason);
```
...
[Player]
        |
        v
[UGA_Jump / UMovementAdapterComponent]
   |          |
   v          v
...
|---|---|---:|---|---|
| ジャンプ初速 | Data Asset / Struct / Config | Data Asset定義値 | Primary Data Asset | 要件固有の判定・調整データ |
...
- [ ] 接地中の有効入力で1回ジャンプし、着地時にジャンプ状態を解除する。
- [ ] 成功通知が1回発行され、一時状態が完了時に解除される。
- [ ] ジャンプ・落下・着地Animationを再生する。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-FR-PLAYER-003-001` | 正常系 | すべての事前条件を満たす | Input.Jumpを発生させる | 接地中の有効入力で1回ジャンプし、着地時にジャンプ状態を解除する |
| `TC-FR-PLAYER-003-002` | 異常系 | 必須参照または実行条件を1つ無効にする | 同じ要求を発生させる | 状態とリソースを変更せず安全に拒否する |
| `TC-FR-PLAYER-003-003` | 多重・中断 | 処理中または連続要求 | 再要求、死亡、Owner破棄のいずれかを発生させる | 重複せず一時状態を解除しCrashしない |
...
|---|---|---|---|---|
| `ADR-FR-PLAYER-003-001` | 責務配置 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | Blueprintだけにルールと状態を保持する | 状態整合性、検証容易性、データ調整性を保つため |
...
|---:|---|---|---|
| 1 | `FR-PLAYER-003: データ・状態・公開Interface` | 必須データ、状態、Validation、APIが定義される | なし |
| 2 | `FR-PLAYER-003: 主要ロジック` | 正常・拒否・中断・多重実行テストが通る | #1 |
| 3 | `FR-PLAYER-003: Asset・UI・結合確認` | 必須Assetを設定し受入条件を確認できる | #2 |
