...
|---|---|
| 要件ID | `FR-PLAYER-008` |
| 要件名 | 剣、斧、弓をチェックポイントで変更できる |
| 要件種別 | 機能要件 |
| 要件カテゴリ | プレイヤー |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「剣、斧、弓をチェックポイントで変更できる」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
...
- 元要件「剣、斧、弓をチェックポイントで変更できる」を実現する。
- UCombatComponentが旧AbilitySetを解除し、新武器Actor・AbilitySet・属性補正を適用する。
...
| `UCombatComponent / ACheckpointActor` | 実行可否判定、状態変更、主要処理を担当する |
...
- チェックポイント利用中、戦闘外、選択武器定義が有効であることを検証する。
...
- 装備武器、Ability、見た目、保存対象状態が要件どおり反映される。
...
4. UCombatComponentが旧AbilitySetを解除し、新武器Actor・AbilitySet・属性補正を適用する。
5. 装備武器、Ability、見た目、保存対象状態を反映する。
...
|---|---|---|---|
| `State.Interaction.Checkpoint` | 要件の状態・受付・通知 | 処理開始または条件成立時 | 完了・中断・条件解除時 |
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 実行入力 | InputAction / Gameplay Event / System Event | Yes | 武器ID（剣・斧・弓） |
| 状態・対象参照 | Gameplay Tag / Attribute / Weak Object Reference | Yes | チェックポイント利用中、戦闘外、選択武器定義が有効であることを検証する |
| 設定データ | Data Asset / Config | Yes | WeaponDefinition / AbilitySet |

### 10.2 出力

| 出力項目 | 型・形式 | 説明 |
|---|---|---|
| 実行結果 | State / Attribute / Result Event | 装備武器、Ability、見た目、保存対象状態 |
| 通知 | Delegate / Gameplay Event | 完了・失敗・中断を必要な連携先へ通知する |
| フィードバック | UI / Animation / VFX / SE | 武器モデル・メニュー選択・能力表示を更新する |
...
| クラス・システム | 責務 | 実装区分 |
|---|---|---|
| `UCombatComponent / ACheckpointActor` | 実行可否、状態遷移、主要ロジック、重複防止 | `C++` |
| `Primary Data Asset` | WeaponDefinition / AbilitySetとAsset参照 | `Data Asset / Config` |
...
- WeaponDefinition / AbilitySetを保持する。
...
void CancelChangeWeapon();
bool IsChangeWeaponActive() const;
```

### 12.2 通知イベント

```cpp
DECLARE_MULTICAST_DELEGATE(FOnChangeWeaponStarted);
DECLARE_MULTICAST_DELEGATE(FOnChangeWeaponCompleted);
DECLARE_MULTICAST_DELEGATE(FOnChangeWeaponCancelled);
DECLARE_MULTICAST_DELEGATE_OneParam(FOnChangeWeaponFailed, EFailureReason);
```
...
[Player]
        |
        v
[UCombatComponent / ACheckpointActor]
   |          |
   v          v
...
|---|---|---:|---|---|
| WeaponDefinition / AbilitySet | Data Asset / Struct / Config | Data Asset定義値 | Primary Data Asset | 要件固有の判定・調整データ |
...
- [ ] チェックポイントUIで選んだ武器へ一度だけ切り替わり、対応Abilityが使用可能になる。
- [ ] 成功通知が1回発行され、一時状態が完了時に解除される。
- [ ] 武器モデル・メニュー選択・能力表示を更新する。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-FR-PLAYER-008-001` | 正常系 | すべての事前条件を満たす | 武器ID（剣・斧・弓）を発生させる | チェックポイントUIで選んだ武器へ一度だけ切り替わり、対応Abilityが使用可能になる |
| `TC-FR-PLAYER-008-002` | 異常系 | 必須参照または実行条件を1つ無効にする | 同じ要求を発生させる | 状態とリソースを変更せず安全に拒否する |
| `TC-FR-PLAYER-008-003` | 多重・中断 | 処理中または連続要求 | 再要求、死亡、Owner破棄のいずれかを発生させる | 重複せず一時状態を解除しCrashしない |
...
|---|---|---|---|---|
| `ADR-FR-PLAYER-008-001` | 責務配置 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | Blueprintだけにルールと状態を保持する | 状態整合性、検証容易性、データ調整性を保つため |
...
|---:|---|---|---|
| 1 | `FR-PLAYER-008: データ・状態・公開Interface` | 必須データ、状態、Validation、APIが定義される | なし |
| 2 | `FR-PLAYER-008: 主要ロジック` | 正常・拒否・中断・多重実行テストが通る | #1 |
| 3 | `FR-PLAYER-008: Asset・UI・結合確認` | 必須Assetを設定し受入条件を確認できる | #2 |
