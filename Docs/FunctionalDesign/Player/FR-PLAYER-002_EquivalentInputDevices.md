...
| 要件種別 | 機能要件 |
| 要件カテゴリ | プレイヤー |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「ゲームパッドとキーボード・マウスで同等に操作できる」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
...
入力デバイスに依存せず同じゲームプレイ機能へ到達できるようにする。
...
- 元要件「ゲームパッドとキーボード・マウスで同等に操作できる」を実現する。
- Enhanced InputのActionを共通Input Tagへ変換して同一処理へ配送する。
...
| `AActionPlayerController / Enhanced Input` | 実行可否判定、状態変更、主要処理を担当する |
...
- LocalPlayer、Input Mapping Context、InputAction参照を検証する。
...
4. Enhanced InputのActionを共通Input Tagへ変換して同一処理へ配送する。
5. 共通Input Tagを反映する。
...
|---|---|---|---|
| `Input.*` | 要件の状態・受付・通知 | 処理開始または条件成立時 | 完了・中断・条件解除時 |
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 実行入力 | InputAction / Gameplay Event / System Event | Yes | ゲームパッドまたはキーボード・マウス入力 |
| 状態・対象参照 | Gameplay Tag / Attribute / Weak Object Reference | Yes | LocalPlayer、Input Mapping Context、InputAction参照を検証する |
| 設定データ | Data Asset / Config | Yes | Input Mapping Context |

### 10.2 出力

| 出力項目 | 型・形式 | 説明 |
|---|---|---|
| 実行結果 | State / Attribute / Result Event | 共通Input Tag |
| 通知 | Delegate / Gameplay Event | 完了・失敗・中断を必要な連携先へ通知する |
| フィードバック | UI / Animation / VFX / SE | 操作デバイス変更時に対応するUI表記へ切り替える |
...
| クラス・システム | 責務 | 実装区分 |
|---|---|---|
| `AActionPlayerController / Enhanced Input` | 実行可否、状態遷移、主要ロジック、重複防止 | `C++` |
| `Primary Data Asset` | Input Mapping ContextとAsset参照 | `Data Asset / Config` |
...
void RequestRouteInput();
void CancelRouteInput();
bool IsRouteInputActive() const;
```

### 12.2 通知イベント

```cpp
DECLARE_MULTICAST_DELEGATE(FOnRouteInputStarted);
DECLARE_MULTICAST_DELEGATE(FOnRouteInputCompleted);
DECLARE_MULTICAST_DELEGATE(FOnRouteInputCancelled);
DECLARE_MULTICAST_DELEGATE_OneParam(FOnRouteInputFailed, EFailureReason);
```
...
[Player]
        |
        v
[AActionPlayerController / Enhanced Input]
   |          |
   v          v
...
|---|---|---:|---|---|
| Input Mapping Context | Data Asset / Struct / Config | TBD | Primary Data Asset | 要件固有の判定・調整データ |
...
- [ ] 必須操作が両デバイスに割り当てられ、同じ実行結果になる。
- [ ] 成功通知が1回発行され、一時状態が完了時に解除される。
- [ ] 操作デバイス変更時に対応するUI表記へ切り替える。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-FR-PLAYER-002-001` | 正常系 | すべての事前条件を満たす | ゲームパッドまたはキーボード・マウス入力を発生させる | 必須操作が両デバイスに割り当てられ、同じ実行結果になる |
| `TC-FR-PLAYER-002-002` | 異常系 | 必須参照または実行条件を1つ無効にする | 同じ要求を発生させる | 状態とリソースを変更せず安全に拒否する |
| `TC-FR-PLAYER-002-003` | 多重・中断 | 処理中または連続要求 | 再要求、死亡、Owner破棄のいずれかを発生させる | 重複せず一時状態を解除しCrashしない |
...
| ID | 内容 | 影響範囲 | 決定期限 | 状態 |
|---|---|---|---|---|
| `Q-FR-PLAYER-002` | 最終キーアサインと再バインド対応範囲はTBD | 設計・データ・テスト | 実装Issue開始前 | Open |
...
|---|---|---|---|---|
| `ADR-FR-PLAYER-002-001` | 責務配置 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | Blueprintだけにルールと状態を保持する | 状態整合性、検証容易性、データ調整性を保つため |
...
|---:|---|---|---|
| 1 | `FR-PLAYER-002: データ・状態・公開Interface` | 必須データ、状態、Validation、APIが定義される | なし |
| 2 | `FR-PLAYER-002: 主要ロジック` | 正常・拒否・中断・多重実行テストが通る | #1 |
| 3 | `FR-PLAYER-002: Asset・UI・結合確認` | 必須Assetを設定し受入条件を確認できる | #2 |
