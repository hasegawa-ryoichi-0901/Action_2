...
|---|---|
| 要件ID | `FR-PLAYER-018` |
| 要件名 | 回数制限付き回復アイテムを使用できる |
| 要件種別 | 機能要件 |
| 要件カテゴリ | プレイヤー |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「回数制限付き回復アイテムを使用できる」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
...
- 元要件「回数制限付き回復アイテムを使用できる」を実現する。
- 使用開始後にMontageを完了した場合だけ回数を1減らしHealthを回復する。被弾時は中断する。
...
| `UGA_Heal / CharacterAttributeSet` | 実行可否判定、状態変更、主要処理を担当する |
...
- 生存、所持数1以上、回復禁止Tagなし、Heal Ability未実行を検証する。
...
- HealthとHealingItemCount変更が要件どおり反映される。
...
4. 使用開始後にMontageを完了した場合だけ回数を1減らしHealthを回復する。被弾時は中断する。
5. HealthとHealingItemCount変更を反映する。
...
|---|---|---|---|
| `State.Action.Healing` | 要件の状態・受付・通知 | 処理開始または条件成立時 | 完了・中断・条件解除時 |
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 実行入力 | InputAction / Gameplay Event / System Event | Yes | Input.Heal |
| 状態・対象参照 | Gameplay Tag / Attribute / Weak Object Reference | Yes | 生存、所持数1以上、回復禁止Tagなし、Heal Ability未実行を検証する |
| 設定データ | Data Asset / Config | Yes | 回復量・所持上限・使用時間 |

### 10.2 出力

| 出力項目 | 型・形式 | 説明 |
|---|---|---|
| 実行結果 | State / Attribute / Result Event | HealthとHealingItemCount変更 |
| 通知 | Delegate / Gameplay Event | 完了・失敗・中断を必要な連携先へ通知する |
| フィードバック | UI / Animation / VFX / SE | 回復Montage、VFX、SE、HUD個数を更新する |
...
| クラス・システム | 責務 | 実装区分 |
|---|---|---|
| `UGA_Heal / CharacterAttributeSet` | 実行可否、状態遷移、主要ロジック、重複防止 | `C++` |
| `Primary Data Asset` | 回復量・所持上限・使用時間とAsset参照 | `Data Asset / Config` |
...
void RequestHeal();
void CancelHeal();
bool IsHealActive() const;
```

### 12.2 通知イベント

```cpp
DECLARE_MULTICAST_DELEGATE(FOnHealStarted);
DECLARE_MULTICAST_DELEGATE(FOnHealCompleted);
DECLARE_MULTICAST_DELEGATE(FOnHealCancelled);
DECLARE_MULTICAST_DELEGATE_OneParam(FOnHealFailed, EFailureReason);
```
...
[Player]
        |
        v
[UGA_Heal / CharacterAttributeSet]
   |          |
   v          v
...
|---|---|---:|---|---|
| 回復量・所持上限・使用時間 | Data Asset / Struct / Config | TBD | Primary Data Asset | 要件固有の判定・調整データ |
...
| 種別 | 内容 |
|---|---|
| UI | 回復Montage、VFX、SE、HUD個数を更新する |
...
- [ ] 所持数があるとき1回使用でき、完了時にのみ1個消費して定義量を回復する。
- [ ] 成功通知が1回発行され、一時状態が完了時に解除される。
- [ ] 回復Montage、VFX、SE、HUD個数を更新する。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-FR-PLAYER-018-001` | 正常系 | すべての事前条件を満たす | Input.Healを発生させる | 所持数があるとき1回使用でき、完了時にのみ1個消費して定義量を回復する |
| `TC-FR-PLAYER-018-002` | 異常系 | 必須参照または実行条件を1つ無効にする | 同じ要求を発生させる | 状態とリソースを変更せず安全に拒否する |
| `TC-FR-PLAYER-018-003` | 多重・中断 | 処理中または連続要求 | 再要求、死亡、Owner破棄のいずれかを発生させる | 重複せず一時状態を解除しCrashしない |
...
| ID | 内容 | 影響範囲 | 決定期限 | 状態 |
|---|---|---|---|---|
| `Q-FR-PLAYER-018` | 所持数、回復量、回復時間はTBD | 設計・データ・テスト | 実装Issue開始前 | Open |
...
|---|---|---|---|---|
| `ADR-FR-PLAYER-018-001` | 責務配置 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | Blueprintだけにルールと状態を保持する | 状態整合性、検証容易性、データ調整性を保つため |
...
|---:|---|---|---|
| 1 | `FR-PLAYER-018: データ・状態・公開Interface` | 必須データ、状態、Validation、APIが定義される | なし |
| 2 | `FR-PLAYER-018: 主要ロジック` | 正常・拒否・中断・多重実行テストが通る | #1 |
| 3 | `FR-PLAYER-018: Asset・UI・結合確認` | 必須Assetを設定し受入条件を確認できる | #2 |
