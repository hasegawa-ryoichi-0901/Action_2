...
| 要件種別 | 機能要件 |
| 要件カテゴリ | プレイヤー |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「ロックオン対象を切り替えられる」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
...
- 元要件「ロックオン対象を切り替えられる」を実現する。
- 画面上の左右方向と現在対象を基準に次候補を選択する。
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 実行入力 | InputAction / Gameplay Event / System Event | Yes | 切替方向、現在対象、敵候補 |
| 状態・対象参照 | Gameplay Tag / Attribute / Weak Object Reference | Yes | 手動ロック中で切替方向に有効候補が存在することを検証する |
| 設定データ | Data Asset / Config | Yes | 切替デッドゾーン・再入力待機 |

### 10.2 出力

| 出力項目 | 型・形式 | 説明 |
|---|---|---|
| 実行結果 | State / Attribute / Result Event | 新しい固定ターゲット参照 |
| 通知 | Delegate / Gameplay Event | 完了・失敗・中断を必要な連携先へ通知する |
| フィードバック | UI / Animation / VFX / SE | ロックオンマーカーを新対象へ移動する |
...
| `Primary Data Asset` | 切替デッドゾーン・再入力待機とAsset参照 | `Data Asset / Config` |
...
void RequestSwitchLockTarget();
void CancelSwitchLockTarget();
bool IsSwitchLockTargetActive() const;
```

### 12.2 通知イベント

```cpp
DECLARE_MULTICAST_DELEGATE(FOnSwitchLockTargetStarted);
DECLARE_MULTICAST_DELEGATE(FOnSwitchLockTargetCompleted);
DECLARE_MULTICAST_DELEGATE(FOnSwitchLockTargetCancelled);
DECLARE_MULTICAST_DELEGATE_OneParam(FOnSwitchLockTargetFailed, EFailureReason);
```
...
|---|---|---:|---|---|
| 切替デッドゾーン・再入力待機 | Data Asset / Struct / Config | TBD | Primary Data Asset | 要件固有の判定・調整データ |
...
- [ ] 切替入力1回につき指定方向の妥当な別対象へ1回だけ切り替わる。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-FR-PLAYER-007-001` | 正常系 | すべての事前条件を満たす | 切替方向、現在対象、敵候補を発生させる | 切替入力1回につき指定方向の妥当な別対象へ1回だけ切り替わる |
| `TC-FR-PLAYER-007-002` | 異常系 | 必須参照または実行条件を1つ無効にする | 同じ要求を発生させる | 状態とリソースを変更せず安全に拒否する |
| `TC-FR-PLAYER-007-003` | 多重・中断 | 処理中または連続要求 | 再要求、死亡、Owner破棄のいずれかを発生させる | 重複せず一時状態を解除しCrashしない |
...
| ID | 内容 | 影響範囲 | 決定期限 | 状態 |
|---|---|---|---|---|
| `Q-FR-PLAYER-007` | 候補スコアと入力デッドゾーンはTBD | 設計・データ・テスト | 実装Issue開始前 | Open |
...
|---|---|---|---|---|
| `ADR-FR-PLAYER-007-001` | 責務配置 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | Blueprintだけにルールと状態を保持する | 状態整合性、検証容易性、データ調整性を保つため |
...
|---:|---|---|---|
| 1 | `FR-PLAYER-007: データ・状態・公開Interface` | 必須データ、状態、Validation、APIが定義される | なし |
| 2 | `FR-PLAYER-007: 主要ロジック` | 正常・拒否・中断・多重実行テストが通る | #1 |
| 3 | `FR-PLAYER-007: Asset・UI・結合確認` | 必須Assetを設定し受入条件を確認できる | #2 |
