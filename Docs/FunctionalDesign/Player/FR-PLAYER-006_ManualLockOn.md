...
| 要件種別 | 機能要件 |
| 要件カテゴリ | プレイヤー |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「手動ロックオンを使用できる」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
...
注視対象を固定し、1対1や集団戦で位置関係を把握しやすくする。
...
- 元要件「手動ロックオンを使用できる」を実現する。
- 入力で対象を選択し、カメラとFacingへ継続的な注視要求を送る。
...
| `UTargetingComponent / AActionPlayerController` | 実行可否判定、状態変更、主要処理を担当する |
...
4. 入力で対象を選択し、カメラとFacingへ継続的な注視要求を送る。
5. 固定ターゲット参照とロック状態を反映する。
...
|---|---|---|---|
| `State.Targeting.Locked` | 要件の状態・受付・通知 | 処理開始または条件成立時 | 完了・中断・条件解除時 |
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 実行入力 | InputAction / Gameplay Event / System Event | Yes | Input.LockOn、敵候補 |
| 状態・対象参照 | Gameplay Tag / Attribute / Weak Object Reference | Yes | 対象が生存中・範囲内・遮蔽許容時間内であることを検証する |
| 設定データ | Data Asset / Config | Yes | ロックオン最大距離・遮蔽解除時間 |

### 10.2 出力

| 出力項目 | 型・形式 | 説明 |
|---|---|---|
| 実行結果 | State / Attribute / Result Event | 固定ターゲット参照とロック状態 |
| 通知 | Delegate / Gameplay Event | 完了・失敗・中断を必要な連携先へ通知する |
| フィードバック | UI / Animation / VFX / SE | 対象マーカーとロック状態をHUD表示する |
...
| クラス・システム | 責務 | 実装区分 |
|---|---|---|
| `UTargetingComponent / AActionPlayerController` | 実行可否、状態遷移、主要ロジック、重複防止 | `C++` |
| `Primary Data Asset` | ロックオン最大距離・遮蔽解除時間とAsset参照 | `Data Asset / Config` |
...
```cpp
DECLARE_MULTICAST_DELEGATE(FOnLockOnStarted);
DECLARE_MULTICAST_DELEGATE(FOnLockOnCompleted);
DECLARE_MULTICAST_DELEGATE(FOnLockOnCancelled);
DECLARE_MULTICAST_DELEGATE_OneParam(FOnLockOnFailed, EFailureReason);
```
...
[Player]
        |
        v
[UTargetingComponent / AActionPlayerController]
   |          |
   v          v
...
|---|---|---:|---|---|
| ロックオン最大距離・遮蔽解除時間 | Data Asset / Struct / Config | TBD | Primary Data Asset | 要件固有の判定・調整データ |
...
- [ ] 有効候補にロックし、再入力または解除条件でロックを解除する。
- [ ] 成功通知が1回発行され、一時状態が完了時に解除される。
- [ ] 対象マーカーとロック状態をHUD表示する。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-FR-PLAYER-006-001` | 正常系 | すべての事前条件を満たす | Input.LockOn、敵候補を発生させる | 有効候補にロックし、再入力または解除条件でロックを解除する |
| `TC-FR-PLAYER-006-002` | 異常系 | 必須参照または実行条件を1つ無効にする | 同じ要求を発生させる | 状態とリソースを変更せず安全に拒否する |
| `TC-FR-PLAYER-006-003` | 多重・中断 | 処理中または連続要求 | 再要求、死亡、Owner破棄のいずれかを発生させる | 重複せず一時状態を解除しCrashしない |
...
| ID | 内容 | 影響範囲 | 決定期限 | 状態 |
|---|---|---|---|---|
| `Q-FR-PLAYER-006` | 最大距離と遮蔽による解除時間はTBD | 設計・データ・テスト | 実装Issue開始前 | Open |
...
|---|---|---|---|---|
| `ADR-FR-PLAYER-006-001` | 責務配置 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | Blueprintだけにルールと状態を保持する | 状態整合性、検証容易性、データ調整性を保つため |
...
|---:|---|---|---|
| 1 | `FR-PLAYER-006: データ・状態・公開Interface` | 必須データ、状態、Validation、APIが定義される | なし |
| 2 | `FR-PLAYER-006: 主要ロジック` | 正常・拒否・中断・多重実行テストが通る | #1 |
| 3 | `FR-PLAYER-006: Asset・UI・結合確認` | 必須Assetを設定し受入条件を確認できる | #2 |
