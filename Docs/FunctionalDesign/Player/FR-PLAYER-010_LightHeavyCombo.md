...
|---|---|
| 要件ID | `FR-PLAYER-010` |
| 要件名 | 通常攻撃と強攻撃を組み合わせられる |
| 要件種別 | 機能要件 |
| 要件カテゴリ | プレイヤー |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「通常攻撃と強攻撃を組み合わせられる」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
...
- 元要件「通常攻撃と強攻撃を組み合わせられる」を実現する。
- 入力バッファとComboDefinitionから通常・強攻撃の有効な派生先を選ぶ。
...
| `UGA_AttackBase / UInputBufferComponent` | 実行可否判定、状態変更、主要処理を担当する |
...
- 武器、スタミナ、Combo受付Window、禁止Tagを検証する。
...
- 次のAttackDefinitionとコンボ段数が要件どおり反映される。
...
[開始条件成立]
      |
      v
[Input.Attack.Light / Heavyを受け付ける]
      |
      v
[武器、スタミナ、Combo受付Window、禁止Tagを検証する]
      |
...
4. 入力バッファとComboDefinitionから通常・強攻撃の有効な派生先を選ぶ。
5. 次のAttackDefinitionとコンボ段数を反映する。
...
|---|---|---|---|
| `Window.Combo.AcceptInput` | 要件の状態・受付・通知 | 処理開始または条件成立時 | 完了・中断・条件解除時 |
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 実行入力 | InputAction / Gameplay Event / System Event | Yes | Input.Attack.Light / Heavy |
| 状態・対象参照 | Gameplay Tag / Attribute / Weak Object Reference | Yes | 武器、スタミナ、Combo受付Window、禁止Tagを検証する |
| 設定データ | Data Asset / Config | Yes | ComboDefinition / AttackDefinition |

### 10.2 出力

| 出力項目 | 型・形式 | 説明 |
|---|---|---|
| 実行結果 | State / Attribute / Result Event | 次のAttackDefinitionとコンボ段数 |
| 通知 | Delegate / Gameplay Event | 完了・失敗・中断を必要な連携先へ通知する |
| フィードバック | UI / Animation / VFX / SE | 武器別Montage、ヒットVFX・SE、入力受付確認を出す |
...
| クラス・システム | 責務 | 実装区分 |
|---|---|---|
| `UGA_AttackBase / UInputBufferComponent` | 実行可否、状態遷移、主要ロジック、重複防止 | `C++` |
| `Primary Data Asset` | ComboDefinition / AttackDefinitionとAsset参照 | `Data Asset / Config` |
...
- ComboDefinition / AttackDefinitionを保持する。
...
bool CanAttackCombo() const;
void RequestAttackCombo();
void CancelAttackCombo();
bool IsAttackComboActive() const;
```

### 12.2 通知イベント

```cpp
DECLARE_MULTICAST_DELEGATE(FOnAttackComboStarted);
DECLARE_MULTICAST_DELEGATE(FOnAttackComboCompleted);
DECLARE_MULTICAST_DELEGATE(FOnAttackComboCancelled);
DECLARE_MULTICAST_DELEGATE_OneParam(FOnAttackComboFailed, EFailureReason);
```
...
[Player]
        |
        v
[UGA_AttackBase / UInputBufferComponent]
   |          |
   v          v
...
|---|---|---:|---|---|
| ComboDefinition / AttackDefinition | Data Asset / Struct / Config | Data Asset定義値 | Primary Data Asset | 要件固有の判定・調整データ |
...
| 種別 | 内容 |
|---|---|
| UI | 武器別Montage、ヒットVFX・SE、入力受付確認を出す |
...
- [ ] 定義済み受付時間内の通常・強入力で有効な派生へ進み、時間外は新規攻撃になる。
- [ ] 成功通知が1回発行され、一時状態が完了時に解除される。
- [ ] 武器別Montage、ヒットVFX・SE、入力受付確認を出す。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-FR-PLAYER-010-001` | 正常系 | すべての事前条件を満たす | Input.Attack.Light / Heavyを発生させる | 定義済み受付時間内の通常・強入力で有効な派生へ進み、時間外は新規攻撃になる |
| `TC-FR-PLAYER-010-002` | 異常系 | 必須参照または実行条件を1つ無効にする | 同じ要求を発生させる | 状態とリソースを変更せず安全に拒否する |
| `TC-FR-PLAYER-010-003` | 多重・中断 | 処理中または連続要求 | 再要求、死亡、Owner破棄のいずれかを発生させる | 重複せず一時状態を解除しCrashしない |
...
|---|---|---|---|---|
| `ADR-FR-PLAYER-010-001` | 責務配置 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | Blueprintだけにルールと状態を保持する | 状態整合性、検証容易性、データ調整性を保つため |
...
|---:|---|---|---|
| 1 | `FR-PLAYER-010: データ・状態・公開Interface` | 必須データ、状態、Validation、APIが定義される | なし |
| 2 | `FR-PLAYER-010: 主要ロジック` | 正常・拒否・中断・多重実行テストが通る | #1 |
| 3 | `FR-PLAYER-010: Asset・UI・結合確認` | 必須Assetを設定し受入条件を確認できる | #2 |
