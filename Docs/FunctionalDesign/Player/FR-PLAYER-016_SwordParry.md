...
| 要件種別 | 機能要件 |
| 要件カテゴリ | プレイヤー |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「剣でパリィできる」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
...
- 元要件「剣でパリィできる」を実現する。
- 成功時に攻撃を無効化して敵Postureを大きく削り、失敗時は専用硬直へ遷移する。
...
| `UGA_Parry` | 実行可否判定、状態変更、主要処理を担当する |
...
- 剣装備、スタミナ、Window.Parry.Active、攻撃のParryable Tagを検証する。
...
- Parry結果とPosture Damageが要件どおり反映される。
...
[開始条件成立]
      |
      v
[Input.Parry、敵Attack Eventを受け付ける]
      |
      v
[剣装備、スタミナ、Window.Parry.Active、攻撃のParryable Tagを検証する]
      |
...
4. 成功時に攻撃を無効化して敵Postureを大きく削り、失敗時は専用硬直へ遷移する。
5. Parry結果とPosture Damageを反映する。
...
|---|---|---|---|
| `Window.Parry.Active` | 要件の状態・受付・通知 | 処理開始または条件成立時 | 完了・中断・条件解除時 |
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 実行入力 | InputAction / Gameplay Event / System Event | Yes | Input.Parry、敵Attack Event |
| 状態・対象参照 | Gameplay Tag / Attribute / Weak Object Reference | Yes | 剣装備、スタミナ、Window.Parry.Active、攻撃のParryable Tagを検証する |
| 設定データ | Data Asset / Config | Yes | ParryWindow / FailureRecovery / PostureDamage |

### 10.2 出力

| 出力項目 | 型・形式 | 説明 |
|---|---|---|
| 実行結果 | State / Attribute / Result Event | Parry結果とPosture Damage |
| 通知 | Delegate / Gameplay Event | 完了・失敗・中断を必要な連携先へ通知する |
| フィードバック | UI / Animation / VFX / SE | 成功・失敗Montage、VFX、SE、Hit Stopを区別する |
...
| クラス・システム | 責務 | 実装区分 |
|---|---|---|
| `UGA_Parry` | 実行可否、状態遷移、主要ロジック、重複防止 | `C++` |
| `Primary Data Asset` | ParryWindow / FailureRecovery / PostureDamageとAsset参照 | `Data Asset / Config` |
...
- ParryWindow / FailureRecovery / PostureDamageを保持する。
...
```cpp
DECLARE_MULTICAST_DELEGATE(FOnParryStarted);
DECLARE_MULTICAST_DELEGATE(FOnParryCompleted);
DECLARE_MULTICAST_DELEGATE(FOnParryCancelled);
DECLARE_MULTICAST_DELEGATE_OneParam(FOnParryFailed, EFailureReason);
```
...
|---|---|---:|---|---|
| ParryWindow / FailureRecovery / PostureDamage | Data Asset / Struct / Config | TBD | Primary Data Asset | 要件固有の判定・調整データ |
...
| 種別 | 内容 |
|---|---|
| UI | 成功・失敗Montage、VFX、SE、Hit Stopを区別する |
...
- [ ] Parryable攻撃と受付時間が重なった場合のみ成功し、Unparryable攻撃は成功しない。
- [ ] 成功通知が1回発行され、一時状態が完了時に解除される。
- [ ] 成功・失敗Montage、VFX、SE、Hit Stopを区別する。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-FR-PLAYER-016-001` | 正常系 | すべての事前条件を満たす | Input.Parry、敵Attack Eventを発生させる | Parryable攻撃と受付時間が重なった場合のみ成功し、Unparryable攻撃は成功しない |
| `TC-FR-PLAYER-016-002` | 異常系 | 必須参照または実行条件を1つ無効にする | 同じ要求を発生させる | 状態とリソースを変更せず安全に拒否する |
| `TC-FR-PLAYER-016-003` | 多重・中断 | 処理中または連続要求 | 再要求、死亡、Owner破棄のいずれかを発生させる | 重複せず一時状態を解除しCrashしない |
...
| ID | 内容 | 影響範囲 | 決定期限 | 状態 |
|---|---|---|---|---|
| `Q-FR-PLAYER-016` | 受付時間と失敗硬直はTBD | 設計・データ・テスト | 実装Issue開始前 | Open |
...
|---|---|---|---|---|
| `ADR-FR-PLAYER-016-001` | 責務配置 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | Blueprintだけにルールと状態を保持する | 状態整合性、検証容易性、データ調整性を保つため |
...
|---:|---|---|---|
| 1 | `FR-PLAYER-016: データ・状態・公開Interface` | 必須データ、状態、Validation、APIが定義される | なし |
| 2 | `FR-PLAYER-016: 主要ロジック` | 正常・拒否・中断・多重実行テストが通る | #1 |
| 3 | `FR-PLAYER-016: Asset・UI・結合確認` | 必須Assetを設定し受入条件を確認できる | #2 |
