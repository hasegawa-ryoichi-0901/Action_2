...
| 要件種別 | 機能要件 |
| 要件カテゴリ | プレイヤー |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「空中攻撃を実行できる」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
...
- 元要件「空中攻撃を実行できる」を実現する。
- 装備武器のAirAttackDefinitionからUGA_AirAttackを起動する。
- 成功・拒否・中断時の状態整合性と通知までを扱う。

### 3.2 対象外

- 空中コンボ（Vertical Slice対象外）。
...
| `UGA_AirAttack / UCombatComponent` | 実行可否判定、状態変更、主要処理を担当する |
...
- 攻撃判定、ダメージ、着地または終了状態が要件どおり反映される。
...
4. 装備武器のAirAttackDefinitionからUGA_AirAttackを起動する。
5. 攻撃判定、ダメージ、着地または終了状態を反映する。
...
|---|---|---|---|
| `State.Action.Attacking` | 要件の状態・受付・通知 | 処理開始または条件成立時 | 完了・中断・条件解除時 |
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 実行入力 | InputAction / Gameplay Event / System Event | Yes | Input.Attackと空中状態 |
| 状態・対象参照 | Gameplay Tag / Attribute / Weak Object Reference | Yes | 非接地、武器装備、スタミナ、攻撃禁止Tagを検証する |
| 設定データ | Data Asset / Config | Yes | AirAttackDefinition |

### 10.2 出力

| 出力項目 | 型・形式 | 説明 |
|---|---|---|
| 実行結果 | State / Attribute / Result Event | 攻撃判定、ダメージ、着地または終了状態 |
| 通知 | Delegate / Gameplay Event | 完了・失敗・中断を必要な連携先へ通知する |
| フィードバック | UI / Animation / VFX / SE | 武器別Montage、VFX、SE、ヒット反応を再生する |
...
| クラス・システム | 責務 | 実装区分 |
|---|---|---|
| `UGA_AirAttack / UCombatComponent` | 実行可否、状態遷移、主要ロジック、重複防止 | `C++` |
| `Primary Data Asset` | AirAttackDefinitionとAsset参照 | `Data Asset / Config` |
...
bool IsAirAttackActive() const;
```

### 12.2 通知イベント

```cpp
DECLARE_MULTICAST_DELEGATE(FOnAirAttackStarted);
DECLARE_MULTICAST_DELEGATE(FOnAirAttackCompleted);
DECLARE_MULTICAST_DELEGATE(FOnAirAttackCancelled);
DECLARE_MULTICAST_DELEGATE_OneParam(FOnAirAttackFailed, EFailureReason);
```
...
[Player]
        |
        v
[UGA_AirAttack / UCombatComponent]
   |          |
   v          v
...
|---|---|---:|---|---|
| AirAttackDefinition | Data Asset / Struct / Config | Data Asset定義値 | Primary Data Asset | 要件固有の判定・調整データ |
...
| 種別 | 内容 |
|---|---|
| UI | 武器別Montage、VFX、SE、ヒット反応を再生する |
...
- [ ] 空中で攻撃入力すると装備武器の空中攻撃が1回発動する。
- [ ] 成功通知が1回発行され、一時状態が完了時に解除される。
- [ ] 武器別Montage、VFX、SE、ヒット反応を再生する。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-FR-PLAYER-004-001` | 正常系 | すべての事前条件を満たす | Input.Attackと空中状態を発生させる | 空中で攻撃入力すると装備武器の空中攻撃が1回発動する |
| `TC-FR-PLAYER-004-002` | 異常系 | 必須参照または実行条件を1つ無効にする | 同じ要求を発生させる | 状態とリソースを変更せず安全に拒否する |
| `TC-FR-PLAYER-004-003` | 多重・中断 | 処理中または連続要求 | 再要求、死亡、Owner破棄のいずれかを発生させる | 重複せず一時状態を解除しCrashしない |
...
|---|---|---|---|---|
| `ADR-FR-PLAYER-004-001` | 責務配置 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | Blueprintだけにルールと状態を保持する | 状態整合性、検証容易性、データ調整性を保つため |
...
|---:|---|---|---|
| 1 | `FR-PLAYER-004: データ・状態・公開Interface` | 必須データ、状態、Validation、APIが定義される | なし |
| 2 | `FR-PLAYER-004: 主要ロジック` | 正常・拒否・中断・多重実行テストが通る | #1 |
| 3 | `FR-PLAYER-004: Asset・UI・結合確認` | 必須Assetを設定し受入条件を確認できる | #2 |
