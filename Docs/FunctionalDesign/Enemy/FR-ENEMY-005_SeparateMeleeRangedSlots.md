...
|---|---|
| 要件ID | `FR-ENEMY-005` |
| 要件名 | 近接攻撃枠と遠距離攻撃枠を分離する |
| 要件種別 | 機能要件 |
| 要件カテゴリ | 敵 |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「近接攻撃枠と遠距離攻撃枠を分離する」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
...
近接と遠距離の攻撃頻度を独立調整し、片方が他方を不必要に停止しないようにする。
...
- 元要件「近接攻撃枠と遠距離攻撃枠を分離する」を実現する。
- CoordinatorがMeleeとRangedの別カウンタ・待機列・解放Tokenを管理する。
...
4. CoordinatorがMeleeとRangedの別カウンタ・待機列・解放Tokenを管理する。
5. 種別ごとの許可・拒否を反映する。
...
|---|---|---|---|
| `State.AI.AttackSlot.*` | 要件の状態・受付・通知 | 処理開始または条件成立時 | 完了・中断・条件解除時 |
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 実行入力 | InputAction / Gameplay Event / System Event | Yes | 敵ID、Melee/Ranged種別 |
| 状態・対象参照 | Gameplay Tag / Attribute / Weak Object Reference | Yes | 攻撃定義のSlotPolicyと要求種別を検証する |
| 設定データ | Data Asset / Config | Yes | MeleeSlotPolicy / RangedSlotPolicy |

### 10.2 出力

| 出力項目 | 型・形式 | 説明 |
|---|---|---|
| 実行結果 | State / Attribute / Result Event | 種別ごとの許可・拒否 |
| 通知 | Delegate / Gameplay Event | 完了・失敗・中断を必要な連携先へ通知する |
| フィードバック | UI / Animation / VFX / SE | デバッグ表示で枠種別を色分けする |
...
| `EnemyDefinition` | MeleeSlotPolicy / RangedSlotPolicyとAsset参照 | `Data Asset / Config` |
...
- MeleeSlotPolicy / RangedSlotPolicyを保持する。
...
bool CanRequestAttackSlot() const;
void RequestRequestAttackSlot();
void CancelRequestAttackSlot();
bool IsRequestAttackSlotActive() const;
```

### 12.2 通知イベント

```cpp
DECLARE_MULTICAST_DELEGATE(FOnRequestAttackSlotStarted);
DECLARE_MULTICAST_DELEGATE(FOnRequestAttackSlotCompleted);
DECLARE_MULTICAST_DELEGATE(FOnRequestAttackSlotCancelled);
DECLARE_MULTICAST_DELEGATE_OneParam(FOnRequestAttackSlotFailed, EFailureReason);
```
...
|---|---|---:|---|---|
| MeleeSlotPolicy / RangedSlotPolicy | Data Asset / Struct / Config | TBD | EnemyDefinition | 要件固有の判定・調整データ |
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-FR-ENEMY-005-001` | 正常系 | すべての事前条件を満たす | 敵ID、Melee/Ranged種別を発生させる | 近接枠が満杯でも空き遠距離枠を取得でき、逆方向も独立する |
| `TC-FR-ENEMY-005-002` | 異常系 | 必須参照または実行条件を1つ無効にする | 同じ要求を発生させる | 状態とリソースを変更せず安全に拒否する |
| `TC-FR-ENEMY-005-003` | 多重・中断 | 処理中または連続要求 | 再要求、死亡、Owner破棄のいずれかを発生させる | 重複せず一時状態を解除しCrashしない |
...
| ID | 内容 | 影響範囲 | 決定期限 | 状態 |
|---|---|---|---|---|
| `Q-FR-ENEMY-005` | 遠距離攻撃枠の最大数はTBD | 設計・データ・テスト | 実装Issue開始前 | Open |
...
|---|---|---|---|---|
| `ADR-FR-ENEMY-005-001` | 責務配置 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | Blueprintだけにルールと状態を保持する | 状態整合性、検証容易性、データ調整性を保つため |
...
|---:|---|---|---|
| 1 | `FR-ENEMY-005: データ・状態・公開Interface` | 必須データ、状態、Validation、APIが定義される | なし |
| 2 | `FR-ENEMY-005: 主要ロジック` | 正常・拒否・中断・多重実行テストが通る | #1 |
| 3 | `FR-ENEMY-005: Asset・UI・結合確認` | 必須Assetを設定し受入条件を確認できる | #2 |
