...
| 要件種別 | 機能要件 |
| 要件カテゴリ | 敵 |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「近接通常敵を実装する」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
...
- 元要件「近接通常敵を実装する」を実現する。
- 索敵・接近・攻撃枠取得・近接Ability実行・枠解放をBehavior Treeで制御する。
...
| `AEnemyAIController / Behavior Tree` | 実行可否判定、状態変更、主要処理を担当する |
...
- AIController、Behavior Tree、EnemyDefinition、Targetが有効か検証する。
...
4. 索敵・接近・攻撃枠取得・近接Ability実行・枠解放をBehavior Treeで制御する。
5. 移動・近接攻撃・待機状態を反映する。
...
|---|---|---|---|
| `State.AI.Combat` | 要件の状態・受付・通知 | 処理開始または条件成立時 | 完了・中断・条件解除時 |
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 実行入力 | InputAction / Gameplay Event / System Event | Yes | 知覚Target、近接攻撃定義 |
| 状態・対象参照 | Gameplay Tag / Attribute / Weak Object Reference | Yes | AIController、Behavior Tree、EnemyDefinition、Targetが有効か検証する |
| 設定データ | Data Asset / Config | Yes | FEnemyDefinition / 近接AttackDefinitions |

### 10.2 出力

| 出力項目 | 型・形式 | 説明 |
|---|---|---|
| 実行結果 | State / Attribute / Result Event | 移動・近接攻撃・待機状態 |
| 通知 | Delegate / Gameplay Event | 完了・失敗・中断を必要な連携先へ通知する |
| フィードバック | UI / Animation / VFX / SE | 予備動作、攻撃、被弾、死亡をAnimation・VFX・SEで示す |
...
| クラス・システム | 責務 | 実装区分 |
|---|---|---|
| `AEnemyAIController / Behavior Tree` | 実行可否、状態遷移、主要ロジック、重複防止 | `C++` |
| `EnemyDefinition` | FEnemyDefinition / 近接AttackDefinitionsとAsset参照 | `Data Asset / Config` |
...
- FEnemyDefinition / 近接AttackDefinitionsを保持する。
...
bool CanRunMeleeEnemy() const;
void RequestRunMeleeEnemy();
void CancelRunMeleeEnemy();
bool IsRunMeleeEnemyActive() const;
```

### 12.2 通知イベント

```cpp
DECLARE_MULTICAST_DELEGATE(FOnRunMeleeEnemyStarted);
DECLARE_MULTICAST_DELEGATE(FOnRunMeleeEnemyCompleted);
DECLARE_MULTICAST_DELEGATE(FOnRunMeleeEnemyCancelled);
DECLARE_MULTICAST_DELEGATE_OneParam(FOnRunMeleeEnemyFailed, EFailureReason);
```
...
[AEnemyAIController]
        |
        v
[AEnemyAIController / Behavior Tree]
   |          |
   v          v
...
|---|---|---:|---|---|
| FEnemyDefinition / 近接AttackDefinitions | Data Asset / Struct / Config | Data Asset定義値 | EnemyDefinition | 要件固有の判定・調整データ |
...
| 種別 | 内容 |
|---|---|
| UI | 予備動作、攻撃、被弾、死亡をAnimation・VFX・SEで示す |
...
- [ ] Target発見後に接近し、近接枠取得時だけ有効距離から攻撃する。
- [ ] 成功通知が1回発行され、一時状態が完了時に解除される。
- [ ] 予備動作、攻撃、被弾、死亡をAnimation・VFX・SEで示す。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-FR-ENEMY-001-001` | 正常系 | すべての事前条件を満たす | 知覚Target、近接攻撃定義を発生させる | Target発見後に接近し、近接枠取得時だけ有効距離から攻撃する |
| `TC-FR-ENEMY-001-002` | 異常系 | 必須参照または実行条件を1つ無効にする | 同じ要求を発生させる | 状態とリソースを変更せず安全に拒否する |
| `TC-FR-ENEMY-001-003` | 多重・中断 | 処理中または連続要求 | 再要求、死亡、Owner破棄のいずれかを発生させる | 重複せず一時状態を解除しCrashしない |
...
|---|---|---|---|---|
| `ADR-FR-ENEMY-001-001` | 責務配置 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | Blueprintだけにルールと状態を保持する | 状態整合性、検証容易性、データ調整性を保つため |
...
|---:|---|---|---|
| 1 | `FR-ENEMY-001: データ・状態・公開Interface` | 必須データ、状態、Validation、APIが定義される | なし |
| 2 | `FR-ENEMY-001: 主要ロジック` | 正常・拒否・中断・多重実行テストが通る | #1 |
| 3 | `FR-ENEMY-001: Asset・UI・結合確認` | 必須Assetを設定し受入条件を確認できる | #2 |
