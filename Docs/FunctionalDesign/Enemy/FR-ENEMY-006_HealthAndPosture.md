...
| 要件種別 | 機能要件 |
| 要件カテゴリ | 敵 |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「HPと体勢値を持つ」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
...
- 元要件「HPと体勢値を持つ」を実現する。
- EnemyDefinitionからMaxHealth・MaxPostureを初期化し、Gameplay Effectで変更する。
...
| `AEnemyCharacter / AttributeSet` | 実行可否判定、状態変更、主要処理を担当する |
...
- ASCとCharacterAttributeSet・PostureAttributeSetが初期化済みか検証する。
...
[開始条件成立]
      |
      v
[EnemyDefinition、Damage Eventを受け付ける]
      |
      v
[ASCとCharacterAttributeSet・PostureAttributeSetが初期化済みか検証する]
      |
...
4. EnemyDefinitionからMaxHealth・MaxPostureを初期化し、Gameplay Effectで変更する。
5. Health・Posture属性を反映する。
...
|---|---|---|---|
| `Attribute.Health / Attribute.Posture` | 要件の状態・受付・通知 | 処理開始または条件成立時 | 完了・中断・条件解除時 |
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 実行入力 | InputAction / Gameplay Event / System Event | Yes | EnemyDefinition、Damage Event |
| 状態・対象参照 | Gameplay Tag / Attribute / Weak Object Reference | Yes | ASCとCharacterAttributeSet・PostureAttributeSetが初期化済みか検証する |
| 設定データ | Data Asset / Config | Yes | MaxHealth / MaxPosture |

### 10.2 出力

| 出力項目 | 型・形式 | 説明 |
|---|---|---|
| 実行結果 | State / Attribute / Result Event | Health・Posture属性 |
| 通知 | Delegate / Gameplay Event | 完了・失敗・中断を必要な連携先へ通知する |
| フィードバック | UI / Animation / VFX / SE | HPをHUD、体勢変化をリアクションまたはゲージで通知する |
...
| クラス・システム | 責務 | 実装区分 |
|---|---|---|
| `AEnemyCharacter / AttributeSet` | 実行可否、状態遷移、主要ロジック、重複防止 | `C++` |
| `EnemyDefinition` | MaxHealth / MaxPostureとAsset参照 | `Data Asset / Config` |
...
bool CanApplyEnemyAttributes() const;
void RequestApplyEnemyAttributes();
void CancelApplyEnemyAttributes();
bool IsApplyEnemyAttributesActive() const;
```

### 12.2 通知イベント

```cpp
DECLARE_MULTICAST_DELEGATE(FOnApplyEnemyAttributesStarted);
DECLARE_MULTICAST_DELEGATE(FOnApplyEnemyAttributesCompleted);
DECLARE_MULTICAST_DELEGATE(FOnApplyEnemyAttributesCancelled);
DECLARE_MULTICAST_DELEGATE_OneParam(FOnApplyEnemyAttributesFailed, EFailureReason);
```
...
|---|---|---:|---|---|
| MaxHealth / MaxPosture | Data Asset / Struct / Config | Data Asset定義値 | EnemyDefinition | 要件固有の判定・調整データ |
...
| 種別 | 内容 |
|---|---|
| UI | HPをHUD、体勢変化をリアクションまたはゲージで通知する |
...
- [ ] スポーン時に有効な最大値で初期化され、HealthとPostureが独立して増減する。
- [ ] 成功通知が1回発行され、一時状態が完了時に解除される。
- [ ] HPをHUD、体勢変化をリアクションまたはゲージで通知する。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-FR-ENEMY-006-001` | 正常系 | すべての事前条件を満たす | EnemyDefinition、Damage Eventを発生させる | スポーン時に有効な最大値で初期化され、HealthとPostureが独立して増減する |
| `TC-FR-ENEMY-006-002` | 異常系 | 必須参照または実行条件を1つ無効にする | 同じ要求を発生させる | 状態とリソースを変更せず安全に拒否する |
| `TC-FR-ENEMY-006-003` | 多重・中断 | 処理中または連続要求 | 再要求、死亡、Owner破棄のいずれかを発生させる | 重複せず一時状態を解除しCrashしない |
...
|---|---|---|---|---|
| `ADR-FR-ENEMY-006-001` | 責務配置 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | Blueprintだけにルールと状態を保持する | 状態整合性、検証容易性、データ調整性を保つため |
...
|---:|---|---|---|
| 1 | `FR-ENEMY-006: データ・状態・公開Interface` | 必須データ、状態、Validation、APIが定義される | なし |
| 2 | `FR-ENEMY-006: 主要ロジック` | 正常・拒否・中断・多重実行テストが通る | #1 |
| 3 | `FR-ENEMY-006: Asset・UI・結合確認` | 必須Assetを設定し受入条件を確認できる | #2 |
