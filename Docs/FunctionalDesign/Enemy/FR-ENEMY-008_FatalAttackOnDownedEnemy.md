...
| 要件種別 | 機能要件 |
| 要件カテゴリ | 敵 |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「ダウン時に専用致命攻撃を受ける」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
...
- 元要件「ダウン時に専用致命攻撃を受ける」を実現する。
- 正面または背後から対象を固定し、武器のFatalAttackDefinitionを実行する。
...
| `UGA_FatalAttack / UCombatComponent` | 実行可否判定、状態変更、主要処理を担当する |
...
- 対象がDownedかつWindow.FatalAttack.Available内で、Playerが実行可能か検証する。
...
4. 正面または背後から対象を固定し、武器のFatalAttackDefinitionを実行する。
5. 致命ダメージと終了リアクションを反映する。
...
|---|---|---|---|
| `Window.FatalAttack.Available` | 要件の状態・受付・通知 | 処理開始または条件成立時 | 完了・中断・条件解除時 |
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 実行入力 | InputAction / Gameplay Event / System Event | Yes | 致命攻撃入力、Downed対象 |
| 状態・対象参照 | Gameplay Tag / Attribute / Weak Object Reference | Yes | 対象がDownedかつWindow.FatalAttack.Available内で、Playerが実行可能か検証する |
| 設定データ | Data Asset / Config | Yes | FatalAttackDefinition |

### 10.2 出力

| 出力項目 | 型・形式 | 説明 |
|---|---|---|
| 実行結果 | State / Attribute / Result Event | 致命ダメージと終了リアクション |
| 通知 | Delegate / Gameplay Event | 完了・失敗・中断を必要な連携先へ通知する |
| フィードバック | UI / Animation / VFX / SE | 専用Montage、カメラ、VFX、SE、Hit Stopを同期する |
...
| クラス・システム | 責務 | 実装区分 |
|---|---|---|
| `UGA_FatalAttack / UCombatComponent` | 実行可否、状態遷移、主要ロジック、重複防止 | `C++` |
| `EnemyDefinition` | FatalAttackDefinitionとAsset参照 | `Data Asset / Config` |
...
bool CanFatalAttack() const;
void RequestFatalAttack();
void CancelFatalAttack();
bool IsFatalAttackActive() const;
```

### 12.2 通知イベント

```cpp
DECLARE_MULTICAST_DELEGATE(FOnFatalAttackStarted);
DECLARE_MULTICAST_DELEGATE(FOnFatalAttackCompleted);
DECLARE_MULTICAST_DELEGATE(FOnFatalAttackCancelled);
DECLARE_MULTICAST_DELEGATE_OneParam(FOnFatalAttackFailed, EFailureReason);
```
...
[AEnemyAIController]
        |
        v
[UGA_FatalAttack / UCombatComponent]
   |          |
   v          v
...
|---|---|---:|---|---|
| FatalAttackDefinition | Data Asset / Struct / Config | Data Asset定義値 | EnemyDefinition | 要件固有の判定・調整データ |
...
| 種別 | 内容 |
|---|---|
| UI | 専用Montage、カメラ、VFX、SE、Hit Stopを同期する |
...
- [ ] ダウン中の対象にのみ正面・背後から致命攻撃を1回実行できる。
- [ ] 成功通知が1回発行され、一時状態が完了時に解除される。
- [ ] 専用Montage、カメラ、VFX、SE、Hit Stopを同期する。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-FR-ENEMY-008-001` | 正常系 | すべての事前条件を満たす | 致命攻撃入力、Downed対象を発生させる | ダウン中の対象にのみ正面・背後から致命攻撃を1回実行できる |
| `TC-FR-ENEMY-008-002` | 異常系 | 必須参照または実行条件を1つ無効にする | 同じ要求を発生させる | 状態とリソースを変更せず安全に拒否する |
| `TC-FR-ENEMY-008-003` | 多重・中断 | 処理中または連続要求 | 再要求、死亡、Owner破棄のいずれかを発生させる | 重複せず一時状態を解除しCrashしない |
...
|---|---|---|---|---|
| `ADR-FR-ENEMY-008-001` | 責務配置 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | Blueprintだけにルールと状態を保持する | 状態整合性、検証容易性、データ調整性を保つため |
...
|---:|---|---|---|
| 1 | `FR-ENEMY-008: データ・状態・公開Interface` | 必須データ、状態、Validation、APIが定義される | なし |
| 2 | `FR-ENEMY-008: 主要ロジック` | 正常・拒否・中断・多重実行テストが通る | #1 |
| 3 | `FR-ENEMY-008: Asset・UI・結合確認` | 必須Assetを設定し受入条件を確認できる | #2 |
