...
| 要件種別 | 機能要件 |
| 要件カテゴリ | 敵 |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「体勢値が0になるとダウンする」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
...
- 元要件「体勢値が0になるとダウンする」を実現する。
- State.Reaction.Downedを付与しAI・通常Abilityを停止してFatalAttack Windowを開く。
...
| `UEnemyCombatComponent / ASC` | 実行可否判定、状態変更、主要処理を担当する |
...
4. State.Reaction.Downedを付与しAI・通常Abilityを停止してFatalAttack Windowを開く。
5. Downed状態と致命攻撃受付を反映する。
...
|---|---|---|---|
| `State.Reaction.Downed` | 要件の状態・受付・通知 | 処理開始または条件成立時 | 完了・中断・条件解除時 |
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 実行入力 | InputAction / Gameplay Event / System Event | Yes | Posture変更通知 |
| 状態・対象参照 | Gameplay Tag / Attribute / Weak Object Reference | Yes | 生存中かつPostureが0以下になったことを検証する |
| 設定データ | Data Asset / Config | Yes | DownDuration / PostureResetPolicy |

### 10.2 出力

| 出力項目 | 型・形式 | 説明 |
|---|---|---|
| 実行結果 | State / Attribute / Result Event | Downed状態と致命攻撃受付 |
| 通知 | Delegate / Gameplay Event | 完了・失敗・中断を必要な連携先へ通知する |
| フィードバック | UI / Animation / VFX / SE | ダウンAnimationと致命攻撃可能表示を出す |
...
| クラス・システム | 責務 | 実装区分 |
|---|---|---|
| `UEnemyCombatComponent / ASC` | 実行可否、状態遷移、主要ロジック、重複防止 | `C++` |
| `EnemyDefinition` | DownDuration / PostureResetPolicyとAsset参照 | `Data Asset / Config` |
...
- DownDuration / PostureResetPolicyを保持する。
...
bool CanEnterPostureDown() const;
void RequestEnterPostureDown();
void CancelEnterPostureDown();
bool IsEnterPostureDownActive() const;
```

### 12.2 通知イベント

```cpp
DECLARE_MULTICAST_DELEGATE(FOnEnterPostureDownStarted);
DECLARE_MULTICAST_DELEGATE(FOnEnterPostureDownCompleted);
DECLARE_MULTICAST_DELEGATE(FOnEnterPostureDownCancelled);
DECLARE_MULTICAST_DELEGATE_OneParam(FOnEnterPostureDownFailed, EFailureReason);
```
...
|---|---|---:|---|---|
| DownDuration / PostureResetPolicy | Data Asset / Struct / Config | TBD | EnemyDefinition | 要件固有の判定・調整データ |
...
- [ ] 生存敵のPostureが0以下になった時に1回だけダウンし、通常攻撃を停止する。
- [ ] 成功通知が1回発行され、一時状態が完了時に解除される。
- [ ] ダウンAnimationと致命攻撃可能表示を出す。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-FR-ENEMY-007-001` | 正常系 | すべての事前条件を満たす | Posture変更通知を発生させる | 生存敵のPostureが0以下になった時に1回だけダウンし、通常攻撃を停止する |
| `TC-FR-ENEMY-007-002` | 異常系 | 必須参照または実行条件を1つ無効にする | 同じ要求を発生させる | 状態とリソースを変更せず安全に拒否する |
| `TC-FR-ENEMY-007-003` | 多重・中断 | 処理中または連続要求 | 再要求、死亡、Owner破棄のいずれかを発生させる | 重複せず一時状態を解除しCrashしない |
...
| ID | 内容 | 影響範囲 | 決定期限 | 状態 |
|---|---|---|---|---|
| `Q-FR-ENEMY-007` | ダウン時間と復帰時PostureはTBD | 設計・データ・テスト | 実装Issue開始前 | Open |
...
|---|---|---|---|---|
| `ADR-FR-ENEMY-007-001` | 責務配置 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | Blueprintだけにルールと状態を保持する | 状態整合性、検証容易性、データ調整性を保つため |
...
|---:|---|---|---|
| 1 | `FR-ENEMY-007: データ・状態・公開Interface` | 必須データ、状態、Validation、APIが定義される | なし |
| 2 | `FR-ENEMY-007: 主要ロジック` | 正常・拒否・中断・多重実行テストが通る | #1 |
| 3 | `FR-ENEMY-007: Asset・UI・結合確認` | 必須Assetを設定し受入条件を確認できる | #2 |
