...
| 要件種別 | 機能要件 |
| 要件カテゴリ | 敵 |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「遠距離通常敵を実装する」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
...
- 元要件「遠距離通常敵を実装する」を実現する。
- 有効射線と距離を維持し、遠距離枠取得後にProjectile等のAbilityを実行する。
...
- AIController、Behavior Tree、射線、距離、遠距離枠を検証する。
...
4. 有効射線と距離を維持し、遠距離枠取得後にProjectile等のAbilityを実行する。
5. 位置調整・遠距離攻撃・待機状態を反映する。
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 実行入力 | InputAction / Gameplay Event / System Event | Yes | 知覚Target、遠距離攻撃定義 |
| 状態・対象参照 | Gameplay Tag / Attribute / Weak Object Reference | Yes | AIController、Behavior Tree、射線、距離、遠距離枠を検証する |
| 設定データ | Data Asset / Config | Yes | FEnemyDefinition / 遠距離AttackDefinitions |

### 10.2 出力

| 出力項目 | 型・形式 | 説明 |
|---|---|---|
| 実行結果 | State / Attribute / Result Event | 位置調整・遠距離攻撃・待機状態 |
| 通知 | Delegate / Gameplay Event | 完了・失敗・中断を必要な連携先へ通知する |
| フィードバック | UI / Animation / VFX / SE | 射撃予兆、Projectile、着弾を視認可能にする |
...
| `EnemyDefinition` | FEnemyDefinition / 遠距離AttackDefinitionsとAsset参照 | `Data Asset / Config` |
...
- FEnemyDefinition / 遠距離AttackDefinitionsを保持する。
...
bool CanRunRangedEnemy() const;
void RequestRunRangedEnemy();
void CancelRunRangedEnemy();
bool IsRunRangedEnemyActive() const;
```

### 12.2 通知イベント

```cpp
DECLARE_MULTICAST_DELEGATE(FOnRunRangedEnemyStarted);
DECLARE_MULTICAST_DELEGATE(FOnRunRangedEnemyCompleted);
DECLARE_MULTICAST_DELEGATE(FOnRunRangedEnemyCancelled);
DECLARE_MULTICAST_DELEGATE_OneParam(FOnRunRangedEnemyFailed, EFailureReason);
```
...
|---|---|---:|---|---|
| FEnemyDefinition / 遠距離AttackDefinitions | Data Asset / Struct / Config | Data Asset定義値 | EnemyDefinition | 要件固有の判定・調整データ |
...
- [ ] 有効射線と遠距離枠がある場合だけ攻撃し、近すぎる場合は再配置する。
- [ ] 成功通知が1回発行され、一時状態が完了時に解除される。
- [ ] 射撃予兆、Projectile、着弾を視認可能にする。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-FR-ENEMY-002-001` | 正常系 | すべての事前条件を満たす | 知覚Target、遠距離攻撃定義を発生させる | 有効射線と遠距離枠がある場合だけ攻撃し、近すぎる場合は再配置する |
| `TC-FR-ENEMY-002-002` | 異常系 | 必須参照または実行条件を1つ無効にする | 同じ要求を発生させる | 状態とリソースを変更せず安全に拒否する |
| `TC-FR-ENEMY-002-003` | 多重・中断 | 処理中または連続要求 | 再要求、死亡、Owner破棄のいずれかを発生させる | 重複せず一時状態を解除しCrashしない |
...
|---|---|---|---|---|
| `ADR-FR-ENEMY-002-001` | 責務配置 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | Blueprintだけにルールと状態を保持する | 状態整合性、検証容易性、データ調整性を保つため |
...
|---:|---|---|---|
| 1 | `FR-ENEMY-002: データ・状態・公開Interface` | 必須データ、状態、Validation、APIが定義される | なし |
| 2 | `FR-ENEMY-002: 主要ロジック` | 正常・拒否・中断・多重実行テストが通る | #1 |
| 3 | `FR-ENEMY-002: Asset・UI・結合確認` | 必須Assetを設定し受入条件を確認できる | #2 |
