...
|---|---|
| 要件ID | `FR-ENEMY-009` |
| 要件名 | 敵ごとに体勢値の回復設定を変更できる |
| 要件種別 | 機能要件 |
| 要件カテゴリ | 敵 |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「敵ごとに体勢値の回復設定を変更できる」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
...
- 元要件「敵ごとに体勢値の回復設定を変更できる」を実現する。
- 最後の体勢ダメージ後、設定遅延を経て有効な敵だけPostureを最大値まで回復する。
...
| `PostureAttributeSet / UEnemyCombatComponent` | 実行可否判定、状態変更、主要処理を担当する |
...
- EnemyDefinitionの回復有効フラグ・遅延・速度を検証する。
...
4. 最後の体勢ダメージ後、設定遅延を経て有効な敵だけPostureを最大値まで回復する。
5. Posture回復を反映する。
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 実行入力 | InputAction / Gameplay Event / System Event | Yes | PostureRecovery設定、経過時間 |
| 状態・対象参照 | Gameplay Tag / Attribute / Weak Object Reference | Yes | EnemyDefinitionの回復有効フラグ・遅延・速度を検証する |
| 設定データ | Data Asset / Config | Yes | PostureRecoveryEnabled / Delay / Rate |

### 10.2 出力

| 出力項目 | 型・形式 | 説明 |
|---|---|---|
| 実行結果 | State / Attribute / Result Event | Posture回復 |
| 通知 | Delegate / Gameplay Event | 完了・失敗・中断を必要な連携先へ通知する |
| フィードバック | UI / Animation / VFX / SE | 必要に応じて体勢ゲージを更新する |
...
| クラス・システム | 責務 | 実装区分 |
|---|---|---|
| `PostureAttributeSet / UEnemyCombatComponent` | 実行可否、状態遷移、主要ロジック、重複防止 | `C++` |
| `EnemyDefinition` | PostureRecoveryEnabled / Delay / RateとAsset参照 | `Data Asset / Config` |
...
- PostureRecoveryEnabled / Delay / Rateを保持する。
...
void RequestRecoverPosture();
void CancelRecoverPosture();
bool IsRecoverPostureActive() const;
```

### 12.2 通知イベント

```cpp
DECLARE_MULTICAST_DELEGATE(FOnRecoverPostureStarted);
DECLARE_MULTICAST_DELEGATE(FOnRecoverPostureCompleted);
DECLARE_MULTICAST_DELEGATE(FOnRecoverPostureCancelled);
DECLARE_MULTICAST_DELEGATE_OneParam(FOnRecoverPostureFailed, EFailureReason);
```
...
[AEnemyAIController]
        |
        v
[PostureAttributeSet / UEnemyCombatComponent]
   |          |
   v          v
...
|---|---|---:|---|---|
| PostureRecoveryEnabled / Delay / Rate | Data Asset / Struct / Config | TBD | EnemyDefinition | 要件固有の判定・調整データ |
...
- [ ] 回復無効の敵は自然回復せず、有効な敵は固有の遅延・速度で上限を超えず回復する。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-FR-ENEMY-009-001` | 正常系 | すべての事前条件を満たす | PostureRecovery設定、経過時間を発生させる | 回復無効の敵は自然回復せず、有効な敵は固有の遅延・速度で上限を超えず回復する |
| `TC-FR-ENEMY-009-002` | 異常系 | 必須参照または実行条件を1つ無効にする | 同じ要求を発生させる | 状態とリソースを変更せず安全に拒否する |
| `TC-FR-ENEMY-009-003` | 多重・中断 | 処理中または連続要求 | 再要求、死亡、Owner破棄のいずれかを発生させる | 重複せず一時状態を解除しCrashしない |
...
| ID | 内容 | 影響範囲 | 決定期限 | 状態 |
|---|---|---|---|---|
| `Q-FR-ENEMY-009` | 敵ごとの回復速度と遅延はTBD | 設計・データ・テスト | 実装Issue開始前 | Open |
...
|---|---|---|---|---|
| `ADR-FR-ENEMY-009-001` | 責務配置 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | Blueprintだけにルールと状態を保持する | 状態整合性、検証容易性、データ調整性を保つため |
...
|---:|---|---|---|
| 1 | `FR-ENEMY-009: データ・状態・公開Interface` | 必須データ、状態、Validation、APIが定義される | なし |
| 2 | `FR-ENEMY-009: 主要ロジック` | 正常・拒否・中断・多重実行テストが通る | #1 |
| 3 | `FR-ENEMY-009: Asset・UI・結合確認` | 必須Assetを設定し受入条件を確認できる | #2 |
