...
|---|---|
| 要件ID | `FR-ENEMY-003` |
| 要件名 | 通常戦で複数の敵が同時に行動する |
| 要件種別 | 機能要件 |
| 要件カテゴリ | 敵 |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「通常戦で複数の敵が同時に行動する」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
...
- 元要件「通常戦で複数の敵が同時に行動する」を実現する。
- Coordinatorが攻撃枠と待機優先度を管理し、非攻撃敵には位置調整・威嚇・待機を許可する。
...
- 登録敵とPlayer参照の有効性、敵の生存・戦闘状態を検証する。
...
4. Coordinatorが攻撃枠と待機優先度を管理し、非攻撃敵には位置調整・威嚇・待機を許可する。
5. 枠許可・待機優先度を反映する。
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 実行入力 | InputAction / Gameplay Event / System Event | Yes | 登録敵、攻撃枠要求 |
| 状態・対象参照 | Gameplay Tag / Attribute / Weak Object Reference | Yes | 登録敵とPlayer参照の有効性、敵の生存・戦闘状態を検証する |
| 設定データ | Data Asset / Config | Yes | 通常敵最大同時出現数・待機優先度 |

### 10.2 出力

| 出力項目 | 型・形式 | 説明 |
|---|---|---|
| 実行結果 | State / Attribute / Result Event | 枠許可・待機優先度 |
| 通知 | Delegate / Gameplay Event | 完了・失敗・中断を必要な連携先へ通知する |
| フィードバック | UI / Animation / VFX / SE | 開発時に各敵の枠種別と待機理由を表示する |
...
| `EnemyDefinition` | 通常敵最大同時出現数・待機優先度とAsset参照 | `Data Asset / Config` |
...
bool CanCoordinateEnemies() const;
void RequestCoordinateEnemies();
void CancelCoordinateEnemies();
bool IsCoordinateEnemiesActive() const;
```

### 12.2 通知イベント

```cpp
DECLARE_MULTICAST_DELEGATE(FOnCoordinateEnemiesStarted);
DECLARE_MULTICAST_DELEGATE(FOnCoordinateEnemiesCompleted);
DECLARE_MULTICAST_DELEGATE(FOnCoordinateEnemiesCancelled);
DECLARE_MULTICAST_DELEGATE_OneParam(FOnCoordinateEnemiesFailed, EFailureReason);
```
...
|---|---|---:|---|---|
| 通常敵最大同時出現数・待機優先度 | Data Asset / Struct / Config | TBD | EnemyDefinition | 要件固有の判定・調整データ |
...
- [ ] 複数敵が移動・待機・攻撃を並行し、攻撃枠上限を超えない。
- [ ] 成功通知が1回発行され、一時状態が完了時に解除される。
- [ ] 開発時に各敵の枠種別と待機理由を表示する。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-FR-ENEMY-003-001` | 正常系 | すべての事前条件を満たす | 登録敵、攻撃枠要求を発生させる | 複数敵が移動・待機・攻撃を並行し、攻撃枠上限を超えない |
| `TC-FR-ENEMY-003-002` | 異常系 | 必須参照または実行条件を1つ無効にする | 同じ要求を発生させる | 状態とリソースを変更せず安全に拒否する |
| `TC-FR-ENEMY-003-003` | 多重・中断 | 処理中または連続要求 | 再要求、死亡、Owner破棄のいずれかを発生させる | 重複せず一時状態を解除しCrashしない |
...
| ID | 内容 | 影響範囲 | 決定期限 | 状態 |
|---|---|---|---|---|
| `Q-FR-ENEMY-003` | 通常敵最大同時出現数はTBD | 設計・データ・テスト | 実装Issue開始前 | Open |
...
|---|---|---|---|---|
| `ADR-FR-ENEMY-003-001` | 責務配置 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | Blueprintだけにルールと状態を保持する | 状態整合性、検証容易性、データ調整性を保つため |
...
|---:|---|---|---|
| 1 | `FR-ENEMY-003: データ・状態・公開Interface` | 必須データ、状態、Validation、APIが定義される | なし |
| 2 | `FR-ENEMY-003: 主要ロジック` | 正常・拒否・中断・多重実行テストが通る | #1 |
| 3 | `FR-ENEMY-003: Asset・UI・結合確認` | 必須Assetを設定し受入条件を確認できる | #2 |
