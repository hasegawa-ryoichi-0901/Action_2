...
| 要件種別 | 機能要件 |
| 要件カテゴリ | ボス |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「特定攻撃後に大きな隙を持つ」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
...
- 元要件「特定攻撃後に大きな隙を持つ」を実現する。
- 攻撃完了後に定義時間のRecovery状態へ遷移し攻撃・移動選択を禁止する。
...
[開始条件成立]
      |
      v
[完了AttackId、RecoveryWindowを受け付ける]
      |
      v
[使用攻撃のRecoveryWindow設定を検証する]
      |
...
4. 攻撃完了後に定義時間のRecovery状態へ遷移し攻撃・移動選択を禁止する。
5. Boss Recovery状態を反映する。
...
|---|---|---|---|
| `State.Action.Recovery` | 要件の状態・受付・通知 | 処理開始または条件成立時 | 完了・中断・条件解除時 |
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 実行入力 | InputAction / Gameplay Event / System Event | Yes | 完了AttackId、RecoveryWindow |
| 状態・対象参照 | Gameplay Tag / Attribute / Weak Object Reference | Yes | 使用攻撃のRecoveryWindow設定を検証する |
| 設定データ | Data Asset / Config | Yes | RecoveryWindow |

### 10.2 出力

| 出力項目 | 型・形式 | 説明 |
|---|---|---|
| 実行結果 | State / Attribute / Result Event | Boss Recovery状態 |
| 通知 | Delegate / Gameplay Event | 完了・失敗・中断を必要な連携先へ通知する |
| フィードバック | UI / Animation / VFX / SE | 大きな隙をAnimation姿勢と状態で明確に示す |
...
| `BossAttackDefinition` | RecoveryWindowとAsset参照 | `Data Asset / Config` |
...
bool CanApplyBossRecovery() const;
void RequestApplyBossRecovery();
void CancelApplyBossRecovery();
bool IsApplyBossRecoveryActive() const;
```

### 12.2 通知イベント

```cpp
DECLARE_MULTICAST_DELEGATE(FOnApplyBossRecoveryStarted);
DECLARE_MULTICAST_DELEGATE(FOnApplyBossRecoveryCompleted);
DECLARE_MULTICAST_DELEGATE(FOnApplyBossRecoveryCancelled);
DECLARE_MULTICAST_DELEGATE_OneParam(FOnApplyBossRecoveryFailed, EFailureReason);
```
...
|---|---|---:|---|---|
| RecoveryWindow | Data Asset / Struct / Config | TBD | BossAttackDefinition | 要件固有の判定・調整データ |
...
- [ ] RecoveryWindowを持つ攻撃後だけ定義時間中は新規攻撃せず、終了後に通常選択へ戻る。
- [ ] 成功通知が1回発行され、一時状態が完了時に解除される。
- [ ] 大きな隙をAnimation姿勢と状態で明確に示す。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-FR-BOSS-012-001` | 正常系 | すべての事前条件を満たす | 完了AttackId、RecoveryWindowを発生させる | RecoveryWindowを持つ攻撃後だけ定義時間中は新規攻撃せず、終了後に通常選択へ戻る |
| `TC-FR-BOSS-012-002` | 異常系 | 必須参照または実行条件を1つ無効にする | 同じ要求を発生させる | 状態とリソースを変更せず安全に拒否する |
| `TC-FR-BOSS-012-003` | 多重・中断 | 処理中または連続要求 | 再要求、死亡、Owner破棄のいずれかを発生させる | 重複せず一時状態を解除しCrashしない |
...
| ID | 内容 | 影響範囲 | 決定期限 | 状態 |
|---|---|---|---|---|
| `Q-FR-BOSS-012` | 対象攻撃と隙の時間はTBD | 設計・データ・テスト | 実装Issue開始前 | Open |
...
|---|---|---|---|---|
| `ADR-FR-BOSS-012-001` | 責務配置 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | Blueprintだけにルールと状態を保持する | 状態整合性、検証容易性、データ調整性を保つため |
...
|---:|---|---|---|
| 1 | `FR-BOSS-012: データ・状態・公開Interface` | 必須データ、状態、Validation、APIが定義される | なし |
| 2 | `FR-BOSS-012: 主要ロジック` | 正常・拒否・中断・多重実行テストが通る | #1 |
| 3 | `FR-BOSS-012: Asset・UI・結合確認` | 必須Assetを設定し受入条件を確認できる | #2 |
