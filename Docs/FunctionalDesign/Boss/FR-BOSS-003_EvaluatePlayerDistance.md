...
| 要件種別 | 機能要件 |
| 要件カテゴリ | ボス |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「プレイヤーとの距離を評価する」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
...
- 元要件「プレイヤーとの距離を評価する」を実現する。
- 距離をAttackDefinitionのMinimumRange・MaximumRangeへ照合しDistanceScoreを計算する。
...
- Player・Boss位置が有効で攻撃選択タイミングであることを検証する。
...
4. 距離をAttackDefinitionのMinimumRange・MaximumRangeへ照合しDistanceScoreを計算する。
5. DistanceScoreを反映する。
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 実行入力 | InputAction / Gameplay Event / System Event | Yes | Boss位置、Player位置、攻撃距離範囲 |
| 状態・対象参照 | Gameplay Tag / Attribute / Weak Object Reference | Yes | Player・Boss位置が有効で攻撃選択タイミングであることを検証する |
| 設定データ | Data Asset / Config | Yes | MinimumRange / MaximumRange / DistanceScoreCurve |

### 10.2 出力

| 出力項目 | 型・形式 | 説明 |
|---|---|---|
| 実行結果 | State / Attribute / Result Event | DistanceScore |
| 通知 | Delegate / Gameplay Event | 完了・失敗・中断を必要な連携先へ通知する |
| フィードバック | UI / Animation / VFX / SE | AIデバッグ表示に距離と加点内訳を表示する |
...
| `BossAttackDefinition` | MinimumRange / MaximumRange / DistanceScoreCurveとAsset参照 | `Data Asset / Config` |
...
- MinimumRange / MaximumRange / DistanceScoreCurveを保持する。
...
bool CanEvaluateDistance() const;
void RequestEvaluateDistance();
void CancelEvaluateDistance();
bool IsEvaluateDistanceActive() const;
```

### 12.2 通知イベント

```cpp
DECLARE_MULTICAST_DELEGATE(FOnEvaluateDistanceStarted);
DECLARE_MULTICAST_DELEGATE(FOnEvaluateDistanceCompleted);
DECLARE_MULTICAST_DELEGATE(FOnEvaluateDistanceCancelled);
DECLARE_MULTICAST_DELEGATE_OneParam(FOnEvaluateDistanceFailed, EFailureReason);
```
...
|---|---|---:|---|---|
| MinimumRange / MaximumRange / DistanceScoreCurve | Data Asset / Struct / Config | Data Asset定義値 | BossAttackDefinition | 要件固有の判定・調整データ |
...
- [ ] 範囲外攻撃は選択不可または明示的Penaltyとなり、範囲内攻撃へ定義済みScoreが加算される。
- [ ] 成功通知が1回発行され、一時状態が完了時に解除される。
- [ ] AIデバッグ表示に距離と加点内訳を表示する。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-FR-BOSS-003-001` | 正常系 | すべての事前条件を満たす | Boss位置、Player位置、攻撃距離範囲を発生させる | 範囲外攻撃は選択不可または明示的Penaltyとなり、範囲内攻撃へ定義済みScoreが加算される |
| `TC-FR-BOSS-003-002` | 異常系 | 必須参照または実行条件を1つ無効にする | 同じ要求を発生させる | 状態とリソースを変更せず安全に拒否する |
| `TC-FR-BOSS-003-003` | 多重・中断 | 処理中または連続要求 | 再要求、死亡、Owner破棄のいずれかを発生させる | 重複せず一時状態を解除しCrashしない |
...
|---|---|---|---|---|
| `ADR-FR-BOSS-003-001` | 責務配置 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | Blueprintだけにルールと状態を保持する | 状態整合性、検証容易性、データ調整性を保つため |
...
|---:|---|---|---|
| 1 | `FR-BOSS-003: データ・状態・公開Interface` | 必須データ、状態、Validation、APIが定義される | なし |
| 2 | `FR-BOSS-003: 主要ロジック` | 正常・拒否・中断・多重実行テストが通る | #1 |
| 3 | `FR-BOSS-003: Asset・UI・結合確認` | 必須Assetを設定し受入条件を確認できる | #2 |
