...
|---|---|
| 要件ID | `FR-BOSS-006` |
| 要件名 | プレイヤーの残りスタミナを評価する |
| 要件種別 | 機能要件 |
| 要件カテゴリ | ボス |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「プレイヤーの残りスタミナを評価する」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
...
Playerの確定済み資源状態に応じてBossの圧力を調整する。
...
- 元要件「プレイヤーの残りスタミナを評価する」を実現する。
- 残量率をPlayerStaminaModifiersまたはCurveへ入力してScore補正を計算する。
...
- ASCから正規化可能なStamina・MaxStaminaを取得できるか検証する。
...
4. 残量率をPlayerStaminaModifiersまたはCurveへ入力してScore補正を計算する。
5. PlayerStaminaScoreを反映する。
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 実行入力 | InputAction / Gameplay Event / System Event | Yes | Stamina、MaxStamina |
| 状態・対象参照 | Gameplay Tag / Attribute / Weak Object Reference | Yes | ASCから正規化可能なStamina・MaxStaminaを取得できるか検証する |
| 設定データ | Data Asset / Config | Yes | PlayerStaminaModifiers |

### 10.2 出力

| 出力項目 | 型・形式 | 説明 |
|---|---|---|
| 実行結果 | State / Attribute / Result Event | PlayerStaminaScore |
| 通知 | Delegate / Gameplay Event | 完了・失敗・中断を必要な連携先へ通知する |
| フィードバック | UI / Animation / VFX / SE | AIデバッグ表示に残量率と補正値を表示する |
...
| `BossAttackDefinition` | PlayerStaminaModifiersとAsset参照 | `Data Asset / Config` |
...
bool CanEvaluateStamina() const;
void RequestEvaluateStamina();
void CancelEvaluateStamina();
bool IsEvaluateStaminaActive() const;
```

### 12.2 通知イベント

```cpp
DECLARE_MULTICAST_DELEGATE(FOnEvaluateStaminaStarted);
DECLARE_MULTICAST_DELEGATE(FOnEvaluateStaminaCompleted);
DECLARE_MULTICAST_DELEGATE(FOnEvaluateStaminaCancelled);
DECLARE_MULTICAST_DELEGATE_OneParam(FOnEvaluateStaminaFailed, EFailureReason);
```
...
|---|---|---:|---|---|
| PlayerStaminaModifiers | Data Asset / Struct / Config | Data Asset定義値 | BossAttackDefinition | 要件固有の判定・調整データ |
...
- [ ] 残量率の定義区分またはCurveに一致する補正だけが選択時に適用される。
- [ ] 成功通知が1回発行され、一時状態が完了時に解除される。
- [ ] AIデバッグ表示に残量率と補正値を表示する。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-FR-BOSS-006-001` | 正常系 | すべての事前条件を満たす | Stamina、MaxStaminaを発生させる | 残量率の定義区分またはCurveに一致する補正だけが選択時に適用される |
| `TC-FR-BOSS-006-002` | 異常系 | 必須参照または実行条件を1つ無効にする | 同じ要求を発生させる | 状態とリソースを変更せず安全に拒否する |
| `TC-FR-BOSS-006-003` | 多重・中断 | 処理中または連続要求 | 再要求、死亡、Owner破棄のいずれかを発生させる | 重複せず一時状態を解除しCrashしない |
...
|---|---|---|---|---|
| `ADR-FR-BOSS-006-001` | 責務配置 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | Blueprintだけにルールと状態を保持する | 状態整合性、検証容易性、データ調整性を保つため |
...
|---:|---|---|---|
| 1 | `FR-BOSS-006: データ・状態・公開Interface` | 必須データ、状態、Validation、APIが定義される | なし |
| 2 | `FR-BOSS-006: 主要ロジック` | 正常・拒否・中断・多重実行テストが通る | #1 |
| 3 | `FR-BOSS-006: Asset・UI・結合確認` | 必須Assetを設定し受入条件を確認できる | #2 |
