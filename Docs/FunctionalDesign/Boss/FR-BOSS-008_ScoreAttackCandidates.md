...
|---|---|
| 要件ID | `FR-BOSS-008` |
| 要件名 | 状況に応じて攻撃候補の評価値を変更する |
| 要件種別 | 機能要件 |
| 要件カテゴリ | ボス |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「状況に応じて攻撃候補の評価値を変更する」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
...
- 元要件「状況に応じて攻撃候補の評価値を変更する」を実現する。
- BaseScoreへ各Context Scoreを加算しCooldown・反復Penaltyを減算して最大候補を選ぶ。
...
| `UEnemyAttackEvaluator` | 実行可否判定、状態変更、主要処理を担当する |
...
- 候補のPhase・距離・角度・Cooldown・Ability参照を検証する。
...
[開始条件成立]
      |
      v
[AttackDefinitions、Evaluation Contextを受け付ける]
      |
      v
[候補のPhase・距離・角度・Cooldown・Ability参照を検証する]
      |
...
4. BaseScoreへ各Context Scoreを加算しCooldown・反復Penaltyを減算して最大候補を選ぶ。
5. 候補別FinalScoreと選択攻撃を反映する。
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 実行入力 | InputAction / Gameplay Event / System Event | Yes | AttackDefinitions、Evaluation Context |
| 状態・対象参照 | Gameplay Tag / Attribute / Weak Object Reference | Yes | 候補のPhase・距離・角度・Cooldown・Ability参照を検証する |
| 設定データ | Data Asset / Config | Yes | FBossAttackDefinition |

### 10.2 出力

| 出力項目 | 型・形式 | 説明 |
|---|---|---|
| 実行結果 | State / Attribute / Result Event | 候補別FinalScoreと選択攻撃 |
| 通知 | Delegate / Gameplay Event | 完了・失敗・中断を必要な連携先へ通知する |
| フィードバック | UI / Animation / VFX / SE | 選択攻撃と全Score内訳をデバッグ表示する |
...
| クラス・システム | 責務 | 実装区分 |
|---|---|---|
| `UEnemyAttackEvaluator` | 実行可否、状態遷移、主要ロジック、重複防止 | `C++` |
| `BossAttackDefinition` | FBossAttackDefinitionとAsset参照 | `Data Asset / Config` |
...
bool CanScoreAttack() const;
void RequestScoreAttack();
void CancelScoreAttack();
bool IsScoreAttackActive() const;
```

### 12.2 通知イベント

```cpp
DECLARE_MULTICAST_DELEGATE(FOnScoreAttackStarted);
DECLARE_MULTICAST_DELEGATE(FOnScoreAttackCompleted);
DECLARE_MULTICAST_DELEGATE(FOnScoreAttackCancelled);
DECLARE_MULTICAST_DELEGATE_OneParam(FOnScoreAttackFailed, EFailureReason);
```
...
|---|---|---:|---|---|
| FBossAttackDefinition | Data Asset / Struct / Config | TBD | BossAttackDefinition | 要件固有の判定・調整データ |
...
- [ ] 無効候補を除外し、同じContextと乱数Seedでは同じScore内訳を再現できる。
- [ ] 成功通知が1回発行され、一時状態が完了時に解除される。
- [ ] 選択攻撃と全Score内訳をデバッグ表示する。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-FR-BOSS-008-001` | 正常系 | すべての事前条件を満たす | AttackDefinitions、Evaluation Contextを発生させる | 無効候補を除外し、同じContextと乱数Seedでは同じScore内訳を再現できる |
| `TC-FR-BOSS-008-002` | 異常系 | 必須参照または実行条件を1つ無効にする | 同じ要求を発生させる | 状態とリソースを変更せず安全に拒否する |
| `TC-FR-BOSS-008-003` | 多重・中断 | 処理中または連続要求 | 再要求、死亡、Owner破棄のいずれかを発生させる | 重複せず一時状態を解除しCrashしない |
...
| ID | 内容 | 影響範囲 | 決定期限 | 状態 |
|---|---|---|---|---|
| `Q-FR-BOSS-008` | 各攻撃の基礎値・補正値はTBD | 設計・データ・テスト | 実装Issue開始前 | Open |
...
|---|---|---|---|---|
| `ADR-FR-BOSS-008-001` | 責務配置 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | Blueprintだけにルールと状態を保持する | 状態整合性、検証容易性、データ調整性を保つため |
...
|---:|---|---|---|
| 1 | `FR-BOSS-008: データ・状態・公開Interface` | 必須データ、状態、Validation、APIが定義される | なし |
| 2 | `FR-BOSS-008: 主要ロジック` | 正常・拒否・中断・多重実行テストが通る | #1 |
| 3 | `FR-BOSS-008: Asset・UI・結合確認` | 必須Assetを設定し受入条件を確認できる | #2 |
