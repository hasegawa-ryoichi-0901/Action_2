...
|---|---|
| 要件ID | `FR-BOSS-002` |
| 要件名 | HP50%以下で第2フェーズへ移行する |
| 要件種別 | 機能要件 |
| 要件カテゴリ | ボス |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「HP50%以下で第2フェーズへ移行する」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
...
- 元要件「HP50%以下で第2フェーズへ移行する」を実現する。
- StateTreeをPhaseTransition経由でPhase2へ一方向遷移させる。
...
| `StateTreeComponent` | 実行可否判定、状態変更、主要処理を担当する |
...
- 現在Phase1でHealth割合が50%以下になったか検証する。
...
4. StateTreeをPhaseTransition経由でPhase2へ一方向遷移させる。
5. Phase2状態と攻撃候補更新を反映する。
...
|---|---|---|---|
| `State.Boss.Phase2` | 要件の状態・受付・通知 | 処理開始または条件成立時 | 完了・中断・条件解除時 |
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 実行入力 | InputAction / Gameplay Event / System Event | Yes | Boss Health変更 |
| 状態・対象参照 | Gameplay Tag / Attribute / Weak Object Reference | Yes | 現在Phase1でHealth割合が50%以下になったか検証する |
| 設定データ | Data Asset / Config | Yes | PhaseThreshold |

### 10.2 出力

| 出力項目 | 型・形式 | 説明 |
|---|---|---|
| 実行結果 | State / Attribute / Result Event | Phase2状態と攻撃候補更新 |
| 通知 | Delegate / Gameplay Event | 完了・失敗・中断を必要な連携先へ通知する |
| フィードバック | UI / Animation / VFX / SE | Phase移行Animation・VFX・SEを再生する |
...
| クラス・システム | 責務 | 実装区分 |
|---|---|---|
| `StateTreeComponent` | 実行可否、状態遷移、主要ロジック、重複防止 | `C++` |
| `BossAttackDefinition` | PhaseThresholdとAsset参照 | `Data Asset / Config` |
...
bool CanTransitionBossPhase() const;
void RequestTransitionBossPhase();
void CancelTransitionBossPhase();
bool IsTransitionBossPhaseActive() const;
```

### 12.2 通知イベント

```cpp
DECLARE_MULTICAST_DELEGATE(FOnTransitionBossPhaseStarted);
DECLARE_MULTICAST_DELEGATE(FOnTransitionBossPhaseCompleted);
DECLARE_MULTICAST_DELEGATE(FOnTransitionBossPhaseCancelled);
DECLARE_MULTICAST_DELEGATE_OneParam(FOnTransitionBossPhaseFailed, EFailureReason);
```
...
|---|---|---:|---|---|
| PhaseThreshold | Data Asset / Struct / Config | 0.5 | BossAttackDefinition | 要件固有の判定・調整データ |
...
| 種別 | 内容 |
|---|---|
| UI | Phase移行Animation・VFX・SEを再生する |
...
- [ ] HP割合が初めて50%以下になった時に1回だけPhase2へ移行し、回復してもPhase1へ戻らない。
- [ ] 成功通知が1回発行され、一時状態が完了時に解除される。
- [ ] Phase移行Animation・VFX・SEを再生する。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-FR-BOSS-002-001` | 正常系 | すべての事前条件を満たす | Boss Health変更を発生させる | HP割合が初めて50%以下になった時に1回だけPhase2へ移行し、回復してもPhase1へ戻らない |
| `TC-FR-BOSS-002-002` | 異常系 | 必須参照または実行条件を1つ無効にする | 同じ要求を発生させる | 状態とリソースを変更せず安全に拒否する |
| `TC-FR-BOSS-002-003` | 多重・中断 | 処理中または連続要求 | 再要求、死亡、Owner破棄のいずれかを発生させる | 重複せず一時状態を解除しCrashしない |
...
|---|---|---|---|---|
| `ADR-FR-BOSS-002-001` | 責務配置 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | Blueprintだけにルールと状態を保持する | 状態整合性、検証容易性、データ調整性を保つため |
...
|---:|---|---|---|
| 1 | `FR-BOSS-002: データ・状態・公開Interface` | 必須データ、状態、Validation、APIが定義される | なし |
| 2 | `FR-BOSS-002: 主要ロジック` | 正常・拒否・中断・多重実行テストが通る | #1 |
| 3 | `FR-BOSS-002: Asset・UI・結合確認` | 必須Assetを設定し受入条件を確認できる | #2 |
