...
|---|---|
| 要件ID | `FR-BOSS-010` |
| 要件名 | 状況に応じて遠距離攻撃頻度を変更する |
| 要件種別 | 機能要件 |
| 要件カテゴリ | ボス |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「状況に応じて遠距離攻撃頻度を変更する」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
...
固定周期ではない遠距離圧力をPlayerの行動傾向に応じて調整する。
...
- 元要件「状況に応じて遠距離攻撃頻度を変更する」を実現する。
- Trend補正をRanged Tagを持つ候補へ適用し、連続使用Penaltyと併用する。
...
- 遠距離候補の射線・距離・Cooldown・反復回数を検証する。
...
4. Trend補正をRanged Tagを持つ候補へ適用し、連続使用Penaltyと併用する。
5. Ranged攻撃Scoreを反映する。
...
|---|---|---|---|
| `Attack.Type.Ranged` | 要件の状態・受付・通知 | 処理開始または条件成立時 | 完了・中断・条件解除時 |
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 実行入力 | InputAction / Gameplay Event / System Event | Yes | RangeTrend、遠距離候補 |
| 状態・対象参照 | Gameplay Tag / Attribute / Weak Object Reference | Yes | 遠距離候補の射線・距離・Cooldown・反復回数を検証する |
| 設定データ | Data Asset / Config | Yes | RangeTrendModifier / RepetitionPenalty |

### 10.2 出力

| 出力項目 | 型・形式 | 説明 |
|---|---|---|
| 実行結果 | State / Attribute / Result Event | Ranged攻撃Score |
| 通知 | Delegate / Gameplay Event | 完了・失敗・中断を必要な連携先へ通知する |
| フィードバック | UI / Animation / VFX / SE | 遠距離候補の補正と選択回数をデバッグ表示する |
...
| `BossAttackDefinition` | RangeTrendModifier / RepetitionPenaltyとAsset参照 | `Data Asset / Config` |
...
- RangeTrendModifier / RepetitionPenaltyを保持する。
...
bool CanScoreRangedAttack() const;
void RequestScoreRangedAttack();
void CancelScoreRangedAttack();
bool IsScoreRangedAttackActive() const;
```

### 12.2 通知イベント

```cpp
DECLARE_MULTICAST_DELEGATE(FOnScoreRangedAttackStarted);
DECLARE_MULTICAST_DELEGATE(FOnScoreRangedAttackCompleted);
DECLARE_MULTICAST_DELEGATE(FOnScoreRangedAttackCancelled);
DECLARE_MULTICAST_DELEGATE_OneParam(FOnScoreRangedAttackFailed, EFailureReason);
```
...
|---|---|---:|---|---|
| RangeTrendModifier / RepetitionPenalty | Data Asset / Struct / Config | Data Asset定義値 | BossAttackDefinition | 要件固有の判定・調整データ |
...
- [ ] 条件変化により遠距離候補Scoreが変わり、Cooldown中や射線なしでは選択されない。
- [ ] 成功通知が1回発行され、一時状態が完了時に解除される。
- [ ] 遠距離候補の補正と選択回数をデバッグ表示する。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-FR-BOSS-010-001` | 正常系 | すべての事前条件を満たす | RangeTrend、遠距離候補を発生させる | 条件変化により遠距離候補Scoreが変わり、Cooldown中や射線なしでは選択されない |
| `TC-FR-BOSS-010-002` | 異常系 | 必須参照または実行条件を1つ無効にする | 同じ要求を発生させる | 状態とリソースを変更せず安全に拒否する |
| `TC-FR-BOSS-010-003` | 多重・中断 | 処理中または連続要求 | 再要求、死亡、Owner破棄のいずれかを発生させる | 重複せず一時状態を解除しCrashしない |
...
|---|---|---|---|---|
| `ADR-FR-BOSS-010-001` | 責務配置 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | Blueprintだけにルールと状態を保持する | 状態整合性、検証容易性、データ調整性を保つため |
...
|---:|---|---|---|
| 1 | `FR-BOSS-010: データ・状態・公開Interface` | 必須データ、状態、Validation、APIが定義される | なし |
| 2 | `FR-BOSS-010: 主要ロジック` | 正常・拒否・中断・多重実行テストが通る | #1 |
| 3 | `FR-BOSS-010: Asset・UI・結合確認` | 必須Assetを設定し受入条件を確認できる | #2 |
