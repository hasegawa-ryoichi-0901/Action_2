...
|---|---|
| 要件ID | `FR-BOSS-009` |
| 要件名 | 状況に応じて接近・離脱頻度を変更する |
| 要件種別 | 機能要件 |
| 要件カテゴリ | ボス |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「状況に応じて接近・離脱頻度を変更する」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
...
- 元要件「状況に応じて接近・離脱頻度を変更する」を実現する。
- RangeTrendと現在距離からApproach・Retreat候補のScoreを補正する。
...
- 移動候補のNav到達性、Arena境界、Cooldownを検証する。
...
4. RangeTrendと現在距離からApproach・Retreat候補のScoreを補正する。
5. 接近・離脱行動選択を反映する。
...
|---|---|---|---|
| `State.AI.Reposition` | 要件の状態・受付・通知 | 処理開始または条件成立時 | 完了・中断・条件解除時 |
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 実行入力 | InputAction / Gameplay Event / System Event | Yes | 距離、RangeTrend、位置関係 |
| 状態・対象参照 | Gameplay Tag / Attribute / Weak Object Reference | Yes | 移動候補のNav到達性、Arena境界、Cooldownを検証する |
| 設定データ | Data Asset / Config | Yes | RangeTrendModifier |

### 10.2 出力

| 出力項目 | 型・形式 | 説明 |
|---|---|---|
| 実行結果 | State / Attribute / Result Event | 接近・離脱行動選択 |
| 通知 | Delegate / Gameplay Event | 完了・失敗・中断を必要な連携先へ通知する |
| フィードバック | UI / Animation / VFX / SE | 選択理由と目標地点をデバッグ表示する |
...
| `BossAttackDefinition` | RangeTrendModifierとAsset参照 | `Data Asset / Config` |
...
bool CanSelectRangeMovement() const;
void RequestSelectRangeMovement();
void CancelSelectRangeMovement();
bool IsSelectRangeMovementActive() const;
```

### 12.2 通知イベント

```cpp
DECLARE_MULTICAST_DELEGATE(FOnSelectRangeMovementStarted);
DECLARE_MULTICAST_DELEGATE(FOnSelectRangeMovementCompleted);
DECLARE_MULTICAST_DELEGATE(FOnSelectRangeMovementCancelled);
DECLARE_MULTICAST_DELEGATE_OneParam(FOnSelectRangeMovementFailed, EFailureReason);
```
...
|---|---|---:|---|---|
| RangeTrendModifier | Data Asset / Struct / Config | Data Asset定義値 | BossAttackDefinition | 要件固有の判定・調整データ |
...
- [ ] 定義された長距離傾向時に接近候補、近距離偏重時に離脱候補のScoreが指定どおり変化する。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-FR-BOSS-009-001` | 正常系 | すべての事前条件を満たす | 距離、RangeTrend、位置関係を発生させる | 定義された長距離傾向時に接近候補、近距離偏重時に離脱候補のScoreが指定どおり変化する |
| `TC-FR-BOSS-009-002` | 異常系 | 必須参照または実行条件を1つ無効にする | 同じ要求を発生させる | 状態とリソースを変更せず安全に拒否する |
| `TC-FR-BOSS-009-003` | 多重・中断 | 処理中または連続要求 | 再要求、死亡、Owner破棄のいずれかを発生させる | 重複せず一時状態を解除しCrashしない |
...
|---|---|---|---|---|
| `ADR-FR-BOSS-009-001` | 責務配置 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | Blueprintだけにルールと状態を保持する | 状態整合性、検証容易性、データ調整性を保つため |
...
|---:|---|---|---|
| 1 | `FR-BOSS-009: データ・状態・公開Interface` | 必須データ、状態、Validation、APIが定義される | なし |
| 2 | `FR-BOSS-009: 主要ロジック` | 正常・拒否・中断・多重実行テストが通る | #1 |
| 3 | `FR-BOSS-009: Asset・UI・結合確認` | 必須Assetを設定し受入条件を確認できる | #2 |
