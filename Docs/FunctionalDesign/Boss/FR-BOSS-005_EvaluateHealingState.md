...
| 要件種別 | 機能要件 |
| 要件カテゴリ | ボス |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「プレイヤーの回復状態を評価する」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
...
- 元要件「プレイヤーの回復状態を評価する」を実現する。
- HealingStateModifierを攻撃候補Scoreへ加算する。
...
- State.Action.Healingが付与済みであることだけを参照し未反映入力を参照しない。
...
4. HealingStateModifierを攻撃候補Scoreへ加算する。
5. HealingStateScoreを反映する。
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 実行入力 | InputAction / Gameplay Event / System Event | Yes | 確定済みHealing Tag、攻撃候補 |
| 状態・対象参照 | Gameplay Tag / Attribute / Weak Object Reference | Yes | State.Action.Healingが付与済みであることだけを参照し未反映入力を参照しない |
| 設定データ | Data Asset / Config | Yes | HealingStateModifier |

### 10.2 出力

| 出力項目 | 型・形式 | 説明 |
|---|---|---|
| 実行結果 | State / Attribute / Result Event | HealingStateScore |
| 通知 | Delegate / Gameplay Event | 完了・失敗・中断を必要な連携先へ通知する |
| フィードバック | UI / Animation / VFX / SE | AIデバッグ表示にHealing状態と補正値を表示する |
...
| `BossAttackDefinition` | HealingStateModifierとAsset参照 | `Data Asset / Config` |
...
bool CanEvaluateHealing() const;
void RequestEvaluateHealing();
void CancelEvaluateHealing();
bool IsEvaluateHealingActive() const;
```

### 12.2 通知イベント

```cpp
DECLARE_MULTICAST_DELEGATE(FOnEvaluateHealingStarted);
DECLARE_MULTICAST_DELEGATE(FOnEvaluateHealingCompleted);
DECLARE_MULTICAST_DELEGATE(FOnEvaluateHealingCancelled);
DECLARE_MULTICAST_DELEGATE_OneParam(FOnEvaluateHealingFailed, EFailureReason);
```
...
|---|---|---:|---|---|
| HealingStateModifier | Data Asset / Struct / Config | Data Asset定義値 | BossAttackDefinition | 要件固有の判定・調整データ |
...
| 種別 | 内容 |
|---|---|
| UI | AIデバッグ表示にHealing状態と補正値を表示する |
...
- [ ] Heal Ability開始後だけ補正が適用され、入力バッファ段階では適用されない。
- [ ] 成功通知が1回発行され、一時状態が完了時に解除される。
- [ ] AIデバッグ表示にHealing状態と補正値を表示する。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-FR-BOSS-005-001` | 正常系 | すべての事前条件を満たす | 確定済みHealing Tag、攻撃候補を発生させる | Heal Ability開始後だけ補正が適用され、入力バッファ段階では適用されない |
| `TC-FR-BOSS-005-002` | 異常系 | 必須参照または実行条件を1つ無効にする | 同じ要求を発生させる | 状態とリソースを変更せず安全に拒否する |
| `TC-FR-BOSS-005-003` | 多重・中断 | 処理中または連続要求 | 再要求、死亡、Owner破棄のいずれかを発生させる | 重複せず一時状態を解除しCrashしない |
...
|---|---|---|---|---|
| `ADR-FR-BOSS-005-001` | 責務配置 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | Blueprintだけにルールと状態を保持する | 状態整合性、検証容易性、データ調整性を保つため |
...
|---:|---|---|---|
| 1 | `FR-BOSS-005: データ・状態・公開Interface` | 必須データ、状態、Validation、APIが定義される | なし |
| 2 | `FR-BOSS-005: 主要ロジック` | 正常・拒否・中断・多重実行テストが通る | #1 |
| 3 | `FR-BOSS-005: Asset・UI・結合確認` | 必須Assetを設定し受入条件を確認できる | #2 |
