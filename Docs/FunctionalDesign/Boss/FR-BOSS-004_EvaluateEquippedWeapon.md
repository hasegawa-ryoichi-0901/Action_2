...
| 要件種別 | 機能要件 |
| 要件カテゴリ | ボス |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「プレイヤーの装備武器を評価する」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
...
- 元要件「プレイヤーの装備武器を評価する」を実現する。
- PlayerWeaponModifiersから装備武器に対応するScore補正を取得する。
...
4. PlayerWeaponModifiersから装備武器に対応するScore補正を取得する。
5. WeaponMatchScoreを反映する。
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 実行入力 | InputAction / Gameplay Event / System Event | Yes | WeaponId、攻撃候補 |
| 状態・対象参照 | Gameplay Tag / Attribute / Weak Object Reference | Yes | 装備WeaponIdが確定済みで有効か検証する |
| 設定データ | Data Asset / Config | Yes | PlayerWeaponModifiers |

### 10.2 出力

| 出力項目 | 型・形式 | 説明 |
|---|---|---|
| 実行結果 | State / Attribute / Result Event | WeaponMatchScore |
| 通知 | Delegate / Gameplay Event | 完了・失敗・中断を必要な連携先へ通知する |
| フィードバック | UI / Animation / VFX / SE | AIデバッグ表示に武器IDと補正値を表示する |
...
| `BossAttackDefinition` | PlayerWeaponModifiersとAsset参照 | `Data Asset / Config` |
...
bool CanEvaluateWeapon() const;
void RequestEvaluateWeapon();
void CancelEvaluateWeapon();
bool IsEvaluateWeaponActive() const;
```

### 12.2 通知イベント

```cpp
DECLARE_MULTICAST_DELEGATE(FOnEvaluateWeaponStarted);
DECLARE_MULTICAST_DELEGATE(FOnEvaluateWeaponCompleted);
DECLARE_MULTICAST_DELEGATE(FOnEvaluateWeaponCancelled);
DECLARE_MULTICAST_DELEGATE_OneParam(FOnEvaluateWeaponFailed, EFailureReason);
```
...
|---|---|---:|---|---|
| PlayerWeaponModifiers | Data Asset / Struct / Config | Data Asset定義値 | BossAttackDefinition | 要件固有の判定・調整データ |
...
- [ ] 同一状況で武器IDを変えると定義された候補のScoreだけが指定量変化する。
- [ ] 成功通知が1回発行され、一時状態が完了時に解除される。
- [ ] AIデバッグ表示に武器IDと補正値を表示する。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-FR-BOSS-004-001` | 正常系 | すべての事前条件を満たす | WeaponId、攻撃候補を発生させる | 同一状況で武器IDを変えると定義された候補のScoreだけが指定量変化する |
| `TC-FR-BOSS-004-002` | 異常系 | 必須参照または実行条件を1つ無効にする | 同じ要求を発生させる | 状態とリソースを変更せず安全に拒否する |
| `TC-FR-BOSS-004-003` | 多重・中断 | 処理中または連続要求 | 再要求、死亡、Owner破棄のいずれかを発生させる | 重複せず一時状態を解除しCrashしない |
...
|---|---|---|---|---|
| `ADR-FR-BOSS-004-001` | 責務配置 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | Blueprintだけにルールと状態を保持する | 状態整合性、検証容易性、データ調整性を保つため |
...
|---:|---|---|---|
| 1 | `FR-BOSS-004: データ・状態・公開Interface` | 必須データ、状態、Validation、APIが定義される | なし |
| 2 | `FR-BOSS-004: 主要ロジック` | 正常・拒否・中断・多重実行テストが通る | #1 |
| 3 | `FR-BOSS-004: Asset・UI・結合確認` | 必須Assetを設定し受入条件を確認できる | #2 |
