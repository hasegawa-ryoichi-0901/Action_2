...
|---|---|
| 要件ID | `FR-BOSS-013` |
| 要件名 | ジャスト回避またはパリィ成功時に反撃可能な隙を作る |
| 要件種別 | 機能要件 |
| 要件カテゴリ | ボス |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「ジャスト回避またはパリィ成功時に反撃可能な隙を作る」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
...
- 元要件「ジャスト回避またはパリィ成功時に反撃可能な隙を作る」を実現する。
- Bossへ短いRecoveryまたはCounter Vulnerable状態を付与し、重複Eventを統合する。
...
| `StateTree / UEnemyAttackEvaluator` | 実行可否判定、状態変更、主要処理を担当する |
...
- Boss攻撃に対するPerfectDodgeまたはParry成功Eventを検証する。
...
[開始条件成立]
      |
      v
[PerfectDodge / Parry成功Eventを受け付ける]
      |
      v
[Boss攻撃に対するPerfectDodgeまたはParry成功Eventを検証する]
      |
...
4. Bossへ短いRecoveryまたはCounter Vulnerable状態を付与し、重複Eventを統合する。
5. 反撃可能Windowを反映する。
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 実行入力 | InputAction / Gameplay Event / System Event | Yes | PerfectDodge / Parry成功Event |
| 状態・対象参照 | Gameplay Tag / Attribute / Weak Object Reference | Yes | Boss攻撃に対するPerfectDodgeまたはParry成功Eventを検証する |
| 設定データ | Data Asset / Config | Yes | CounterVulnerabilityDuration |

### 10.2 出力

| 出力項目 | 型・形式 | 説明 |
|---|---|---|
| 実行結果 | State / Attribute / Result Event | 反撃可能Window |
| 通知 | Delegate / Gameplay Event | 完了・失敗・中断を必要な連携先へ通知する |
| フィードバック | UI / Animation / VFX / SE | Bossの隙AnimationとHUD/エフェクトで受付を示す |
...
| クラス・システム | 責務 | 実装区分 |
|---|---|---|
| `StateTree / UEnemyAttackEvaluator` | 実行可否、状態遷移、主要ロジック、重複防止 | `C++` |
| `BossAttackDefinition` | CounterVulnerabilityDurationとAsset参照 | `Data Asset / Config` |
...
- CounterVulnerabilityDurationを保持する。
...
bool CanOpenBossCounterWindow() const;
void RequestOpenBossCounterWindow();
void CancelOpenBossCounterWindow();
bool IsOpenBossCounterWindowActive() const;
```

### 12.2 通知イベント

```cpp
DECLARE_MULTICAST_DELEGATE(FOnOpenBossCounterWindowStarted);
DECLARE_MULTICAST_DELEGATE(FOnOpenBossCounterWindowCompleted);
DECLARE_MULTICAST_DELEGATE(FOnOpenBossCounterWindowCancelled);
DECLARE_MULTICAST_DELEGATE_OneParam(FOnOpenBossCounterWindowFailed, EFailureReason);
```
...
[ABossAIController]
        |
        v
[StateTree / UEnemyAttackEvaluator]
   |          |
   v          v
...
|---|---|---:|---|---|
| CounterVulnerabilityDuration | Data Asset / Struct / Config | TBD | BossAttackDefinition | 要件固有の判定・調整データ |
...
| 種別 | 内容 |
|---|---|
| UI | Bossの隙AnimationとHUD/エフェクトで受付を示す |
...
- [ ] 各成功Eventにつき1回だけ反撃Windowが開き、時間切れまたは反撃成立で閉じる。
- [ ] 成功通知が1回発行され、一時状態が完了時に解除される。
- [ ] Bossの隙AnimationとHUD/エフェクトで受付を示す。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-FR-BOSS-013-001` | 正常系 | すべての事前条件を満たす | PerfectDodge / Parry成功Eventを発生させる | 各成功Eventにつき1回だけ反撃Windowが開き、時間切れまたは反撃成立で閉じる |
| `TC-FR-BOSS-013-002` | 異常系 | 必須参照または実行条件を1つ無効にする | 同じ要求を発生させる | 状態とリソースを変更せず安全に拒否する |
| `TC-FR-BOSS-013-003` | 多重・中断 | 処理中または連続要求 | 再要求、死亡、Owner破棄のいずれかを発生させる | 重複せず一時状態を解除しCrashしない |
...
| ID | 内容 | 影響範囲 | 決定期限 | 状態 |
|---|---|---|---|---|
| `Q-FR-BOSS-013` | 反撃可能時間とBossリアクションはTBD | 設計・データ・テスト | 実装Issue開始前 | Open |
...
|---|---|---|---|---|
| `ADR-FR-BOSS-013-001` | 責務配置 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | Blueprintだけにルールと状態を保持する | 状態整合性、検証容易性、データ調整性を保つため |
...
|---:|---|---|---|
| 1 | `FR-BOSS-013: データ・状態・公開Interface` | 必須データ、状態、Validation、APIが定義される | なし |
| 2 | `FR-BOSS-013: 主要ロジック` | 正常・拒否・中断・多重実行テストが通る | #1 |
| 3 | `FR-BOSS-013: Asset・UI・結合確認` | 必須Assetを設定し受入条件を確認できる | #2 |
