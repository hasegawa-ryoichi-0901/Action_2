...
|---|---|
| 要件ID | `FR-BOSS-014` |
| 要件名 | 弓で距離を取られた場合に専用接近行動を選択できる |
| 要件種別 | 機能要件 |
| 要件カテゴリ | ボス |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「弓で距離を取られた場合に専用接近行動を選択できる」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
...
弓で遠距離を維持する単一戦術にBossが対応できるようにする。
...
- 元要件「弓で距離を取られた場合に専用接近行動を選択できる」を実現する。
- Bowかつ長距離Contextで専用GapCloser候補へScore補正を適用する。
...
- Player武器がBow、距離・射線・専用行動Cooldown・到達性を検証する。
...
4. Bowかつ長距離Contextで専用GapCloser候補へScore補正を適用する。
5. 専用接近行動選択を反映する。
...
|---|---|---|---|
| `State.AI.Approaching` | 要件の状態・受付・通知 | 処理開始または条件成立時 | 完了・中断・条件解除時 |
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 実行入力 | InputAction / Gameplay Event / System Event | Yes | WeaponId、距離、RangeTrend |
| 状態・対象参照 | Gameplay Tag / Attribute / Weak Object Reference | Yes | Player武器がBow、距離・射線・専用行動Cooldown・到達性を検証する |
| 設定データ | Data Asset / Config | Yes | Bow GapCloser Definition |

### 10.2 出力

| 出力項目 | 型・形式 | 説明 |
|---|---|---|
| 実行結果 | State / Attribute / Result Event | 専用接近行動選択 |
| 通知 | Delegate / Gameplay Event | 完了・失敗・中断を必要な連携先へ通知する |
| フィードバック | UI / Animation / VFX / SE | 接近行動の明確な予備動作を表示する |
...
| `BossAttackDefinition` | Bow GapCloser DefinitionとAsset参照 | `Data Asset / Config` |
...
bool CanSelectBowGapCloser() const;
void RequestSelectBowGapCloser();
void CancelSelectBowGapCloser();
bool IsSelectBowGapCloserActive() const;
```

### 12.2 通知イベント

```cpp
DECLARE_MULTICAST_DELEGATE(FOnSelectBowGapCloserStarted);
DECLARE_MULTICAST_DELEGATE(FOnSelectBowGapCloserCompleted);
DECLARE_MULTICAST_DELEGATE(FOnSelectBowGapCloserCancelled);
DECLARE_MULTICAST_DELEGATE_OneParam(FOnSelectBowGapCloserFailed, EFailureReason);
```
...
|---|---|---:|---|---|
| Bow GapCloser Definition | Data Asset / Struct / Config | TBD | BossAttackDefinition | 要件固有の判定・調整データ |
...
- [ ] Bow装備で定義距離を維持した場合に専用候補が有効となり、Cooldown中は再選択されない。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-FR-BOSS-014-001` | 正常系 | すべての事前条件を満たす | WeaponId、距離、RangeTrendを発生させる | Bow装備で定義距離を維持した場合に専用候補が有効となり、Cooldown中は再選択されない |
| `TC-FR-BOSS-014-002` | 異常系 | 必須参照または実行条件を1つ無効にする | 同じ要求を発生させる | 状態とリソースを変更せず安全に拒否する |
| `TC-FR-BOSS-014-003` | 多重・中断 | 処理中または連続要求 | 再要求、死亡、Owner破棄のいずれかを発生させる | 重複せず一時状態を解除しCrashしない |
...
| ID | 内容 | 影響範囲 | 決定期限 | 状態 |
|---|---|---|---|---|
| `Q-FR-BOSS-014` | 発動距離・傾向閾値・CooldownはTBD | 設計・データ・テスト | 実装Issue開始前 | Open |
...
|---|---|---|---|---|
| `ADR-FR-BOSS-014-001` | 責務配置 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | Blueprintだけにルールと状態を保持する | 状態整合性、検証容易性、データ調整性を保つため |
...
|---:|---|---|---|
| 1 | `FR-BOSS-014: データ・状態・公開Interface` | 必須データ、状態、Validation、APIが定義される | なし |
| 2 | `FR-BOSS-014: 主要ロジック` | 正常・拒否・中断・多重実行テストが通る | #1 |
| 3 | `FR-BOSS-014: Asset・UI・結合確認` | 必須Assetを設定し受入条件を確認できる | #2 |
