...
| 要件種別 | 機能要件 |
| 要件カテゴリ | ボス |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「1対1で戦う」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
...
- 元要件「1対1で戦う」を実現する。
- Encounter開始時に通常敵の介入を禁止し、BossとPlayerを戦闘対象として登録する。
...
| `AActionGameMode / Boss Encounter` | 実行可否判定、状態変更、主要処理を担当する |
...
- Boss・Playerが生存しEncounter未開始、Arena条件成立を検証する。
...
4. Encounter開始時に通常敵の介入を禁止し、BossとPlayerを戦闘対象として登録する。
5. Boss Encounter状態を反映する。
...
|---|---|---|---|
| `State.Encounter.Boss` | 要件の状態・受付・通知 | 処理開始または条件成立時 | 完了・中断・条件解除時 |
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 実行入力 | InputAction / Gameplay Event / System Event | Yes | Arena進入または開始Trigger |
| 状態・対象参照 | Gameplay Tag / Attribute / Weak Object Reference | Yes | Boss・Playerが生存しEncounter未開始、Arena条件成立を検証する |
| 設定データ | Data Asset / Config | Yes | Boss Encounter Definition |

### 10.2 出力

| 出力項目 | 型・形式 | 説明 |
|---|---|---|
| 実行結果 | State / Attribute / Result Event | Boss Encounter状態 |
| 通知 | Delegate / Gameplay Event | 完了・失敗・中断を必要な連携先へ通知する |
| フィードバック | UI / Animation / VFX / SE | Boss HP UIとArena状態を表示する |
...
| クラス・システム | 責務 | 実装区分 |
|---|---|---|
| `AActionGameMode / Boss Encounter` | 実行可否、状態遷移、主要ロジック、重複防止 | `C++` |
| `BossAttackDefinition` | Boss Encounter DefinitionとAsset参照 | `Data Asset / Config` |
...
- Boss Encounter Definitionを保持する。
...
bool CanStartBossEncounter() const;
void RequestStartBossEncounter();
void CancelStartBossEncounter();
bool IsStartBossEncounterActive() const;
```

### 12.2 通知イベント

```cpp
DECLARE_MULTICAST_DELEGATE(FOnStartBossEncounterStarted);
DECLARE_MULTICAST_DELEGATE(FOnStartBossEncounterCompleted);
DECLARE_MULTICAST_DELEGATE(FOnStartBossEncounterCancelled);
DECLARE_MULTICAST_DELEGATE_OneParam(FOnStartBossEncounterFailed, EFailureReason);
```
...
[ABossAIController]
        |
        v
[AActionGameMode / Boss Encounter]
   |          |
   v          v
...
|---|---|---:|---|---|
| Boss Encounter Definition | Data Asset / Struct / Config | Data Asset定義値 | BossAttackDefinition | 要件固有の判定・調整データ |
...
- [ ] Boss戦中にBoss以外の通常敵が戦闘へ新規参加しない。
- [ ] 成功通知が1回発行され、一時状態が完了時に解除される。
- [ ] Boss HP UIとArena状態を表示する。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-FR-BOSS-001-001` | 正常系 | すべての事前条件を満たす | Arena進入または開始Triggerを発生させる | Boss戦中にBoss以外の通常敵が戦闘へ新規参加しない |
| `TC-FR-BOSS-001-002` | 異常系 | 必須参照または実行条件を1つ無効にする | 同じ要求を発生させる | 状態とリソースを変更せず安全に拒否する |
| `TC-FR-BOSS-001-003` | 多重・中断 | 処理中または連続要求 | 再要求、死亡、Owner破棄のいずれかを発生させる | 重複せず一時状態を解除しCrashしない |
...
|---|---|---|---|---|
| `ADR-FR-BOSS-001-001` | 責務配置 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | Blueprintだけにルールと状態を保持する | 状態整合性、検証容易性、データ調整性を保つため |
...
|---:|---|---|---|
| 1 | `FR-BOSS-001: データ・状態・公開Interface` | 必須データ、状態、Validation、APIが定義される | なし |
| 2 | `FR-BOSS-001: 主要ロジック` | 正常・拒否・中断・多重実行テストが通る | #1 |
| 3 | `FR-BOSS-001: Asset・UI・結合確認` | 必須Assetを設定し受入条件を確認できる | #2 |
