...
| 要件種別 | 機能要件 |
| 要件カテゴリ | ボス |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「攻撃コンボを分岐できる」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
...
Playerの反応と状況に応じてBossコンボの終点を変化させる。
...
- 元要件「攻撃コンボを分岐できる」を実現する。
- 攻撃完了前の分岐点でContextを再評価し、有効な派生Abilityを選ぶ。
...
- 現在攻撃のComboBranches、受付Window、各派生条件を検証する。
...
- 次のBoss Attack Abilityが要件どおり反映される。
...
[開始条件成立]
      |
      v
[現在AttackId、ComboBranches、Contextを受け付ける]
      |
      v
[現在攻撃のComboBranches、受付Window、各派生条件を検証する]
      |
...
4. 攻撃完了前の分岐点でContextを再評価し、有効な派生Abilityを選ぶ。
5. 次のBoss Attack Abilityを反映する。
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 実行入力 | InputAction / Gameplay Event / System Event | Yes | 現在AttackId、ComboBranches、Context |
| 状態・対象参照 | Gameplay Tag / Attribute / Weak Object Reference | Yes | 現在攻撃のComboBranches、受付Window、各派生条件を検証する |
| 設定データ | Data Asset / Config | Yes | ComboBranches |

### 10.2 出力

| 出力項目 | 型・形式 | 説明 |
|---|---|---|
| 実行結果 | State / Attribute / Result Event | 次のBoss Attack Ability |
| 通知 | Delegate / Gameplay Event | 完了・失敗・中断を必要な連携先へ通知する |
| フィードバック | UI / Animation / VFX / SE | 派生ごとのAnimation予兆を識別可能にする |
...
| `BossAttackDefinition` | ComboBranchesとAsset参照 | `Data Asset / Config` |
...
bool CanBranchBossCombo() const;
void RequestBranchBossCombo();
void CancelBranchBossCombo();
bool IsBranchBossComboActive() const;
```

### 12.2 通知イベント

```cpp
DECLARE_MULTICAST_DELEGATE(FOnBranchBossComboStarted);
DECLARE_MULTICAST_DELEGATE(FOnBranchBossComboCompleted);
DECLARE_MULTICAST_DELEGATE(FOnBranchBossComboCancelled);
DECLARE_MULTICAST_DELEGATE_OneParam(FOnBranchBossComboFailed, EFailureReason);
```
...
|---|---|---:|---|---|
| ComboBranches | Data Asset / Struct / Config | Data Asset定義値 | BossAttackDefinition | 要件固有の判定・調整データ |
...
- [ ] 定義済み分岐点でのみ有効派生へ進み、無効・未定義時は現在コンボを終了する。
- [ ] 成功通知が1回発行され、一時状態が完了時に解除される。
- [ ] 派生ごとのAnimation予兆を識別可能にする。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-FR-BOSS-011-001` | 正常系 | すべての事前条件を満たす | 現在AttackId、ComboBranches、Contextを発生させる | 定義済み分岐点でのみ有効派生へ進み、無効・未定義時は現在コンボを終了する |
| `TC-FR-BOSS-011-002` | 異常系 | 必須参照または実行条件を1つ無効にする | 同じ要求を発生させる | 状態とリソースを変更せず安全に拒否する |
| `TC-FR-BOSS-011-003` | 多重・中断 | 処理中または連続要求 | 再要求、死亡、Owner破棄のいずれかを発生させる | 重複せず一時状態を解除しCrashしない |
...
|---|---|---|---|---|
| `ADR-FR-BOSS-011-001` | 責務配置 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | Blueprintだけにルールと状態を保持する | 状態整合性、検証容易性、データ調整性を保つため |
...
|---:|---|---|---|
| 1 | `FR-BOSS-011: データ・状態・公開Interface` | 必須データ、状態、Validation、APIが定義される | なし |
| 2 | `FR-BOSS-011: 主要ロジック` | 正常・拒否・中断・多重実行テストが通る | #1 |
| 3 | `FR-BOSS-011: Asset・UI・結合確認` | 必須Assetを設定し受入条件を確認できる | #2 |
