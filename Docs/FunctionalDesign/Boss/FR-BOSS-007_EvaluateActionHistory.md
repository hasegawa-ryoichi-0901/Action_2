...
|---|---|
| 要件ID | `FR-BOSS-007` |
| 要件名 | 直近数秒の行動と戦闘全体の傾向を評価する |
| 要件種別 | 機能要件 |
| 要件カテゴリ | ボス |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「直近数秒の行動と戦闘全体の傾向を評価する」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
...
短期反応と戦闘全体への適応を両立し、単調なBoss選択を避ける。
...
- 元要件「直近数秒の行動と戦闘全体の傾向を評価する」を実現する。
- Recent WindowとBattle Trendを別集計し各攻撃候補の補正へ渡す。
...
| アクター | 役割 |
|---|---|
| `ABossAIController` | 入力または処理開始条件を発行する |
| `Boss ActionHistory / UEnemyAttackEvaluator` | 実行可否判定、状態変更、主要処理を担当する |
| `Boss / Player` | 処理結果の反映または影響を受ける |
...
- 実行済み行動イベントだけが時刻付きで記録されているか検証する。
...
- RecentHistoryScore / BattleTrendScoreが要件どおり反映される。
...
[Recent WindowとBattle Trendを別集計し各攻撃候補の補正へ渡す]
      |
      v
[RecentHistoryScore / BattleTrendScoreを通知]
      |
      v
[一時状態を解除して終了]
```
...
4. Recent WindowとBattle Trendを別集計し各攻撃候補の補正へ渡す。
5. RecentHistoryScore / BattleTrendScoreを反映する。
...
|---|---|---|---|
| `Event.PlayerAction.*` | 要件の状態・受付・通知 | 処理開始または条件成立時 | 完了・中断・条件解除時 |
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 実行入力 | InputAction / Gameplay Event / System Event | Yes | 確定済みPlayer Action履歴 |
| 状態・対象参照 | Gameplay Tag / Attribute / Weak Object Reference | Yes | 実行済み行動イベントだけが時刻付きで記録されているか検証する |
| 設定データ | Data Asset / Config | Yes | RecentWindowSeconds / TrendWeights |

### 10.2 出力

| 出力項目 | 型・形式 | 説明 |
|---|---|---|
| 実行結果 | State / Attribute / Result Event | RecentHistoryScore / BattleTrendScore |
| 通知 | Delegate / Gameplay Event | 完了・失敗・中断を必要な連携先へ通知する |
| フィードバック | UI / Animation / VFX / SE | AIデバッグ表示に回避率・距離傾向等を表示する |
...
| クラス・システム | 責務 | 実装区分 |
|---|---|---|
| `Boss ActionHistory / UEnemyAttackEvaluator` | 実行可否、状態遷移、主要ロジック、重複防止 | `C++` |
| `BossAttackDefinition` | RecentWindowSeconds / TrendWeightsとAsset参照 | `Data Asset / Config` |
...
- RecentWindowSeconds / TrendWeightsを保持する。
...
bool CanEvaluateActionHistory() const;
void RequestEvaluateActionHistory();
void CancelEvaluateActionHistory();
bool IsEvaluateActionHistoryActive() const;
```

### 12.2 通知イベント

```cpp
DECLARE_MULTICAST_DELEGATE(FOnEvaluateActionHistoryStarted);
DECLARE_MULTICAST_DELEGATE(FOnEvaluateActionHistoryCompleted);
DECLARE_MULTICAST_DELEGATE(FOnEvaluateActionHistoryCancelled);
DECLARE_MULTICAST_DELEGATE_OneParam(FOnEvaluateActionHistoryFailed, EFailureReason);
```
...
[ABossAIController]
        |
        v
[Boss ActionHistory / UEnemyAttackEvaluator]
   |          |
   v          v
[BossAttackDefinition]  [Boss / Player]
        |
        v
...
|---|---|---:|---|---|
| RecentWindowSeconds / TrendWeights | Data Asset / Struct / Config | TBD | BossAttackDefinition | 要件固有の判定・調整データ |
...
- [ ] 時間窓外の履歴は短期集計から除外され、全体集計はEncounter終了まで維持される。
- [ ] 成功通知が1回発行され、一時状態が完了時に解除される。
- [ ] AIデバッグ表示に回避率・距離傾向等を表示する。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-FR-BOSS-007-001` | 正常系 | すべての事前条件を満たす | 確定済みPlayer Action履歴を発生させる | 時間窓外の履歴は短期集計から除外され、全体集計はEncounter終了まで維持される |
| `TC-FR-BOSS-007-002` | 異常系 | 必須参照または実行条件を1つ無効にする | 同じ要求を発生させる | 状態とリソースを変更せず安全に拒否する |
| `TC-FR-BOSS-007-003` | 多重・中断 | 処理中または連続要求 | 再要求、死亡、Owner破棄のいずれかを発生させる | 重複せず一時状態を解除しCrashしない |
...
| ID | 内容 | 影響範囲 | 決定期限 | 状態 |
|---|---|---|---|---|
| `Q-FR-BOSS-007` | 直近時間窓と傾向評価の重みはTBD | 設計・データ・テスト | 実装Issue開始前 | Open |
...
|---|---|---|---|---|
| `ADR-FR-BOSS-007-001` | 責務配置 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | Blueprintだけにルールと状態を保持する | 状態整合性、検証容易性、データ調整性を保つため |
...
|---:|---|---|---|
| 1 | `FR-BOSS-007: データ・状態・公開Interface` | 必須データ、状態、Validation、APIが定義される | なし |
| 2 | `FR-BOSS-007: 主要ロジック` | 正常・拒否・中断・多重実行テストが通る | #1 |
| 3 | `FR-BOSS-007: Asset・UI・結合確認` | 必須Assetを設定し受入条件を確認できる | #2 |
