...
| 要件種別 | 機能要件 |
| 要件カテゴリ | ステージ |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「導入テキストを表示する」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
...
- 元要件「導入テキストを表示する」を実現する。
- 進行状態に応じて導入Widgetを一度表示し、決定入力で閉じて操作を開始する。
...
| `AActionGameMode / Intro Widget` | 実行可否判定、状態変更、主要処理を担当する |
...
- 新規開始または該当進行状態で、Text DataとWidget Classが有効か検証する。
...
4. 進行状態に応じて導入Widgetを一度表示し、決定入力で閉じて操作を開始する。
5. 導入Widgetと既読状態を反映する。
...
|---|---|---|---|
| `State.UI.Intro` | 要件の状態・受付・通知 | 処理開始または条件成立時 | 完了・中断・条件解除時 |
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 実行入力 | InputAction / Gameplay Event / System Event | Yes | 新規開始Event、導入Text Data |
| 状態・対象参照 | Gameplay Tag / Attribute / Weak Object Reference | Yes | 新規開始または該当進行状態で、Text DataとWidget Classが有効か検証する |
| 設定データ | Data Asset / Config | Yes | IntroText / WidgetClass |

### 10.2 出力

| 出力項目 | 型・形式 | 説明 |
|---|---|---|
| 実行結果 | State / Attribute / Result Event | 導入Widgetと既読状態 |
| 通知 | Delegate / Gameplay Event | 完了・失敗・中断を必要な連携先へ通知する |
| フィードバック | UI / Animation / VFX / SE | ローカライズ可能なTextと入力案内を表示する |
...
| クラス・システム | 責務 | 実装区分 |
|---|---|---|
| `AActionGameMode / Intro Widget` | 実行可否、状態遷移、主要ロジック、重複防止 | `C++` |
| `Stage Data / Data Asset` | IntroText / WidgetClassとAsset参照 | `Data Asset / Config` |
...
bool CanShowIntroText() const;
void RequestShowIntroText();
void CancelShowIntroText();
bool IsShowIntroTextActive() const;
```

### 12.2 通知イベント

```cpp
DECLARE_MULTICAST_DELEGATE(FOnShowIntroTextStarted);
DECLARE_MULTICAST_DELEGATE(FOnShowIntroTextCompleted);
DECLARE_MULTICAST_DELEGATE(FOnShowIntroTextCancelled);
DECLARE_MULTICAST_DELEGATE_OneParam(FOnShowIntroTextFailed, EFailureReason);
```
...
|---|---|---:|---|---|
| IntroText / WidgetClass | Data Asset / Struct / Config | TBD | Stage Data / Data Asset | 要件固有の判定・調整データ |
...
- [ ] 新規開始時に全文が表示され、閉じた後にPlayer操作が可能になる。
- [ ] 成功通知が1回発行され、一時状態が完了時に解除される。
- [ ] ローカライズ可能なTextと入力案内を表示する。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-FR-STAGE-001-001` | 正常系 | すべての事前条件を満たす | 新規開始Event、導入Text Dataを発生させる | 新規開始時に全文が表示され、閉じた後にPlayer操作が可能になる |
| `TC-FR-STAGE-001-002` | 異常系 | 必須参照または実行条件を1つ無効にする | 同じ要求を発生させる | 状態とリソースを変更せず安全に拒否する |
| `TC-FR-STAGE-001-003` | 多重・中断 | 処理中または連続要求 | 再要求、死亡、Owner破棄のいずれかを発生させる | 重複せず一時状態を解除しCrashしない |
...
| ID | 内容 | 影響範囲 | 決定期限 | 状態 |
|---|---|---|---|---|
| `Q-FR-STAGE-001` | 導入テキスト本文と演出仕様はTBD | 設計・データ・テスト | 実装Issue開始前 | Open |
...
|---|---|---|---|---|
| `ADR-FR-STAGE-001-001` | 責務配置 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | Blueprintだけにルールと状態を保持する | 状態整合性、検証容易性、データ調整性を保つため |
...
|---:|---|---|---|
| 1 | `FR-STAGE-001: データ・状態・公開Interface` | 必須データ、状態、Validation、APIが定義される | なし |
| 2 | `FR-STAGE-001: 主要ロジック` | 正常・拒否・中断・多重実行テストが通る | #1 |
| 3 | `FR-STAGE-001: Asset・UI・結合確認` | 必須Assetを設定し受入条件を確認できる | #2 |
