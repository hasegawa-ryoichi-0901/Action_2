# FR-STAGE-011 ボス登場演出は工数に余裕がある場合に実装する
...
|---|---|
| 要件ID | `FR-STAGE-011` |
| 要件名 | ボス登場演出は工数に余裕がある場合に実装する |
| 要件種別 | 機能要件 |
| 要件カテゴリ | ステージ |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「ボス登場演出は工数に余裕がある場合に実装する」 |
| 優先度 | `Could` |
| 設計状態 | `Draft` |
| 対応範囲 | 追加要件（現時点のVertical Slice対象外） |
| 関連Issue | 未割当 |
| 関連PR | 未割当 |
| 関連設計書 | [Docs/02_GameDesign.md](../../02_GameDesign.md)<br>[Docs/07_ClassDesign.md](../../07_ClassDesign.md)<br>[Docs/08_DataDesign.md](../../08_DataDesign.md)<br>[Docs/09_SaveCheckpointDeath.md](../../09_SaveCheckpointDeath.md) |

---

## 2. 目的

Boss戦開始の期待感と状態遷移を演出で明確にする。
...
- 元要件「ボス登場演出は工数に余裕がある場合に実装する」を実現する。
- 入力制御とCamera Sequenceを開始し、完了・Skip・中断で確実にEncounterへ遷移する。
...
| アクター | 役割 |
|---|---|
| `Player / AActionGameMode` | 入力または処理開始条件を発行する |
| `Boss Encounter / Level Sequence` | 実行可否判定、状態変更、主要処理を担当する |
| `Player / Stage Actor` | 処理結果の反映または影響を受ける |
...
- Boss Encounter開始前、演出Asset有効、未再生であることを検証する。
...
4. 入力制御とCamera Sequenceを開始し、完了・Skip・中断で確実にEncounterへ遷移する。
5. Intro演出完了Eventを反映する。
...
|---|---|---|---|
| `State.Cinematic.BossIntro` | 要件の状態・受付・通知 | 処理開始または条件成立時 | 完了・中断・条件解除時 |
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 実行入力 | InputAction / Gameplay Event / System Event | Yes | Boss Arena進入Event |
| 状態・対象参照 | Gameplay Tag / Attribute / Weak Object Reference | Yes | Boss Encounter開始前、演出Asset有効、未再生であることを検証する |
| 設定データ | Data Asset / Config | Yes | LevelSequence / SkipPolicy |

### 10.2 出力

| 出力項目 | 型・形式 | 説明 |
|---|---|---|
| 実行結果 | State / Attribute / Result Event | Intro演出完了Event |
| 通知 | Delegate / Gameplay Event | 完了・失敗・中断を必要な連携先へ通知する |
| フィードバック | UI / Animation / VFX / SE | Camera・Animation・VFX・SE・字幕を同期する |
...
| クラス・システム | 責務 | 実装区分 |
|---|---|---|
| `Boss Encounter / Level Sequence` | 実行可否、状態遷移、主要ロジック、重複防止 | `C++` |
| `Stage Data / Data Asset` | LevelSequence / SkipPolicyとAsset参照 | `Data Asset / Config` |
...
bool CanPlayBossIntro() const;
void RequestPlayBossIntro();
void CancelPlayBossIntro();
bool IsPlayBossIntroActive() const;
```

### 12.2 通知イベント

```cpp
DECLARE_MULTICAST_DELEGATE(FOnPlayBossIntroStarted);
DECLARE_MULTICAST_DELEGATE(FOnPlayBossIntroCompleted);
DECLARE_MULTICAST_DELEGATE(FOnPlayBossIntroCancelled);
DECLARE_MULTICAST_DELEGATE_OneParam(FOnPlayBossIntroFailed, EFailureReason);
```

### 12.3 呼び出し方向

```text
[Player / AActionGameMode]
        |
        v
[Boss Encounter / Level Sequence]
   |          |
   v          v
[Stage Data / Data Asset]  [Player / Stage Actor]
        |
        v
...
|---|---|---:|---|---|
| LevelSequence / SkipPolicy | Data Asset / Struct / Config | TBD | Stage Data / Data Asset | 要件固有の判定・調整データ |
...
| 種別 | 内容 |
|---|---|
| UI | Camera・Animation・VFX・SE・字幕を同期する |
...
- [ ] 演出を実装する場合、完了またはSkip後にBoss戦が開始し進行不能にならない。
- [ ] 成功通知が1回発行され、一時状態が完了時に解除される。
- [ ] Camera・Animation・VFX・SE・字幕を同期する。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-FR-STAGE-011-001` | 正常系 | すべての事前条件を満たす | Boss Arena進入Eventを発生させる | 演出を実装する場合、完了またはSkip後にBoss戦が開始し進行不能にならない |
| `TC-FR-STAGE-011-002` | 異常系 | 必須参照または実行条件を1つ無効にする | 同じ要求を発生させる | 状態とリソースを変更せず安全に拒否する |
| `TC-FR-STAGE-011-003` | 多重・中断 | 処理中または連続要求 | 再要求、死亡、Owner破棄のいずれかを発生させる | 重複せず一時状態を解除しCrashしない |
...
| ID | 内容 | 影響範囲 | 決定期限 | 状態 |
|---|---|---|---|---|
| `Q-FR-STAGE-011` | 工数判定、演出内容、Skip仕様はTBD | 設計・データ・テスト | 実装Issue開始前 | Open |
...
|---|---|---|---|---|
| `ADR-FR-STAGE-011-001` | 責務配置 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | Blueprintだけにルールと状態を保持する | 状態整合性、検証容易性、データ調整性を保つため |
...
|---:|---|---|---|
| 1 | `FR-STAGE-011: データ・状態・公開Interface` | 必須データ、状態、Validation、APIが定義される | なし |
| 2 | `FR-STAGE-011: 主要ロジック` | 正常・拒否・中断・多重実行テストが通る | #1 |
| 3 | `FR-STAGE-011: Asset・UI・結合確認` | 必須Assetを設定し受入条件を確認できる | #2 |
