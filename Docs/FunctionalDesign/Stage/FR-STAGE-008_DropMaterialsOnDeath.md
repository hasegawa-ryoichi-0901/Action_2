...
| 要件種別 | 機能要件 |
| 要件カテゴリ | ステージ |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「死亡時に強化素材を落とす」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
...
- 元要件「死亡時に強化素材を落とす」を実現する。
- 所持素材を0へ移し、死亡位置へSourceDeathId付きADeathDropActorを生成する。
...
| `Death System / ADeathDropActor` | 実行可否判定、状態変更、主要処理を担当する |
...
- Player死亡確定、所持素材量、死亡位置、既存Drop参照を検証する。
...
[開始条件成立]
      |
      v
[Player Death Event、所持素材、位置を受け付ける]
      |
      v
[Player死亡確定、所持素材量、死亡位置、既存Drop参照を検証する]
      |
...
4. 所持素材を0へ移し、死亡位置へSourceDeathId付きADeathDropActorを生成する。
5. DeathDropと所持素材0を反映する。
...
|---|---|---|---|
| `State.Dead` | 要件の状態・受付・通知 | 処理開始または条件成立時 | 完了・中断・条件解除時 |
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 実行入力 | InputAction / Gameplay Event / System Event | Yes | Player Death Event、所持素材、位置 |
| 状態・対象参照 | Gameplay Tag / Attribute / Weak Object Reference | Yes | Player死亡確定、所持素材量、死亡位置、既存Drop参照を検証する |
| 設定データ | Data Asset / Config | Yes | DroppedMaterialAmount / SourceDeathId |

### 10.2 出力

| 出力項目 | 型・形式 | 説明 |
|---|---|---|
| 実行結果 | State / Attribute / Result Event | DeathDropと所持素材0 |
| 通知 | Delegate / Gameplay Event | 完了・失敗・中断を必要な連携先へ通知する |
| フィードバック | UI / Animation / VFX / SE | 素材落下位置を識別可能に表示する |
...
| クラス・システム | 責務 | 実装区分 |
|---|---|---|
| `Death System / ADeathDropActor` | 実行可否、状態遷移、主要ロジック、重複防止 | `C++` |
| `Stage Data / Data Asset` | DroppedMaterialAmount / SourceDeathIdとAsset参照 | `Data Asset / Config` |
...
- DroppedMaterialAmount / SourceDeathIdを保持する。
...
bool CanCreateDeathDrop() const;
void RequestCreateDeathDrop();
void CancelCreateDeathDrop();
bool IsCreateDeathDropActive() const;
```

### 12.2 通知イベント

```cpp
DECLARE_MULTICAST_DELEGATE(FOnCreateDeathDropStarted);
DECLARE_MULTICAST_DELEGATE(FOnCreateDeathDropCompleted);
DECLARE_MULTICAST_DELEGATE(FOnCreateDeathDropCancelled);
DECLARE_MULTICAST_DELEGATE_OneParam(FOnCreateDeathDropFailed, EFailureReason);
```
...
|---|---|---:|---|---|
| DroppedMaterialAmount / SourceDeathId | Data Asset / Struct / Config | Data Asset定義値 | Stage Data / Data Asset | 要件固有の判定・調整データ |
...
- [ ] 素材所持中の死亡で同量のDropが1つ生成され、Player所持量が0になる。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-FR-STAGE-008-001` | 正常系 | すべての事前条件を満たす | Player Death Event、所持素材、位置を発生させる | 素材所持中の死亡で同量のDropが1つ生成され、Player所持量が0になる |
| `TC-FR-STAGE-008-002` | 異常系 | 必須参照または実行条件を1つ無効にする | 同じ要求を発生させる | 状態とリソースを変更せず安全に拒否する |
| `TC-FR-STAGE-008-003` | 多重・中断 | 処理中または連続要求 | 再要求、死亡、Owner破棄のいずれかを発生させる | 重複せず一時状態を解除しCrashしない |
...
|---|---|---|---|---|
| `ADR-FR-STAGE-008-001` | 責務配置 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | Blueprintだけにルールと状態を保持する | 状態整合性、検証容易性、データ調整性を保つため |
...
|---:|---|---|---|
| 1 | `FR-STAGE-008: データ・状態・公開Interface` | 必須データ、状態、Validation、APIが定義される | なし |
| 2 | `FR-STAGE-008: 主要ロジック` | 正常・拒否・中断・多重実行テストが通る | #1 |
| 3 | `FR-STAGE-008: Asset・UI・結合確認` | 必須Assetを設定し受入条件を確認できる | #2 |
