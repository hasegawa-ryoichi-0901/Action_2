...
|---|---|
| 要件ID | `FR-STAGE-005` |
| 要件名 | チェックポイントで回復アイテムを補充する |
| 要件種別 | 機能要件 |
| 要件カテゴリ | ステージ |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「チェックポイントで回復アイテムを補充する」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
...
- 元要件「チェックポイントで回復アイテムを補充する」を実現する。
- HealingItemCountをMaxHealingItemCountへ設定する。
...
| `UCheckpointSubsystem / CharacterAttributeSet` | 実行可否判定、状態変更、主要処理を担当する |
...
- Checkpoint Rest成立とMaxHealingItemCountが有効か検証する。
...
4. HealingItemCountをMaxHealingItemCountへ設定する。
5. HealingItemCountを反映する。
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 実行入力 | InputAction / Gameplay Event / System Event | Yes | Checkpoint Rest Event |
| 状態・対象参照 | Gameplay Tag / Attribute / Weak Object Reference | Yes | Checkpoint Rest成立とMaxHealingItemCountが有効か検証する |
| 設定データ | Data Asset / Config | Yes | MaxHealingItemCount |

### 10.2 出力

| 出力項目 | 型・形式 | 説明 |
|---|---|---|
| 実行結果 | State / Attribute / Result Event | HealingItemCount |
| 通知 | Delegate / Gameplay Event | 完了・失敗・中断を必要な連携先へ通知する |
| フィードバック | UI / Animation / VFX / SE | HUDとチェックポイント結果表示へ補充数を反映する |
...
| クラス・システム | 責務 | 実装区分 |
|---|---|---|
| `UCheckpointSubsystem / CharacterAttributeSet` | 実行可否、状態遷移、主要ロジック、重複防止 | `C++` |
| `Stage Data / Data Asset` | MaxHealingItemCountとAsset参照 | `Data Asset / Config` |
...
bool CanRefillHealingItems() const;
void RequestRefillHealingItems();
void CancelRefillHealingItems();
bool IsRefillHealingItemsActive() const;
```

### 12.2 通知イベント

```cpp
DECLARE_MULTICAST_DELEGATE(FOnRefillHealingItemsStarted);
DECLARE_MULTICAST_DELEGATE(FOnRefillHealingItemsCompleted);
DECLARE_MULTICAST_DELEGATE(FOnRefillHealingItemsCancelled);
DECLARE_MULTICAST_DELEGATE_OneParam(FOnRefillHealingItemsFailed, EFailureReason);
```
...
[Player / AActionGameMode]
        |
        v
[UCheckpointSubsystem / CharacterAttributeSet]
   |          |
   v          v
...
|---|---|---:|---|---|
| MaxHealingItemCount | Data Asset / Struct / Config | Data Asset定義値 | Stage Data / Data Asset | 要件固有の判定・調整データ |
...
- [ ] Checkpoint利用後に回復アイテム数が上限と一致し、上限を超えない。
- [ ] 成功通知が1回発行され、一時状態が完了時に解除される。
- [ ] HUDとチェックポイント結果表示へ補充数を反映する。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-FR-STAGE-005-001` | 正常系 | すべての事前条件を満たす | Checkpoint Rest Eventを発生させる | Checkpoint利用後に回復アイテム数が上限と一致し、上限を超えない |
| `TC-FR-STAGE-005-002` | 異常系 | 必須参照または実行条件を1つ無効にする | 同じ要求を発生させる | 状態とリソースを変更せず安全に拒否する |
| `TC-FR-STAGE-005-003` | 多重・中断 | 処理中または連続要求 | 再要求、死亡、Owner破棄のいずれかを発生させる | 重複せず一時状態を解除しCrashしない |
...
|---|---|---|---|---|
| `ADR-FR-STAGE-005-001` | 責務配置 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | Blueprintだけにルールと状態を保持する | 状態整合性、検証容易性、データ調整性を保つため |
...
|---:|---|---|---|
| 1 | `FR-STAGE-005: データ・状態・公開Interface` | 必須データ、状態、Validation、APIが定義される | なし |
| 2 | `FR-STAGE-005: 主要ロジック` | 正常・拒否・中断・多重実行テストが通る | #1 |
| 3 | `FR-STAGE-005: Asset・UI・結合確認` | 必須Assetを設定し受入条件を確認できる | #2 |
