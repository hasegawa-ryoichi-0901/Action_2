...
| 要件種別 | 機能要件 |
| 要件カテゴリ | ステージ |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「チェックポイントを使用できる」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
...
- 元要件「チェックポイントを使用できる」を実現する。
- InteractionでRestを開始しActiveCheckpointId更新と各連携処理を一度ずつ呼ぶ。
...
| `ACheckpointActor / UCheckpointSubsystem` | 実行可否判定、状態変更、主要処理を担当する |
...
- Playerが有効なCheckpointの操作範囲内で生存していることを検証する。
...
- ActiveCheckpointと休息状態が要件どおり反映される。
...
4. InteractionでRestを開始しActiveCheckpointId更新と各連携処理を一度ずつ呼ぶ。
5. ActiveCheckpointと休息状態を反映する。
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 実行入力 | InputAction / Gameplay Event / System Event | Yes | Interact入力、CheckpointId |
| 状態・対象参照 | Gameplay Tag / Attribute / Weak Object Reference | Yes | Playerが有効なCheckpointの操作範囲内で生存していることを検証する |
| 設定データ | Data Asset / Config | Yes | CheckpointId |

### 10.2 出力

| 出力項目 | 型・形式 | 説明 |
|---|---|---|
| 実行結果 | State / Attribute / Result Event | ActiveCheckpointと休息状態 |
| 通知 | Delegate / Gameplay Event | 完了・失敗・中断を必要な連携先へ通知する |
| フィードバック | UI / Animation / VFX / SE | 利用可能表示とチェックポイントメニューを表示する |
...
| クラス・システム | 責務 | 実装区分 |
|---|---|---|
| `ACheckpointActor / UCheckpointSubsystem` | 実行可否、状態遷移、主要ロジック、重複防止 | `C++` |
| `Stage Data / Data Asset` | CheckpointIdとAsset参照 | `Data Asset / Config` |
...
void RequestUseCheckpoint();
void CancelUseCheckpoint();
bool IsUseCheckpointActive() const;
```

### 12.2 通知イベント

```cpp
DECLARE_MULTICAST_DELEGATE(FOnUseCheckpointStarted);
DECLARE_MULTICAST_DELEGATE(FOnUseCheckpointCompleted);
DECLARE_MULTICAST_DELEGATE(FOnUseCheckpointCancelled);
DECLARE_MULTICAST_DELEGATE_OneParam(FOnUseCheckpointFailed, EFailureReason);
```
...
[Player / AActionGameMode]
        |
        v
[ACheckpointActor / UCheckpointSubsystem]
   |          |
   v          v
...
|---|---|---:|---|---|
| CheckpointId | Data Asset / Struct / Config | Data Asset定義値 | Stage Data / Data Asset | 要件固有の判定・調整データ |
...
- [ ] 有効なCheckpointを操作するとActiveCheckpointが更新され、メニューを終了して操作へ戻れる。
- [ ] 成功通知が1回発行され、一時状態が完了時に解除される。
- [ ] 利用可能表示とチェックポイントメニューを表示する。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-FR-STAGE-003-001` | 正常系 | すべての事前条件を満たす | Interact入力、CheckpointIdを発生させる | 有効なCheckpointを操作するとActiveCheckpointが更新され、メニューを終了して操作へ戻れる |
| `TC-FR-STAGE-003-002` | 異常系 | 必須参照または実行条件を1つ無効にする | 同じ要求を発生させる | 状態とリソースを変更せず安全に拒否する |
| `TC-FR-STAGE-003-003` | 多重・中断 | 処理中または連続要求 | 再要求、死亡、Owner破棄のいずれかを発生させる | 重複せず一時状態を解除しCrashしない |
...
|---|---|---|---|---|
| `ADR-FR-STAGE-003-001` | 責務配置 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | Blueprintだけにルールと状態を保持する | 状態整合性、検証容易性、データ調整性を保つため |
...
|---:|---|---|---|
| 1 | `FR-STAGE-003: データ・状態・公開Interface` | 必須データ、状態、Validation、APIが定義される | なし |
| 2 | `FR-STAGE-003: 主要ロジック` | 正常・拒否・中断・多重実行テストが通る | #1 |
| 3 | `FR-STAGE-003: Asset・UI・結合確認` | 必須Assetを設定し受入条件を確認できる | #2 |
