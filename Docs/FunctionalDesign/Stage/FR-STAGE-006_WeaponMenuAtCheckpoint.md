...
|---|---|
| 要件ID | `FR-STAGE-006` |
| 要件名 | チェックポイントで武器を変更する |
| 要件種別 | 機能要件 |
| 要件カテゴリ | ステージ |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「チェックポイントで武器を変更する」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
...
- 元要件「チェックポイントで武器を変更する」を実現する。
- Checkpoint MenuからWeapon Menuを開き、確定選択をUCombatComponentへ要求する。
...
| `Checkpoint Widget / UCombatComponent` | 実行可否判定、状態変更、主要処理を担当する |
...
- Checkpoint利用中かつWeaponDefinition一覧が有効か検証する。
...
4. Checkpoint MenuからWeapon Menuを開き、確定選択をUCombatComponentへ要求する。
5. 武器変更要求と選択表示を反映する。
...
|---|---|---|---|
| `State.UI.WeaponMenu` | 要件の状態・受付・通知 | 処理開始または条件成立時 | 完了・中断・条件解除時 |
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 実行入力 | InputAction / Gameplay Event / System Event | Yes | 武器メニュー操作、WeaponId |
| 状態・対象参照 | Gameplay Tag / Attribute / Weak Object Reference | Yes | Checkpoint利用中かつWeaponDefinition一覧が有効か検証する |
| 設定データ | Data Asset / Config | Yes | WeaponDefinition一覧 |

### 10.2 出力

| 出力項目 | 型・形式 | 説明 |
|---|---|---|
| 実行結果 | State / Attribute / Result Event | 武器変更要求と選択表示 |
| 通知 | Delegate / Gameplay Event | 完了・失敗・中断を必要な連携先へ通知する |
| フィードバック | UI / Animation / VFX / SE | 現在装備・武器特性・確定結果を表示する |
...
| クラス・システム | 責務 | 実装区分 |
|---|---|---|
| `Checkpoint Widget / UCombatComponent` | 実行可否、状態遷移、主要ロジック、重複防止 | `C++` |
| `Stage Data / Data Asset` | WeaponDefinition一覧とAsset参照 | `Data Asset / Config` |
...
bool CanOpenWeaponMenu() const;
void RequestOpenWeaponMenu();
void CancelOpenWeaponMenu();
bool IsOpenWeaponMenuActive() const;
```

### 12.2 通知イベント

```cpp
DECLARE_MULTICAST_DELEGATE(FOnOpenWeaponMenuStarted);
DECLARE_MULTICAST_DELEGATE(FOnOpenWeaponMenuCompleted);
DECLARE_MULTICAST_DELEGATE(FOnOpenWeaponMenuCancelled);
DECLARE_MULTICAST_DELEGATE_OneParam(FOnOpenWeaponMenuFailed, EFailureReason);
```
...
[Player / AActionGameMode]
        |
        v
[Checkpoint Widget / UCombatComponent]
   |          |
   v          v
...
|---|---|---:|---|---|
| WeaponDefinition一覧 | Data Asset / Struct / Config | Data Asset定義値 | Stage Data / Data Asset | 要件固有の判定・調整データ |
...
- [ ] Checkpoint利用中だけメニューを開け、剣・斧・弓の確定選択が装備へ反映される。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-FR-STAGE-006-001` | 正常系 | すべての事前条件を満たす | 武器メニュー操作、WeaponIdを発生させる | Checkpoint利用中だけメニューを開け、剣・斧・弓の確定選択が装備へ反映される |
| `TC-FR-STAGE-006-002` | 異常系 | 必須参照または実行条件を1つ無効にする | 同じ要求を発生させる | 状態とリソースを変更せず安全に拒否する |
| `TC-FR-STAGE-006-003` | 多重・中断 | 処理中または連続要求 | 再要求、死亡、Owner破棄のいずれかを発生させる | 重複せず一時状態を解除しCrashしない |
...
|---|---|---|---|---|
| `ADR-FR-STAGE-006-001` | 責務配置 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | Blueprintだけにルールと状態を保持する | 状態整合性、検証容易性、データ調整性を保つため |
...
|---:|---|---|---|
| 1 | `FR-STAGE-006: データ・状態・公開Interface` | 必須データ、状態、Validation、APIが定義される | なし |
| 2 | `FR-STAGE-006: 主要ロジック` | 正常・拒否・中断・多重実行テストが通る | #1 |
| 3 | `FR-STAGE-006: Asset・UI・結合確認` | 必須Assetを設定し受入条件を確認できる | #2 |
