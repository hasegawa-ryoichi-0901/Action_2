...
| 要件種別 | 機能要件 |
| 要件カテゴリ | ステージ |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「落とした素材を回収できる」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
...
- 元要件「落とした素材を回収できる」を実現する。
- InteractionまたはOverlapでDroppedMaterialAmountを1回加算しDropを破棄する。
...
| `UInteractionComponent / ADeathDropActor` | 実行可否判定、状態変更、主要処理を担当する |
...
- PlayerとDeathDropが有効で未回収、重複処理中でないことを検証する。
...
- UpgradeMaterial加算とDrop消滅が要件どおり反映される。
...
4. InteractionまたはOverlapでDroppedMaterialAmountを1回加算しDropを破棄する。
5. UpgradeMaterial加算とDrop消滅を反映する。
...
|---|---|---|---|
| `Event.DeathDrop.Recovered` | 要件の状態・受付・通知 | 処理開始または条件成立時 | 完了・中断・条件解除時 |
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 実行入力 | InputAction / Gameplay Event / System Event | Yes | DeathDrop接触 |
| 状態・対象参照 | Gameplay Tag / Attribute / Weak Object Reference | Yes | PlayerとDeathDropが有効で未回収、重複処理中でないことを検証する |
| 設定データ | Data Asset / Config | Yes | DroppedMaterialAmount |

### 10.2 出力

| 出力項目 | 型・形式 | 説明 |
|---|---|---|
| 実行結果 | State / Attribute / Result Event | UpgradeMaterial加算とDrop消滅 |
| 通知 | Delegate / Gameplay Event | 完了・失敗・中断を必要な連携先へ通知する |
| フィードバック | UI / Animation / VFX / SE | 回収VFX・SEと素材HUDを更新する |
...
| クラス・システム | 責務 | 実装区分 |
|---|---|---|
| `UInteractionComponent / ADeathDropActor` | 実行可否、状態遷移、主要ロジック、重複防止 | `C++` |
| `Stage Data / Data Asset` | DroppedMaterialAmountとAsset参照 | `Data Asset / Config` |
...
bool CanRecoverDeathDrop() const;
void RequestRecoverDeathDrop();
void CancelRecoverDeathDrop();
bool IsRecoverDeathDropActive() const;
```

### 12.2 通知イベント

```cpp
DECLARE_MULTICAST_DELEGATE(FOnRecoverDeathDropStarted);
DECLARE_MULTICAST_DELEGATE(FOnRecoverDeathDropCompleted);
DECLARE_MULTICAST_DELEGATE(FOnRecoverDeathDropCancelled);
DECLARE_MULTICAST_DELEGATE_OneParam(FOnRecoverDeathDropFailed, EFailureReason);
```
...
[Player / AActionGameMode]
        |
        v
[UInteractionComponent / ADeathDropActor]
   |          |
   v          v
...
|---|---|---:|---|---|
| DroppedMaterialAmount | Data Asset / Struct / Config | Data Asset定義値 | Stage Data / Data Asset | 要件固有の判定・調整データ |
...
- [ ] 有効Dropへ接触すると保存量と同量が1回だけ加算され、Dropが消える。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-FR-STAGE-009-001` | 正常系 | すべての事前条件を満たす | DeathDrop接触を発生させる | 有効Dropへ接触すると保存量と同量が1回だけ加算され、Dropが消える |
| `TC-FR-STAGE-009-002` | 異常系 | 必須参照または実行条件を1つ無効にする | 同じ要求を発生させる | 状態とリソースを変更せず安全に拒否する |
| `TC-FR-STAGE-009-003` | 多重・中断 | 処理中または連続要求 | 再要求、死亡、Owner破棄のいずれかを発生させる | 重複せず一時状態を解除しCrashしない |
...
|---|---|---|---|---|
| `ADR-FR-STAGE-009-001` | 責務配置 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | Blueprintだけにルールと状態を保持する | 状態整合性、検証容易性、データ調整性を保つため |
...
|---:|---|---|---|
| 1 | `FR-STAGE-009: データ・状態・公開Interface` | 必須データ、状態、Validation、APIが定義される | なし |
| 2 | `FR-STAGE-009: 主要ロジック` | 正常・拒否・中断・多重実行テストが通る | #1 |
| 3 | `FR-STAGE-009: Asset・UI・結合確認` | 必須Assetを設定し受入条件を確認できる | #2 |
