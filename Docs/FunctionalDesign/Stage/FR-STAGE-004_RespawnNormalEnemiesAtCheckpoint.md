...
|---|---|
| 要件ID | `FR-STAGE-004` |
| 要件名 | チェックポイント使用時に通常敵が復活する |
| 要件種別 | 機能要件 |
| 要件カテゴリ | ステージ |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「チェックポイント使用時に通常敵が復活する」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
...
休息にリスクと再攻略コストを持たせ、ゲームループをリセットする。
...
- 元要件「チェックポイント使用時に通常敵が復活する」を実現する。
- UCheckpointSubsystemが通常敵の生存状態を初期配置へ戻す。Boss撃破状態は対象外とする。
...
| `UCheckpointSubsystem / Enemy Spawn System` | 実行可否判定、状態変更、主要処理を担当する |
...
- Checkpoint利用成立後、登録済み通常敵のSpawn Definitionを検証する。
...
[開始条件成立]
      |
      v
[Checkpoint Rest Event、敵登録情報を受け付ける]
      |
      v
[Checkpoint利用成立後、登録済み通常敵のSpawn Definitionを検証する]
      |
...
4. UCheckpointSubsystemが通常敵の生存状態を初期配置へ戻す。Boss撃破状態は対象外とする。
5. 通常敵の初期状態を反映する。
...
|---|---|---|---|
| `Event.Checkpoint.Rest` | 要件の状態・受付・通知 | 処理開始または条件成立時 | 完了・中断・条件解除時 |
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 実行入力 | InputAction / Gameplay Event / System Event | Yes | Checkpoint Rest Event、敵登録情報 |
| 状態・対象参照 | Gameplay Tag / Attribute / Weak Object Reference | Yes | Checkpoint利用成立後、登録済み通常敵のSpawn Definitionを検証する |
| 設定データ | Data Asset / Config | Yes | Enemy Spawn Id / ResetPolicy |

### 10.2 出力

| 出力項目 | 型・形式 | 説明 |
|---|---|---|
| 実行結果 | State / Attribute / Result Event | 通常敵の初期状態 |
| 通知 | Delegate / Gameplay Event | 完了・失敗・中断を必要な連携先へ通知する |
| フィードバック | UI / Animation / VFX / SE | 画面外または安全な遷移中に復活を完了する |
...
| クラス・システム | 責務 | 実装区分 |
|---|---|---|
| `UCheckpointSubsystem / Enemy Spawn System` | 実行可否、状態遷移、主要ロジック、重複防止 | `C++` |
| `Stage Data / Data Asset` | Enemy Spawn Id / ResetPolicyとAsset参照 | `Data Asset / Config` |
...
- Enemy Spawn Id / ResetPolicyを保持する。
...
bool CanResetNormalEnemies() const;
void RequestResetNormalEnemies();
void CancelResetNormalEnemies();
bool IsResetNormalEnemiesActive() const;
```

### 12.2 通知イベント

```cpp
DECLARE_MULTICAST_DELEGATE(FOnResetNormalEnemiesStarted);
DECLARE_MULTICAST_DELEGATE(FOnResetNormalEnemiesCompleted);
DECLARE_MULTICAST_DELEGATE(FOnResetNormalEnemiesCancelled);
DECLARE_MULTICAST_DELEGATE_OneParam(FOnResetNormalEnemiesFailed, EFailureReason);
```
...
[Player / AActionGameMode]
        |
        v
[UCheckpointSubsystem / Enemy Spawn System]
   |          |
   v          v
...
|---|---|---:|---|---|
| Enemy Spawn Id / ResetPolicy | Data Asset / Struct / Config | Data Asset定義値 | Stage Data / Data Asset | 要件固有の判定・調整データ |
...
- [ ] Checkpoint使用完了後、撃破済み通常敵が初期位置・初期属性で1体ずつ復活する。
- [ ] 成功通知が1回発行され、一時状態が完了時に解除される。
- [ ] 画面外または安全な遷移中に復活を完了する。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-FR-STAGE-004-001` | 正常系 | すべての事前条件を満たす | Checkpoint Rest Event、敵登録情報を発生させる | Checkpoint使用完了後、撃破済み通常敵が初期位置・初期属性で1体ずつ復活する |
| `TC-FR-STAGE-004-002` | 異常系 | 必須参照または実行条件を1つ無効にする | 同じ要求を発生させる | 状態とリソースを変更せず安全に拒否する |
| `TC-FR-STAGE-004-003` | 多重・中断 | 処理中または連続要求 | 再要求、死亡、Owner破棄のいずれかを発生させる | 重複せず一時状態を解除しCrashしない |
...
|---|---|---|---|---|
| `ADR-FR-STAGE-004-001` | 責務配置 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | Blueprintだけにルールと状態を保持する | 状態整合性、検証容易性、データ調整性を保つため |
...
|---:|---|---|---|
| 1 | `FR-STAGE-004: データ・状態・公開Interface` | 必須データ、状態、Validation、APIが定義される | なし |
| 2 | `FR-STAGE-004: 主要ロジック` | 正常・拒否・中断・多重実行テストが通る | #1 |
| 3 | `FR-STAGE-004: Asset・UI・結合確認` | 必須Assetを設定し受入条件を確認できる | #2 |
