...
| 要件種別 | 機能要件 |
| 要件カテゴリ | ステージ |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「チュートリアルを実施する」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
...
移動・戦闘・チェックポイントの必須操作を実戦前に学習可能にする。
...
- 元要件「チュートリアルを実施する」を実現する。
- Stepごとの説明を表示し、実際の操作Event成立で次Stepへ進める。
...
| `Tutorial System / UI` | 実行可否判定、状態変更、主要処理を担当する |
...
- Tutorial Step Data、対象InputAction、進行条件が有効か検証する。
...
[開始条件成立]
      |
      v
[Tutorial開始Event、Player Action Eventを受け付ける]
      |
      v
[Tutorial Step Data、対象InputAction、進行条件が有効か検証する]
      |
...
4. Stepごとの説明を表示し、実際の操作Event成立で次Stepへ進める。
5. Step進行と完了状態を反映する。
...
|---|---|---|---|
| `State.Tutorial.Active` | 要件の状態・受付・通知 | 処理開始または条件成立時 | 完了・中断・条件解除時 |
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 実行入力 | InputAction / Gameplay Event / System Event | Yes | Tutorial開始Event、Player Action Event |
| 状態・対象参照 | Gameplay Tag / Attribute / Weak Object Reference | Yes | Tutorial Step Data、対象InputAction、進行条件が有効か検証する |
| 設定データ | Data Asset / Config | Yes | Tutorial Step Data |

### 10.2 出力

| 出力項目 | 型・形式 | 説明 |
|---|---|---|
| 実行結果 | State / Attribute / Result Event | Step進行と完了状態 |
| 通知 | Delegate / Gameplay Event | 完了・失敗・中断を必要な連携先へ通知する |
| フィードバック | UI / Animation / VFX / SE | 現在操作と達成状態をWidget表示する |
...
| クラス・システム | 責務 | 実装区分 |
|---|---|---|
| `Tutorial System / UI` | 実行可否、状態遷移、主要ロジック、重複防止 | `C++` |
| `Stage Data / Data Asset` | Tutorial Step DataとAsset参照 | `Data Asset / Config` |
...
bool CanRunTutorial() const;
void RequestRunTutorial();
void CancelRunTutorial();
bool IsRunTutorialActive() const;
```

### 12.2 通知イベント

```cpp
DECLARE_MULTICAST_DELEGATE(FOnRunTutorialStarted);
DECLARE_MULTICAST_DELEGATE(FOnRunTutorialCompleted);
DECLARE_MULTICAST_DELEGATE(FOnRunTutorialCancelled);
DECLARE_MULTICAST_DELEGATE_OneParam(FOnRunTutorialFailed, EFailureReason);
```
...
|---|---|---:|---|---|
| Tutorial Step Data | Data Asset / Struct / Config | TBD | Stage Data / Data Asset | 要件固有の判定・調整データ |
...
- [ ] 必須Stepを順に完了でき、完了済みStepは同一実行中に重複完了しない。
- [ ] 成功通知が1回発行され、一時状態が完了時に解除される。
- [ ] 現在操作と達成状態をWidget表示する。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-FR-STAGE-002-001` | 正常系 | すべての事前条件を満たす | Tutorial開始Event、Player Action Eventを発生させる | 必須Stepを順に完了でき、完了済みStepは同一実行中に重複完了しない |
| `TC-FR-STAGE-002-002` | 異常系 | 必須参照または実行条件を1つ無効にする | 同じ要求を発生させる | 状態とリソースを変更せず安全に拒否する |
| `TC-FR-STAGE-002-003` | 多重・中断 | 処理中または連続要求 | 再要求、死亡、Owner破棄のいずれかを発生させる | 重複せず一時状態を解除しCrashしない |
...
| ID | 内容 | 影響範囲 | 決定期限 | 状態 |
|---|---|---|---|---|
| `Q-FR-STAGE-002` | チュートリアル項目・再表示・スキップ可否はTBD | 設計・データ・テスト | 実装Issue開始前 | Open |
...
|---|---|---|---|---|
| `ADR-FR-STAGE-002-001` | 責務配置 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | Blueprintだけにルールと状態を保持する | 状態整合性、検証容易性、データ調整性を保つため |
...
|---:|---|---|---|
| 1 | `FR-STAGE-002: データ・状態・公開Interface` | 必須データ、状態、Validation、APIが定義される | なし |
| 2 | `FR-STAGE-002: 主要ロジック` | 正常・拒否・中断・多重実行テストが通る | #1 |
| 3 | `FR-STAGE-002: Asset・UI・結合確認` | 必須Assetを設定し受入条件を確認できる | #2 |
