...
|---|---|
| 要件ID | `FR-STAGE-010` |
| 要件名 | 回収前に再死亡すると以前の素材が消滅する |
| 要件種別 | 機能要件 |
| 要件カテゴリ | ステージ |
| 元要件 | [Docs/01_Requirements.md](../../01_Requirements.md)「回収前に再死亡すると以前の素材が消滅する」 |
| 優先度 | `Must` |
| 設計状態 | `Draft` |
...
死亡損失ルールを一つの未回収Dropに限定し、無制限なDrop蓄積を防ぐ。
...
- 元要件「回収前に再死亡すると以前の素材が消滅する」を実現する。
- 既存Dropを破棄してから新しいSourceDeathIdのDropを生成する。
...
| `Death System / UCheckpointSubsystem` | 実行可否判定、状態変更、主要処理を担当する |
...
[開始条件成立]
      |
      v
[Player Death Event、既存Drop参照を受け付ける]
      |
      v
[新しい死亡処理前に既存未回収Drop参照を検証する]
      |
...
4. 既存Dropを破棄してから新しいSourceDeathIdのDropを生成する。
5. 新しいDropのみを反映する。
...
|---|---|---|---|
| `Event.Player.Death` | 要件の状態・受付・通知 | 処理開始または条件成立時 | 完了・中断・条件解除時 |
...
| 入力項目 | 型・形式 | 必須 | 説明 |
|---|---|---:|---|
| 実行入力 | InputAction / Gameplay Event / System Event | Yes | Player Death Event、既存Drop参照 |
| 状態・対象参照 | Gameplay Tag / Attribute / Weak Object Reference | Yes | 新しい死亡処理前に既存未回収Drop参照を検証する |
| 設定データ | Data Asset / Config | Yes | ActiveDeathDrop参照 |

### 10.2 出力

| 出力項目 | 型・形式 | 説明 |
|---|---|---|
| 実行結果 | State / Attribute / Result Event | 新しいDropのみ |
| 通知 | Delegate / Gameplay Event | 完了・失敗・中断を必要な連携先へ通知する |
| フィードバック | UI / Animation / VFX / SE | 旧Dropマーカーを除去し新しい位置へ更新する |
...
| クラス・システム | 責務 | 実装区分 |
|---|---|---|
| `Death System / UCheckpointSubsystem` | 実行可否、状態遷移、主要ロジック、重複防止 | `C++` |
| `Stage Data / Data Asset` | ActiveDeathDrop参照とAsset参照 | `Data Asset / Config` |
...
bool CanReplaceDeathDrop() const;
void RequestReplaceDeathDrop();
void CancelReplaceDeathDrop();
bool IsReplaceDeathDropActive() const;
```

### 12.2 通知イベント

```cpp
DECLARE_MULTICAST_DELEGATE(FOnReplaceDeathDropStarted);
DECLARE_MULTICAST_DELEGATE(FOnReplaceDeathDropCompleted);
DECLARE_MULTICAST_DELEGATE(FOnReplaceDeathDropCancelled);
DECLARE_MULTICAST_DELEGATE_OneParam(FOnReplaceDeathDropFailed, EFailureReason);
```
...
[Player / AActionGameMode]
        |
        v
[Death System / UCheckpointSubsystem]
   |          |
   v          v
...
|---|---|---:|---|---|
| ActiveDeathDrop参照 | Data Asset / Struct / Config | Data Asset定義値 | Stage Data / Data Asset | 要件固有の判定・調整データ |
...
- [ ] 未回収Dropがある状態で再死亡すると旧Dropが消え、最新死亡分のDropだけが1つ存在する。
- [ ] 成功通知が1回発行され、一時状態が完了時に解除される。
- [ ] 旧Dropマーカーを除去し新しい位置へ更新する。
...
| ID | 分類 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| `TC-FR-STAGE-010-001` | 正常系 | すべての事前条件を満たす | Player Death Event、既存Drop参照を発生させる | 未回収Dropがある状態で再死亡すると旧Dropが消え、最新死亡分のDropだけが1つ存在する |
| `TC-FR-STAGE-010-002` | 異常系 | 必須参照または実行条件を1つ無効にする | 同じ要求を発生させる | 状態とリソースを変更せず安全に拒否する |
| `TC-FR-STAGE-010-003` | 多重・中断 | 処理中または連続要求 | 再要求、死亡、Owner破棄のいずれかを発生させる | 重複せず一時状態を解除しCrashしない |
...
|---|---|---|---|---|
| `ADR-FR-STAGE-010-001` | 責務配置 | C++で状態と判定を管理し、調整値とアセット参照はデータへ分離する | Blueprintだけにルールと状態を保持する | 状態整合性、検証容易性、データ調整性を保つため |
...
|---:|---|---|---|
| 1 | `FR-STAGE-010: データ・状態・公開Interface` | 必須データ、状態、Validation、APIが定義される | なし |
| 2 | `FR-STAGE-010: 主要ロジック` | 正常・拒否・中断・多重実行テストが通る | #1 |
| 3 | `FR-STAGE-010: Asset・UI・結合確認` | 必須Assetを設定し受入条件を確認できる | #2 |
